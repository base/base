...
`base-reth-node`, `base-builder`, and `ingress-rpc` bind
...
so Rust drop order runs the drain after they have stopped. Ingress joins or
cancels builder forwarding, audit publishing, and health-check tasks before its
guard drops.
