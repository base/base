...
- **Removed `reth-node-ethereum` from `base-testing-devnet`.** No Rust source reference to that dependency was found in the builder crate. Removed its test-utils feature forwarding as well. This also removes `reth-ethereum-consensus` and `reth-ethereum-payload-builder` from the Base production graph.
- **Deleted the three Ethereum implementation crates after migrating their consumers.** Engine-tree, RocksDB, and test-harness end-to-end scenarios now run under `base-node-service` against `BaseNode`. Shared engine, stage, RPC, and ExEx tests use component fixtures. Removed the Ethereum RLP-import test launcher and its format-specific scenario; Base initialization and maintenance commands remain available.
...
- Made the existing `revm/p256-aws-lc-rs` production feature explicit in `base-node-service`; previously the unused Ethereum node dependency enabled it. Removing a dependency should not silently change the cryptographic backend used by Base.
...
- **ExEx:** `base-execution-engine-observers` uses `base-execution-engine-observers` for proof history, and `base-execution-state-indexer` also consumes ExEx. The manager, notifications, WAL, and pruning coordination are not unused plugin machinery.
...
| [base-node-service](../../crates/node/service/Cargo.toml) | Optional capability; separate removal decision |
