...
echo "=== Ethereum Devnet Genesis Generator ==="
...
mkdir -p "$OUTPUT_DIR/el" "$OUTPUT_DIR/cl"
...
# =============================================================================
# Generate EL (Execution Layer) Genesis
# =============================================================================
echo ""
...
# Build alloc object
ALLOC="{"
for i in "${!ANVIL_ACCOUNTS[@]}"; do
  if [ $i -gt 0 ]; then
    ALLOC+=","
  fi
  ALLOC+="\"${ANVIL_ACCOUNTS[$i]}\": {\"balance\": \"$BALANCE\"}"
done
ALLOC+="}"
...
echo "  EL: $OUTPUT_DIR/el/genesis.json"
