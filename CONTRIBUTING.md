...
`docker compose up` from the repository root pulls the **public operator node** (`ghcr.io/base/node-reth`). Pass `--build` to compile this tree. Use `just devnet up` for the local developer stack.
