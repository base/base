        ...
        + 'static,
{
    ...
    async fn set_metering_info(
        ...
        let (priority_fee, gas_used, execution_time_us) = meter
            .results
            .first()
            .map_or((U256::ZERO, 0, 0), |r| (r.gas_price, r.gas_used, r.execution_time_us));
    ...
    async fn set_metering_enabled(&self, enabled: bool) -> RpcResult<()> {
        ...
        info!(previous = previous, enabled = enabled, "Metering data collection state changed");
        Ok(())
    }
