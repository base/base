...
use alloy_primitives::{B256, Bytes, U256, bytes, hex::FromHex};
use alloy_rpc_client::RpcClient;
use base_bundles::{Bundle, MeterBundleResponse};
use base_reth_metering::{MeteringApiImpl, MeteringApiServer};
use base_reth_test_utils::{BASE_CHAIN_ID, OpAddOns, OpBuilder, TestHarness};
...
/// Custom launcher that adds the MeteringApiImpl RPC extension.
async fn metering_launcher(
    builder: OpBuilder,
...
async fn setup_harness() -> eyre::Result<TestHarness> {
    TestHarness::with_launcher(metering_launcher).await
}
...
async fn test_meter_bundle_single_transaction() -> eyre::Result<()> {
    ...
    let alice_signer = B256::from_hex(accounts.alice.private_key)?;

    // Build a transaction using Alice's account
    ...
    assert_eq!(result.from_address, accounts.alice.address);
    assert_eq!(
        result.to_address,
        Some(alloy_primitives::address!("0x1111111111111111111111111111111111111111"))
    );
...
async fn test_meter_bundle_multiple_transactions() -> eyre::Result<()> {
    ...
    let alice_signer = B256::from_hex(accounts.alice.private_key)?;
    let bob_signer = B256::from_hex(accounts.bob.private_key)?;
...
async fn test_meter_bundle_gas_calculations() -> eyre::Result<()> {
    ...
    // First transaction with 3 gwei gas price from Alice
    let tx1_inner = TransactionBuilder::default()
        .signer(alice_signer)
        .chain_id(BASE_CHAIN_ID)
        .nonce(0)
    ...
    // Second transaction with 7 gwei gas price from Bob
    let tx2_inner = TransactionBuilder::default()
        .signer(bob_signer)
        .chain_id(BASE_CHAIN_ID)
        .nonce(0)
