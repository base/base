...
//! - Solidity ABI definitions for EntryPoint contracts (v0.6, v0.7, v0.8)
...
pub use bytecode::{
    has_v06_bytecode, has_v07_bytecode, has_v08_bytecode,
...
fn decode_handle_ops_v06(calldata: &Bytes) -> Vec<UserOperation> {
    ...
    match IEntryPointV06::handleOpsCall::abi_decode(&calldata[..]) {
        Ok(decoded) => decoded
            ...
            .into_iter()
            .map(|packed| {
                // Extract factory and paymaster from packed fields
                ...
                let (paymaster, paymaster_data) = if packed.paymasterAndData.len() >= 20 {
                    (
