    ...
    pub async fn run<R: Runtime>(mut self, runtime: R) {
        loop {
            tokio::select! {
                ...
                _ = self.safe_head_tx.closed() => break,
            ...
            let result = tokio::select! {
                ...
                _ = runtime.cancelled() => break,
                _ = self.safe_head_tx.closed() => break,
                result = self.provider.safe_l2_head() => result,
            };
            ...
            match result {
                Ok(head) if head != self.last_safe_head => {
                    tokio::select! {
                        biased;
                        _ = runtime.cancelled() => break,
                        result = self.safe_head_tx.send(head) => {
                            if result.is_err() {
                                break;
                            }
                ...
                Ok(_) => {}
                Err(error) => {
                    warn!(error = %error, "failed to poll safe L2 head");
                }
            }
        }
    }
