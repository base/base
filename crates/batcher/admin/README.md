...
| Method | Behavior |
|---|---|
| `admin_stopBatcher` | Stops block ingestion and drops buffered encoding state. Returns once no submission is in flight. If some still are after the driver's drain timeout (twice the resubmission timeout, 96s by default), returns an error; the batcher stays stopped. Stopping a stopped batcher does not reset it again; it waits the same way. |
