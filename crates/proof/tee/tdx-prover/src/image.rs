...
use base_proof_tee_tdx_verifier::{
    ...
    MIN_SIGNATURE_DATA_LEN, MRTD_OFFSET, REPORT_DATA_OFFSET, RTMR_OFFSET,
    SIGNATURE_DATA_LEN_PREFIX_LEN, TDX_MEASUREMENT_LEN, TDX_QUOTE_HEADER_LEN, TDX_QUOTE_VERSION,
    TDX_REPORT_BODY_LEN, TDX_REPORT_DATA_LEN, TDX_TEE_TYPE,
};

const LOCAL_MRTD: [u8; TDX_MEASUREMENT_LEN] = [0x11; TDX_MEASUREMENT_LEN];
const LOCAL_RTMRS: [[u8; TDX_MEASUREMENT_LEN]; 4] = [
    [0x22; TDX_MEASUREMENT_LEN],
    [0x33; TDX_MEASUREMENT_LEN],
    [0x44; TDX_MEASUREMENT_LEN],
    [0x55; TDX_MEASUREMENT_LEN],
];
...
impl TdxQuoteProvider for TdxMeasurements {
    ...
    fn quote(&self, report_data: &[u8; TDX_REPORT_DATA_LEN]) -> TdxRuntimeResult<Bytes> {
        ...
        for (i, rtmr) in LOCAL_RTMRS.iter().enumerate() {
...
mod tests {
    ...
    fn quote_emits_parseable_quote_with_measurements() {
        ...
        assert_eq!([parsed.rtmr0, parsed.rtmr1, parsed.rtmr2, parsed.rtmr3], LOCAL_RTMRS);
    }
}
