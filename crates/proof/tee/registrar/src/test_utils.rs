...
/// Used only by the CRL layer's full 5-cert fixture.
...
/// Garbage DER bytes for parser-failure tests.
pub const INVALID_DER_BYTES: [u8; 4] = [0xDE, 0xAD, 0xBE, 0xEF];

/// Helpers for decoding the hex fixtures above into DER byte vectors.
#[derive(Debug)]
pub struct CertFixtures;

impl CertFixtures {
    /// Decodes a single hex fixture into DER bytes.
    pub fn decode(hex_str: &str) -> Vec<u8> {
        hex::decode(hex_str).expect("static hex fixture decodes")
    }

    /// Decodes a sequence of hex fixtures into an owned chain of DER vectors.
    pub fn decode_chain(hexes: &[&str]) -> Vec<Vec<u8>> {
        hexes.iter().map(|h| Self::decode(h)).collect()
    }
}
