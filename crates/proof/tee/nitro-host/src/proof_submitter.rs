    ...
    Client: ProverWorkerProvider,
{
    ...
    pub async fn submit_until_delivered(
        ...
        let response = (|| {
            attempts_for_submit.fetch_add(1, Ordering::Relaxed);
            self.client.submit_proof(request_for_submit.clone())
        })
    ...
    Client: ProverWorkerProvider + 'static,
{
    ...
    pub fn spawn_until_delivered(
        ...
        tokio::spawn(async move { self.submit_until_delivered(request).await })
    }
}
...
mod tests {
    ...
    async fn submitter_can_run_as_spawned_task() {
        ...
        let handle = submitter.spawn_until_delivered(submit_request());
        let response = handle
            .await
