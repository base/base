...
fn main() {
    ...
    let input = TdxVerifierInput::decode(&input_bytes).expect("failed to decode TDX input");
    let journal = TdxVerifier::verify(&input).expect("TDX attestation verification failed");
