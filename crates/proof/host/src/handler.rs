...
use base_common_client_ethereum::{Base, Network, Provider};
...
use base_consensus_batch::{BlockInfo, OutputRoot};
use base_consensus_source::BlobWithCommitmentAndProof;
use base_proof_execution::{Hint, HintType, ROOTS_OF_UNITY};
...
impl PayloadWitnessPrefetcher {
    ...
    async fn prefetch_block_inner(&self, kv: SharedKeyValueStore, block_number: u64) -> bool {
        ...
        let execute_payload_response = match base_common_observability_metrics::time!(
...
pub(crate) async fn handle_hint_with_prefetchers(
    ...
    let _timer =
        base_common_observability_metrics::timed!(Metrics::hint_duration_seconds(hint_type_label));
...
mod tests {
    ...
    use base_common_client_ethereum::{
        Base, RootProvider, builder as provider_builder, mock::Asserter,
    };
    use base_consensus_source::{OnlineBeaconClient, OnlineBlobProvider};
    use base_proof_types::ProofRequest;
