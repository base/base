//! Contains a concrete implementation of the [`KeyValueStore`] trait that returns local
//! preimage data from [`HostArgs`].
...
use crate::{HostArgs, KeyValueStore, Result};

/// A synchronous key-value store that returns local preimage data from [`HostArgs`].
#[derive(Debug)]
...
impl LocalInputs {
    /// Creates a new [`LocalInputs`] with the given [`HostArgs`].
    pub const fn new(cfg: HostArgs) -> Self {
        Self { cfg }
    }
}
