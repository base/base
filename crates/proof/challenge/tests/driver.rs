...
use base_challenger::{
    ...
    DisputeProofManager, Driver, DriverComponents, GameScanner, L1HeadProvider, OutputValidator,
    PendingProof, PendingProofs, ProofKind, ProofPhase, ProofUpdate,
    test_utils::{
...
async fn test_step_nullification_failure_preserves_proof() {
    ...
    let entry = driver
        ...
        .pending_proofs()
        .get(&addr(0))
        .expect("proof should be preserved");
    assert!(
...
async fn test_step_invalid_game_no_tee_provider_zk_only() {
    ...
    let entry = driver
        ...
        .pending_proofs()
        .get(&addr(0))
        .expect("ZK proof should be pending");
    assert!(
        matches!(entry.phase, ProofPhase::AwaitingProof { .. }),
...
async fn test_step_no_dispute_skips_proving() {
    ...
    let mut driver: Driver<MockL2Provider, MockZkProofProvider, MockTxManager> =
        Driver::new(DriverComponents {
            dispute: None,
            submitter,
            verifier_client: verifier as Arc<dyn AggregateVerifierClient>,
