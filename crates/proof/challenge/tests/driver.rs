...
use base_challenger::{
    AnchorUpdater, ChallengeSubmitter, ChallengerProofAdapter, DisputeComponents, DisputeIntent,
    DisputeProofManager, Driver, DriverComponents, DriverConfig, GameScanner, OutputValidator,
    PendingProof, ProofKind, ProofPhase,
    test_utils::{
...
fn test_driver(
    ...
    test_driver_with_l1_provider(
        factory,
        verifier,
        l2_provider,
        zk_prover,
        tx_manager,
        Arc::new(MockL1::failure("unused")),
    )
}
...
fn driver_with_ready_proof(
    ...
    driver.dispute.as_mut().unwrap().proof_manager.pending_proofs_mut()
...
async fn test_step_valid_proposals_are_skipped() {
    for game_state in [
        ...
        assert_eq!(driver.dispute.as_ref().unwrap().proof_manager.pending_proofs_len(), 0);
    }
}
...
async fn test_step_invalid_game_proof_succeeded() {
    ...
    driver.step().await.unwrap();
    assert!(
        driver.dispute.as_ref().unwrap().proof_manager.pending_proofs().contains_key(&addr(0)),
...
async fn test_step_nullification_failure_preserves_proof() {
    ...
    let entry =
        driver.dispute.as_ref().unwrap().proof_manager.pending_proofs().get(&addr(0)).expect("proof should be preserved");
    assert!(
...
async fn test_poll_or_submit_drops_stale_challenge_proofs() {
    for game_state in [
        ...
        assert!(!driver.dispute.as_ref().unwrap().proof_manager.pending_proofs().contains_key(&addr(0)));
    }
}
...
async fn test_step_proof_retry_reuses_deterministic_session_id() {
    ...
    let entry = driver.dispute.as_ref().unwrap().proof_manager
        .pending_proofs()
        .get(&addr(0))
...
async fn test_successful_nullify_does_not_track_anchor_update() {
    ...
    let mut driver = test_driver(factory, verifier, l2, default_zk_prover(), default_tx_manager());
    driver.dispute.as_mut().unwrap().proof_manager.pending_proofs_mut().insert(
        addr(0),
        PendingProof {
...
async fn test_step_fraudulent_zk_challenge_legitimate_skips() {
    ...
    assert!(
        driver.dispute.as_ref().unwrap().proof_manager.pending_proofs_len() == 0,
...
async fn test_step_fraudulent_zk_challenge_nullifies_despite_earlier_invalid_root() {
    ...
    let entry =
        driver.dispute.as_ref().unwrap().proof_manager.pending_proofs().get(&addr(0)).expect(
...
#[tokio::test]
async fn test_step_dual_proof_invalid_tee_request_failure_falls_back_to_zk_nullify() {
    // A game with both TEE and ZK proofs verified (via verifyProposalProof,
    // not challenge) where the output roots are invalid and the TEE request
    // fails should fall back to a ZK proof with DisputeIntent::Nullify.
    let (l2, factory, root_15, _root_20) = base_game_mocks();
    ...
    assert_eq!(
        entry.intent,
        DisputeIntent::Nullify,
        "dual-proof game with a failed TEE request should fall back to ZK Nullify"
    );
}
...
/// the gate holds; the dispute pipeline was never entered. The bond/anchor
...
async fn test_step_no_dispute_skips_proving() {
    ...
    let anchor_updater = AnchorUpdater::new(
        ...
        default_l2() as Arc<dyn base_proof_rpc::L2Provider>,
        Address::repeat_byte(0xAA),
        1,
        100,
        100,
