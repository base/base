...
pub enum ProofKind {
    ...
    ///
    /// Carries a pre-built ZK fallback request and intent so the driver can
    /// initiate a ZK proof if the TEE transaction submission fails on-chain.
    Tee {
        /// Pre-built ZK request for fallback if TEE submission fails.
        zk_fallback_request: ProveBlockRequest,
        /// The dispute intent to use for the ZK fallback path.
        zk_fallback_intent: DisputeIntent,
    },
...
impl PendingProof {
    ...
    /// provided `zk_fallback_request` and `zk_fallback_intent`.
