...
//! work, bond discovery, and anchor maintenance. In no-dispute mode
//! (`dispute` is `None`) only the bond/anchor lifecycle runs.
...
/// no-dispute mode. The single `Option` records the mode and keeps the
/// scanning and proving deps all-present-or-all-absent — they cannot disagree.
...
/// Runtime dispute pipeline: game scanning paired with proof-lifecycle
/// management. Held by the [`Driver`] as `Option<DisputePipeline>`.
...
impl<L2: L2Provider, P: ProofRequesterProvider, T: TxManager> Driver<L2, P, T> {
    ...
    pub fn new(components: DriverComponents<L2, P, T>) -> Self {
        Self {
            dispute: components.dispute.map(|dispute| DisputePipeline {
    ...
    pub async fn run(mut self) {
        ...
        while !self.cancel.is_cancelled() {
            ...
            ChallengerMetrics::pending_proofs().set(
                self.dispute.as_ref().map_or(0.0, |d| d.proof_manager.pending_proofs_len() as f64),
            );
            ChallengerMetrics::ignored_games().set(
                self.dispute.as_ref().map_or(0.0, |d| d.proof_manager.ignored_games_len() as f64),
            );
    ...
    async fn process_candidate(&mut self, candidate: CandidateGame) -> eyre::Result<()> {
        ...
        if dispute.proof_manager.is_ignored(game_address) {
            debug!(game = %game_address, "skipping ignored game");
            return Ok(());
        }

        if dispute.proof_manager.has_pending_proof(game_address) {
    ...
    async fn validate_game(
        ...
        let dispute = self
            ...
            .as_ref()
            .expect("dispute components must be set unless --no-dispute is set");
        match dispute.proof_manager.validate_intermediate_roots(params).await {
    ...
    async fn process_invalid_proposal(
        ...
        let sender_address = self.submitter.sender_address();
        self.dispute
            .as_mut()
            .expect("dispute components must be set unless --no-dispute is set")
            .proof_manager
