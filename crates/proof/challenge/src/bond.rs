...
impl<C: Clock> BondManager<C> {
    ...
    /// Returns the effective `DelayedWETH` withdrawal delay, falling back to
    /// `Self::DEFAULT_WETH_DELAY` when the onchain delay has not yet been
    /// read. Both the `check_delay` and `submit_claim_credit` retry-backoff
    /// paths must agree on this value, so the fallback is centralized here.
    pub fn effective_weth_delay(&self) -> Duration {
        self.weth_delay.unwrap_or(Self::DEFAULT_WETH_DELAY)
    }
    ...
    fn check_delay(
        ...
        let delay = self.effective_weth_delay();
