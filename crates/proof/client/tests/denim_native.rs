//! Hermetic native Denim range-proof fixture.
//!
...
use base_proof_client::{FaultProofBlock, FaultProofProgramError, Prologue};
...
const CLAIM_BLOCK: u64 = 7;
const DENIM_TIMESTAMP: u64 = 14;
...
impl Fixture {
    fn new(
        ...
        Self { store, expected, schedule_id }
    }

    async fn replay(&self) -> Result<(BootInfo, Vec<FaultProofBlock>), FaultProofProgramError> {
        let boot = BootInfo::load(&self.store).await?;
        let oracle = FixtureOracle(self.store.clone());
        let driver =
            Prologue::new(oracle.clone(), oracle, BaseEvmFactory::default()).load().await?;
...
fn recorded_vectors() -> Vec<ExpectedBlock> {
    vec![
        ExpectedBlock {
            number: 2,
            timestamp: 14,
...
fn assert_blocks(actual: &[FaultProofBlock], expected: &[ExpectedBlock]) {
...
async fn replays_denim_activation_and_rollover_deterministically() {
    let fixture = Fixture::new(None, None, 1, false);
    fixture.store.check_preimages().unwrap();

    let (first_boot, first) = fixture.replay().await.unwrap();
    let (second_boot, second) = fixture.replay().await.unwrap();
...
async fn honors_schedule_override_and_intermediate_intervals() {
    for interval in [1, 2, 10] {
        let fixture = Fixture::new(None, Some(8), interval, false);
        let (boot, blocks) = fixture.replay().await.unwrap();
        assert_eq!(
            boot.schedule_id,
            b256!("3b364659563c6841d115a6d137ade3201a59bfb84f7987f67ee4a45b62f30836")
        );
...
async fn replays_from_same_second_intermediate_safe_head() {
    let fixture = Fixture::new(Some(2), None, 1, false);
    let (_, blocks) = fixture.replay().await.unwrap();
...
async fn rejects_wrong_root_and_block_claims() {
    ...
    assert!(matches!(wrong_root.replay().await, Err(FaultProofProgramError::InvalidClaim { .. })));
    ...
    assert!(matches!(
        malformed_base_time_claim.replay().await,
        Err(FaultProofProgramError::InvalidClaim { claimed, .. }) if claimed == malformed_root
    ));
...
async fn rejects_wrong_schedule_missing_code_and_malformed_batch() {
    let mut wrong_schedule = Fixture::new(None, None, 1, false);
    insert_local(
        &mut wrong_schedule.store,
        L2_SCHEDULE_BLOCK_NUMBER_KEY,
        (CLAIM_BLOCK - 1).to_be_bytes().to_vec(),
    );
    assert!(wrong_schedule.replay().await.is_err());

    let mut missing_code = Fixture::new(None, None, 1, false);
    missing_code
        .store
        .preimage_map
        .remove(&PreimageKey::new(*BaseTime::IMPLEMENTATION_CODE_HASH, PreimageKeyType::Keccak256));
    assert!(missing_code.replay().await.is_err());

    let malformed = Fixture::new(None, None, 1, true);
    assert!(malformed.replay().await.is_err());
}
