...
//!
#[tokio::test]
#[ignore = "requires a running prover-service and L2_NODE_ADDRESS; run with `cargo nextest run --run-ignored all -p base-prover-service --test snark_groth16_e2e`"]
async fn snark_groth16_e2e_prove_and_verify() {
    ...
    base_prover_service::SnarkE2e::run().await.expect("SNARK e2e test failed");
}
