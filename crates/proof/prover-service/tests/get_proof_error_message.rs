...
use base_prover_service::{
    GetProofRequest, ProofStatus, prover_service_client::ProverServiceClient,
};
...
#[tokio::test]
#[ignore = "requires a running prover-service (set PROVER_GRPC_ADDR); run with `cargo nextest run --run-ignored all -p base-prover-service --test get_proof_error_message`"]
