...
SET job_status = CASE status
    ...
    -- RUNNING requests are actively being generated by the legacy backend-session
    -- path at migration time. Park them as CLAIMED with no lock: the claim query only
    -- selects PENDING rows or CLAIMED rows whose lock_expires_at < NOW(), and a NULL
    -- lock_expires_at is never < NOW(), so the worker path can never hand these out and
    -- cause duplicate execution. Their requester `status` lifecycle still completes them.
