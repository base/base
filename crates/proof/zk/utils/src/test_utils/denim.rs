//! Hermetic Denim range-proof fixture shared by packaged backends.
//!
...
//! code, header, and boot preimages consumed by the proof. To verify the vectors, run
...
impl DenimFixture {
    ...
    pub fn new() -> Self {
        ...
        let channel_data = encode_channel(&config, &batches);
        ...
        insert_local(&mut store, L2_OUTPUT_ROOT_KEY, safe_output.hash().to_vec());
        ...
        insert_local(&mut store, INTERMEDIATE_BLOCK_INTERVAL_KEY, 1u64.to_be_bytes().to_vec());
        insert_local(&mut store, L1_HEAD_NUMBER_KEY, 1u64.to_be_bytes().to_vec());

        let mut pinned = config.clone();
        let schedule_timestamp = pinned.l2_block_timestamp(CLAIM_BLOCK);
        let schedule_id = ScheduleId::pin(&mut pinned, schedule_timestamp);
        Self { store, expected, schedule_id, config }
    }
    ...
    pub async fn expected_public_values(&self) -> BootInfoStruct {
        ...
        let roots = self.expected.iter().map(|block| block.output_root).collect();
        BootInfoStruct::new(boot_info, 1, CLAIM_BLOCK, roots)
    }
}
...
#[cfg(test)]
mod tests {
    use super::{DENIM_FIXTURE_CONTENT_HASH, DenimFixture};

    #[test]
    fn fixture_matches_content_hash() {
        assert_eq!(DenimFixture::new().content_hash(), DENIM_FIXTURE_CONTENT_HASH);
    }
}
