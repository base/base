...
impl ProofDispatcher {
    ...
    /// Dispatches every target from the current dispatcher cursor up to `proposal_head`.
    pub async fn tick(&self, current: &mut RecoveredState, proposal_head: u64) {
            ...
            ProofTarget::next_block(current.l2_block_number, self.config.block_interval)
        {
            if target_block > proposal_head {
                debug!(
                    ...
                    target_block,
                    proposal_head,
                    "Proposal head below dispatch target, waiting for L2 head to advance"
                );
                break;
            }
