...
- **`AggregateVerifierClient`**: Querying individual dispute game instances (status, ZK/TEE prover addresses, output roots), reading onchain configuration (`BLOCK_INTERVAL`, `INTERMEDIATE_BLOCK_INTERVAL`, `TEE_IMAGE_HASH`), and constructing state-changing calls such as `nullify` via [`encode_nullify_calldata`].
