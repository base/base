//! Trait implementations for `base-proof-witness-mpt` traits that are effectively a no-op.
...
impl TrieHinter for NoopTrieHinter {
    ...
    fn hint_execution_witness(
        &self,
        _parent_hash: B256,
        _base_payload_attributes: &base_common_types_payload::BasePayloadAttributes,
    ) -> Result<(), Self::Error> {
        Ok(())
    }
}
