//! Everest fork-activation gate for EIP-8130 RPC reads.

use alloy_consensus::BlockHeader;
use alloy_eips::{BlockId, BlockNumberOrTag};
use base_common_chains::Upgrades;
use base_common_rpc_types::{BaseRpcTypes, EIP8130_PRE_EVEREST_RPC_ERROR};
use jsonrpsee_types::{
    ErrorObjectOwned,
...
/// Rejects EIP-8130 RPC reads issued before the Everest hard fork has
/// activated at the requested block.
///
/// Mirrors the txpool's Everest gate (which rejects EIP-8130 transactions
...
/// against pre-Everest blocks are gated even when Everest is currently active.
#[derive(Debug)]
pub struct Eip8130EverestGate;

impl Eip8130EverestGate {
    /// Errors with `INVALID_PARAMS` if Everest is not active at `block_id`'s
        ...
        <<Eth as RpcNodeCore>::Provider as ChainSpecProvider>::ChainSpec: Upgrades,
    {
        ...
        if !provider.chain_spec().is_everest_active_at_timestamp(timestamp) {
            return Err(ErrorObjectOwned::owned(
                INVALID_PARAMS_CODE,
                EIP8130_PRE_EVEREST_RPC_ERROR,
                None::<()>,
            ));
        }
        Ok(())
        ...
        P: BlockReaderIdExt,
    {
        ...
        .map_err(|err| {
            warn!(
                error = %err,
                block_id = ?block_id,
                "Everest gate: failed to resolve block header"
            );
