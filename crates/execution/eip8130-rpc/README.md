...
request carrying EIP-8130 fields (the required `sender` account, account
changes, calls, `nonce_key`, expiry, metadata). It builds an unsigned simulation
transaction — the caller's prefixed `authenticator(20) || data` blob (or a stub
secp256k1 authorization when absent) lets the intrinsic schedule price
