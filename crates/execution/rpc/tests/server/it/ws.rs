...
use base_execution_rpc::{RpcServerConfig, TransportRpcModuleConfig};
...
async fn launch_ws_eth() -> base_execution_rpc::RpcServerHandle {
    launch_ws().await
}
...
async fn test_eth_subscribe_pending_transactions_receives_tx() {
    ...
    use base_execution_rpc::RpcRegistryInner;
