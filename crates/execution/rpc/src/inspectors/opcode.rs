...
use base_execution_evm_runtime::{
    CallInputs, CallOutcome, CallScheme, ContextTr, CreateInputs, CreateOutcome, CreateScheme,
    Inspector, Interpreter, JournalTr, opcode, opcode::OpCode,
};
...
pub fn immediate_size(bytecode: &base_execution_evm_runtime::interpreter::ExtBytecode) -> u8 {
...
mod tests {
    use base_execution_evm_runtime::{
        Bytecode, Bytes, CacheDB, Context, EmptyDB, InputsImpl, MainContext, SharedMemory,
        hardfork::SpecId, interpreter::ExtBytecode,
    };
    ...
    fn test_opcode_counter_inspector() {
        ...
        let mut context = base_execution_evm_runtime::ReferenceContext::mainnet().with_db(db);
        for _ in &opcodes {
