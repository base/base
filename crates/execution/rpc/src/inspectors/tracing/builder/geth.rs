...
use base_execution_evm_runtime::{
    AccountInfo, DatabaseRef, EvmState, HaltReasonTr, KECCAK_EMPTY, ResultAndState, opcode,
};
...
impl<'a> GethTraceBuilder<'a> {
    ...
    /// [[base_execution_evm_runtime::ExecutionResult]] of the executed
    /// transaction.
    pub fn geth_traces(
        &self,
            ...
            for step in &trace.steps {
                ...
                *used_opcodes.entry(op).or_insert(0) += 1;
                ...
                if let Some(status) = &step.status {
                    if *status == base_execution_evm_runtime::InstructionResult::OutOfGas {
                        out_of_gas = true;
                    }
                }
...
mod tests {
    ...
    use base_execution_evm_runtime::{Account, AccountInfo, CacheDB, EmptyDB};
