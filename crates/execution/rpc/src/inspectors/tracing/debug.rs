...
use base_execution_evm_runtime::{
    Address, Block, CallInputs, CallOutcome, ContextTr, CreateInputs, CreateOutcome, DatabaseRef,
    FrameInput, FrameResult, HaltReasonTr, Inspector, Interpreter, Log, ResultAndState,
    Transaction, U256,
};
use thiserror::Error;

use crate::inspectors::tracing::{
...
pub enum DebugInspector {
    ...
    #[cfg(feature = "js-tracer")]
    /// JS tracer
    Js(Box<crate::inspectors::tracing::js::JsInspector>),
}
        ...
        let this = if let Some(tracer) = tracer {
            ...
            match tracer {
                ...
                GethDebugTracerType::JsTracer(code) => {
                    ...
                    Self::Js(crate::inspectors::tracing::js::JsInspector::new(code, config)?.into())
                }
                _ => {
                    // Note: this match is non-exhaustive in case we need to add support for
                    // additional tracers
...
pub enum DebugInspectorError<DBError = core::convert::Infallible> {
    ...
    #[error(transparent)]
    MuxInspector(#[from] crate::inspectors::tracing::MuxError),
    /// Error from JS inspector
    #[cfg(feature = "js-tracer")]
    #[error(transparent)]
    JsInspector(#[from] crate::inspectors::tracing::js::JsInspectorError),
    /// Database error
