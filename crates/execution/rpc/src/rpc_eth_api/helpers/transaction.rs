...
use base_common_types_chain::{
    BaseReceipt, BaseTxEnvelope, BlockHeader, Recovered, RecoveredBlock, SignedTransaction,
...
use crate::{
    ...
    EthApiError::{self},
    FillTransaction, SignError, TransactionConversionError, TransactionSource,
    utils::binary_search,
};
...
/// [`BlockingTaskPool`](base_common_runtime::pool::BlockingTaskPool) begin with `spawn_`
///
/// ## Calls
///
...
impl BaseEthApi {
    ...
    pub fn historical_transaction_by_hash_at(
        &self,
        hash: B256,
    ) -> impl Future<Output = Result<Option<(TransactionSource, B256)>, BaseEthApiError>> + Send
    {
        async move {
...
impl BaseEthApi {
    ...
    ) -> impl Future<Output = Result<Option<(TransactionSource, BlockId)>, BaseEthApiError>> + Send
    {
        async move {
    ...
    pub fn transaction_and_block(
        ...
        Output = Result<Option<(TransactionSource, Arc<RecoveredBlock>)>, BaseEthApiError>,
    > + Send {
        async move {
