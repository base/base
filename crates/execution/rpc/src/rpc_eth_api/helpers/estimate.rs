...
use base_execution_state_types::{ProviderError, StateProvider};
use futures::Future;
use tracing::trace;

use crate::{
    BaseEthApi, BaseEthApiError, EthApiError, RpcInvalidTransactionError, apply_block_overrides,
...
impl BaseEthApi {
        ...
        S: StateProvider,
    {
        ...
        let gas_refund = match res.result {
            ...
            ExecutionResult::Halt { reason, .. } => {
                ...
                // limit
                return Err(BaseEthApiError::from_evm_halt(reason, tx_env.gas_limit()));
            }
            ExecutionResult::Revert { output, .. } => {
                ...
                return if tx_request_gas_limit.is_some() || tx_request_gas_price.is_some() {
                    ...
                    Err(BaseEthApiError::from_revert(output))
                };
            }
        };
    ...
    pub fn map_out_of_gas_err<DB>(
        evm: &mut base_execution_evm_runtime::BaseEvm<
            DB,
            base_execution_evm_runtime::NoOpInspector,
        >,
        mut tx_env: base_execution_evm_runtime::BaseTransaction,
        max_gas_limit: u64,
    ) -> Result<U256, BaseEthApiError>
    where
        ...
        EthApiError: From<DB::Error>,
    {
        ...
        match retry_res.result {
            ...
            ExecutionResult::Halt { reason, .. } => {
                Err(BaseEthApiError::from_evm_halt(reason, req_gas_limit))
            }
        }
    }
}
