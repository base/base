...
use base_common_types_chain::{BaseReceipt, BlockHeader, SealedHeader};
use base_common_types_rpc::{
    Filter, FilterBlockOption, FilterChanges, FilterId, Log, PendingTransactionFilterKind,
};
use base_execution_state_provider::providers::BlockchainProvider;
use base_execution_state_types::{
    BlockHashReader, BlockIdReader, BlockNumReader, HeaderProvider, ProviderError, ReceiptProvider,
};
use base_execution_txpool::{BaseTransactionPool, NewSubpoolTransactionStream, TransactionPool};
use futures::{
    Future,
...
use crate::{
    BaseEthApi, EthApiError, EthFilterApiServer, EthFilterConfig, EthStateCache,
    EthSubscriptionIdProvider, QueryLimits, RpcErrorFactory,
...
impl EthFilter {
    ...
    /// }
    /// ```
    pub fn new(eth_api: BaseEthApi, config: EthFilterConfig, task_spawner: Runtime) -> Self {
...
impl std::fmt::Debug for EthFilter {
...
impl EthFilterInner {
    ...
    fn eth_cache(&self) -> &EthStateCache {
        self.eth_api.cache()
    }
                    ...
                    if let Ok(Some(pending_block)) = self.eth_api.local_pending_block().await {
                        ...
                        if pending_block.block.number() > info.best_number {
                            ...
                            append_matching_block_logs(
                                &mut all_logs,
                                self.eth_api.converter(),
                                ProviderOrBlock::<BlockchainProvider>::Block(pending_block.block),
                                &filter,
                                &header,
...
impl FullTransactionsReceiver {
    ...
    fn new(stream: NewSubpoolTransactionStream, converter: crate::BaseRpcConverter) -> Self {
...
#[async_trait]
impl FullTransactionsFilter<base_common_types_rpc::BaseTransaction> for FullTransactionsReceiver {
    async fn drain(&self) -> FilterChanges<base_common_types_rpc::BaseTransaction> {
...
mod tests {
    ...
    fn build_test_eth_api(provider: MockEthProvider) -> BaseEthApi {
    ...
    async fn test_range_block_mode_queued_results_priority() {
        ...
        let headers = vec![
            SealedHeader::new(
                base_common_types_chain::Header { number: 100, ..Default::default() },
                FixedBytes::random(),
            ),
            SealedHeader::new(
                base_common_types_chain::Header { number: 101, ..Default::default() },
                FixedBytes::random(),
            ),
        ];
        ...
        let mock_receipt_1 = BaseReceipt::Legacy(base_common_types_chain::Receipt {
            cumulative_gas_used: 100_000,
            logs: vec![],
            status: (true).into(),
        });
        let mock_receipt_2 = BaseReceipt::Eip1559(base_common_types_chain::Receipt {
            cumulative_gas_used: 200_000,
            logs: vec![],
            status: (true).into(),
        });
        let mock_receipt_3 = BaseReceipt::Eip2930(base_common_types_chain::Receipt {
        ...
        let mock_result_1 = ReceiptBlockResult {
            ...
            header: SealedHeader::new(
                base_common_types_chain::Header { number: 42, ..Default::default() },
        ...
        let mock_result_2 = ReceiptBlockResult {
            ...
            recovered_block: None,
            header: SealedHeader::new(
                base_common_types_chain::Header { number: 43, ..Default::default() },
        ...
        assert_eq!(
            receipt_result1.receipts[0].as_receipt().cumulative_gas_used,
            mock_receipt_1.as_receipt().cumulative_gas_used
        );
        assert_eq!(
            receipt_result1.receipts[0].as_receipt().status,
            mock_receipt_1.as_receipt().status
        );
        assert_eq!(receipt_result1.receipts[1].tx_type(), mock_receipt_2.tx_type());
        assert_eq!(
            receipt_result1.receipts[1].as_receipt().cumulative_gas_used,
            mock_receipt_2.as_receipt().cumulative_gas_used
        );
        assert_eq!(
            receipt_result1.receipts[1].as_receipt().status,
            mock_receipt_2.as_receipt().status
        );
        ...
        assert_eq!(
            receipt_result2.receipts[0].as_receipt().cumulative_gas_used,
            mock_receipt_3.as_receipt().cumulative_gas_used
        );
        assert_eq!(
            receipt_result2.receipts[0].as_receipt().status,
            mock_receipt_3.as_receipt().status
        );
    ...
    async fn test_range_block_mode_provider_receipts() {
        ...
        let header_1 = base_common_types_chain::Header { number: 100, ..Default::default() };
        let header_2 = base_common_types_chain::Header { number: 101, ..Default::default() };
        let header_3 = base_common_types_chain::Header { number: 102, ..Default::default() };
        ...
        let receipt_100_1 = BaseReceipt::Legacy(base_common_types_chain::Receipt {
            cumulative_gas_used: 21_000,
            logs: vec![mock_log.clone()],
            status: (true).into(),
        });
        let receipt_100_2 = BaseReceipt::Eip1559(base_common_types_chain::Receipt {
            cumulative_gas_used: 42_000,
            logs: vec![mock_log.clone()],
            status: (true).into(),
        });
        let receipt_101_1 = BaseReceipt::Eip2930(base_common_types_chain::Receipt {
        ...
        assert_eq!(
            receipt_result.receipts[0].as_receipt().cumulative_gas_used,
            receipt_100_1.as_receipt().cumulative_gas_used
        );
        assert_eq!(
            receipt_result.receipts[0].as_receipt().status,
            receipt_100_1.as_receipt().status
        );

        assert_eq!(receipt_result.receipts[1].tx_type(), receipt_100_2.tx_type());
        assert_eq!(
            receipt_result.receipts[1].as_receipt().cumulative_gas_used,
            receipt_100_2.as_receipt().cumulative_gas_used
        );
        assert_eq!(
            receipt_result.receipts[1].as_receipt().status,
            receipt_100_2.as_receipt().status
        );
        ...
        assert_eq!(
            receipt_result2.receipts[0].as_receipt().cumulative_gas_used,
            receipt_101_1.as_receipt().cumulative_gas_used
        );
        assert_eq!(
            receipt_result2.receipts[0].as_receipt().status,
            receipt_101_1.as_receipt().status
        );
    ...
    async fn test_range_block_mode_iterator_exhaustion() {
        ...
        let header_100 = base_common_types_chain::Header { number: 100, ..Default::default() };
        let header_101 = base_common_types_chain::Header { number: 101, ..Default::default() };
        ...
        let mock_receipt = BaseReceipt::Legacy(base_common_types_chain::Receipt {
            cumulative_gas_used: 21_000,
            logs: vec![],
            status: (true).into(),
        });
    ...
    async fn test_cached_mode_with_mock_receipts() {
        ...
        assert_eq!(
            receipt_block_result.receipts[0].as_receipt().cumulative_gas_used,
            mock_receipt.as_receipt().cumulative_gas_used
        );
        assert_eq!(
            receipt_block_result.receipts[0].as_receipt().status,
            mock_receipt.as_receipt().status
        );
    ...
    async fn test_non_consecutive_headers_after_bloom_filter() {
        ...
        for i in 100u64..=103 {
            ...
            let block = BaseBlock {
                header,
                body: base_common_types_chain::BaseBlockBody { transactions, ..Default::default() },
            };
            provider.add_block(hash, block);
        }
