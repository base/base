...
mod tests {
    ...
    use base_execution_evm_runtime::CacheDB;
    use base_execution_state_provider::test_utils::MockEthProvider;

    use crate::{BaseRpcConverter, simulate::resolve_transaction};

    #[test]
    fn test_resolve_transaction_empty_request() {
        let builder = BaseRpcConverter::new(
            crate::test_utils::RpcTestUtils::provider(MockEthProvider::default()),
            Default::default(),
        );
        let mut db = CacheDB::<
            base_execution_evm_runtime::EmptyDBTyped<base_execution_state_types::ProviderError>,
        >::default();
    ...
    fn test_resolve_transaction_partial_eip1559() {
        let mut db = CacheDB::<
            base_execution_evm_runtime::EmptyDBTyped<base_execution_state_types::ProviderError>,
        >::default();
        let rpc_converter = BaseRpcConverter::new(
            crate::test_utils::RpcTestUtils::provider(MockEthProvider::default()),
            Default::default(),
        );
