...
use base_execution_evm_runtime::{
    BlockExecutionError, BlockValidationError, EVMError, EvmDatabaseError, HaltReason,
    InvalidHeader, InvalidTransaction, OutOfGasError, bal::BalError,
};
...
use crate::{
    CallError, CallFeesError, EthTxEnvError, RpcErrorFactory, StateOverrideError,
    TransactionConversionError,
    tracing::{DebugInspectorError, MuxError},
};
...
pub enum EthApiError {
    ...
    #[error(
        "pruned history unavailable: requested {requested}, earliest available {earliest_available}"
    )]
    ...
    /// [`AccountOverride`](base_common_types_rpc::state::AccountOverride) is malformed, e.g. invalid
    /// 7702 bytecode.
...
impl From<crate::tracing::js::JsInspectorError> for EthApiError {
    fn from(error: crate::tracing::js::JsInspectorError) -> Self {
        match error {
            err @ crate::tracing::js::JsInspectorError::JsError(_) => {
...
where
    T: Into<Self>,
    TxError: base_execution_evm_blocks::InvalidTxError,
{
