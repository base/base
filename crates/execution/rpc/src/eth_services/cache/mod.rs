...
use base_common_types_chain::{BaseReceipt, BlockHeader, InMemorySize, RecoveredBlock};
use base_execution_evm_runtime::{
    Bytecode, StorageKey, StorageValue,
    bal::{
...
use base_execution_state_types::{
    BalProvider, BlockReader, CanonStateNotification, Chain, ProviderError, ProviderResult,
    TransactionVariant,
};
...
type TransactionHashResponseSender = oneshot::Sender<Option<CachedTransaction>>;
...
impl EthStateCache {
    ...
    pub async fn get_transaction_by_hash(&self, tx_hash: TxHash) -> Option<CachedTransaction> {
        let (response_tx, rx) = oneshot::channel();
...
/// [`StateProvider`](base_execution_state_types::StateProvider) and keeps data fetched via the provider in
...
/// `crate::EthApi` which is typically invoked by the RPC server, which already uses
...
mod tests {
    ...
    use base_common_types_chain::{
        ...
        RecoveredBlock, SealedHeader, transaction::TransactionMeta,
    };
    ...
    use base_execution_state_types::{
        ...
        StoredBlockBodyIndices, TransactionVariant, TransactionsProvider,
    };
    ...
    fn reorg_evicts_cached_headers() {
        ...
        assert!(
            service.headers_cache.insert(block_hash, Header { number: 42, ..Default::default() })
        );
    ...
    impl BalStore for TestBalStore {
        fn insert(
            &self,
            _num_hash: NumHash,
            _bal: base_execution_state_types::RawBal,
        ) -> ProviderResult<()> {
            Ok(())
        }
        ...
        fn bal_stream(&self) -> base_execution_state_types::BalNotificationStream {
            base_execution_state_types::NoopBalStore.bal_stream()
        }
    }
