...
///
/// `TransactionValidity` is the wire extension payload; see [``].
...
impl BuilderApiImpl {
    ...
    pub const fn with_extensions(
        ...
        Self { pool, accept_extensions, max_extension_items }
    }
}
...
impl BuilderApiServer for BuilderApiImpl {
    async fn insert_validated_transaction(&self, tx: ValidatedTransaction) -> RpcResult<()> {
        ...
        // `TransactionValidity`, the default payload.
...
mod tests {
    ...
    fn test_validity(invalid: bool) -> TransactionValidity {
        ...
        TransactionValidity {
            validity: vec![ValidityPredicate::Storage {
                ...
                slot: U256::ZERO,
                mask: if invalid { U256::ZERO } else { U256::MAX },
    ...
    async fn extension_apply_failure_returns_invalid_params() {
        ...
        let tx = validated_transaction(sender, raw, test_validity(true));
        ...
        assert!(
            err.message().contains("outside its mask"),
    ...
    async fn decode_empty_bytes_returns_invalid_params() {
        ...
        let tx = validated_transaction(Address::ZERO, Bytes::new(), TransactionValidity::default());
    ...
    async fn decode_truncated_tx_returns_invalid_params() {
        ...
        let tx = validated_transaction(sender, truncated, TransactionValidity::default());
