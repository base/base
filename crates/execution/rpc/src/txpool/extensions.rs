...
use base_common_chain_config::{ChainSpecProvider, Upgrades};
...
impl SendRawTransactionValidityApiImpl {
...
#[async_trait]
impl SendRawTransactionValidityApiServer for SendRawTransactionValidityApiImpl {
    async fn send_raw_transaction_validity(
        &self,
        tx: Bytes,
        options: SendRawTransactionValidityOptions,
...
#[async_trait]
impl AdminTxPoolApiServer for AdminTxPoolApiImpl {
    async fn drop_sender_transactions(&self, sender: Address) -> RpcResult<Vec<TxHash>> {
...
mod tests {
    ...
    use base_execution_txpool::{
        NoopTransactionPool, TransactionOrigin, test_utils::MockTransaction,
    };
    ...
    fn zenith_provider() -> MockEthProvider {
        let mut genesis = build_test_genesis_zenith();
        genesis.config.chain_id = ChainConfig::mainnet().chain_id;
        MockEthProvider::new()
            .with_chain_spec(BaseChainSpec::from_genesis(genesis))
            .with_genesis_block()
    }

    /// Provider whose latest header predates Zenith activation, so the fork gate is closed.
    fn pre_zenith_provider() -> MockEthProvider {
        MockEthProvider::new().with_chain_spec(BaseChainSpec::mainnet()).with_genesis_block()
    }

    fn validity_rpc(mock: MockEthProvider) -> SendRawTransactionValidityApiImpl {
        let context = crate::test_utils::RpcTestUtils::context(mock);
        SendRawTransactionValidityApiImpl::new(context.pool, context.provider)
    }

    fn testing_pool() -> crate::test_utils::TestPool {
        let mut genesis = build_test_genesis_zenith();
        genesis.config.chain_id = 1;
        let mock = MockEthProvider::new().with_chain_spec(BaseChainSpec::from_genesis(genesis));
    ...
    async fn send_raw_transaction_validity_enforces_configured_predicate_limit() {
        let context = crate::test_utils::RpcTestUtils::context(zenith_provider());
    ...
    async fn test_drop_sender_with_transactions() {
        ...
        let tx1 =
            MockTransaction::eip1559().with_gas_limit(100_000).with_sender(sender1).with_nonce(0);
        let tx2 =
            MockTransaction::eip1559().with_gas_limit(100_000).with_sender(sender1).with_nonce(1);
        let tx3 =
            MockTransaction::eip1559().with_gas_limit(100_000).with_sender(sender2).with_nonce(0);
