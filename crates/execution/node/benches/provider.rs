//! Benchmarks canonical historical-state reads across Reth storage layouts.
//!
//! Run with:
//!
//! ```bash
//! cargo bench -p base-node-core --bench provider
//! ```
...
fn historical_provider_benches(c: &mut Criterion) {
    let fixtures = [
...
criterion_group!(benches, historical_provider_benches);
