...
struct ProofsStaticManifest {
    version: u8,
    database: String,
    /// Per-run RocksDB metadata archive. The legacy complete archive remains
    /// in `components.proofs` until the migration is complete.
    metadata: SingleArchive,
...
impl ProofsDownloader {
    ...
    async fn run_from_manifest(target_dir: &Path, manifest_url: &str) -> Result<()> {
        ...
        for entry in entries {
            if !entry.output_files.is_empty()
                && Self::verify_outputs(target_dir, &entry.output_files)?
            {
    ...
    async fn fetch_manifest_entries(manifest_url: &str) -> Result<Vec<ProofsManifestEntry>> {
        ...
        // Old manifests have only the complete `components.proofs` archive. Keep
        // that exact path working while producers dual-publish both formats.
        let Some(proofs_static) = manifest.proofs_static else {
            let ComponentManifest::Single(legacy) = manifest
                .components
                .get("proofs")
                .ok_or_else(|| eyre::eyre!("manifest has no 'proofs' component"))?
            else {
                eyre::bail!("proofs component must be a single archive");
            };
            return Ok(vec![Self::legacy_entry_from_archive(legacy, archive_base_url)?]);
        };
        ...
        entries.push(Self::entry_from_archive(&proofs_static.metadata, archive_base_url)?);
        Ok(entries)
    }

    /// Builds an entry for the pre-static, complete proofs archive. Legacy
    /// manifests did not require per-output checksums, so extraction retains the
    /// current command's size-verified behavior.
    fn legacy_entry_from_archive(
        archive: &SingleArchive,
        base_url: &str,
    ) -> Result<ProofsManifestEntry> {
        Self::validate_relative_archive_path(&archive.file)?;
        let archive_url = Url::parse(base_url)
            .and_then(|mut base| {
                if !base.path().ends_with('/') {
                    base.set_path(&format!("{}/", base.path()));
                }
                base.join(&archive.file)
            })
            .map_err(|e| eyre::eyre!("invalid legacy proofs archive URL {}: {e}", archive.file))?
            .to_string();
        Ok(ProofsManifestEntry {
            file_name: archive.file.clone(),
...
mod tests {
    ...
    #[tokio::test]
    async fn fetch_manifest_entries_falls_back_to_legacy_proofs_archive() {
        let manifest = serde_json::json!({
            "components": {"proofs": {"file": "proofs.tar.zst", "size": 1, "output_files": []}}
        });
        let (url, handle) = start_test_server(manifest, HashMap::new()).await;
        let entries = ProofsDownloader::fetch_manifest_entries(&url).await.unwrap();
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].file_name, "proofs.tar.zst");
        assert!(entries[0].output_files.is_empty());
        handle.abort();
    }
    ...
    async fn downloads_metadata_and_static_tables_and_reuses_verified_sst() {
        ...
        let manifest = serde_json::json!({
            "components": {
                "proofs": {"file": "proofs.tar.zst", "size": 1, "output_files": []}
            },
            "proofs_static": {
                "version": 1,
                "database": "rocksdb",
