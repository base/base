...
pub enum AuthorizeError {
    ...
    /// A standard (legacy / EIP-2930 / EIP-1559 / EIP-7702) sender recovered to
    /// a live inline self that is not an unrestricted owner. Standard
    /// transactions have no policy/`to` gate, so a scoped or policy-gated k1
    /// self must not be allowed to originate them. EIP-8130 transactions still
    /// accept a scoped self through [`crate::ActorAuthorizer::authorize_k1`].
    #[error("standard transaction requires the unrestricted default EOA on account {account}")]
    StandardSenderNotAdmin {
