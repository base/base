...
impl ActorAuthorizer {
    ...
    /// (full-owner) self costs no extra read. OPERATOR overrides POLICY, so a
    /// self carrying both bits resolves ungated (matches
    /// [`Eip8130Constants::sender_is_policy_gated`]).
    fn inline_self_policy_target(
        ...
        if Eip8130Constants::sender_is_policy_gated(state.default_eoa_scope) {
