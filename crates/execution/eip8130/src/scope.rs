...
#[non_exhaustive]
pub enum Operation {
    /// Authorizing the transaction sender (`SCOPE_OPERATOR`). A `SCOPE_POLICY`
    /// actor without `SCOPE_OPERATOR` cannot send: nothing enforces its policy.
    Sender,
