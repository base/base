...
use alloy_eips::{BlockId, eip1559::ETHEREUM_BLOCK_GAS_LIMIT_30M};
use alloy_hardforks::EthereumHardforks;
use alloy_primitives::Address;
use base_common_chain_config::{BaseChainSpec, ChainSpecProvider};
use base_common_runtime::Runtime;
use base_common_types_chain::{
    BlockHeader, GotExpected, InvalidTransactionError, Transaction, Typed2718,
    constants::{
...
use base_execution_evm_runtime::{Cfg, StoredAccount as Account};
...
use crate::{
    BaseTransactionValidator, LocalTransactionConfig, TransactionValidationOutcome,
    TransactionValidationTaskExecutor,
    error::{Eip7702PoolTransactionError, InvalidPoolTransactionError},
    traits::TransactionOrigin,
};

/// A [`crate::TransactionValidator`] implementation that validates ethereum transaction.
///
...
impl BaseTransactionValidator {
    ...
    ///
    /// See also [`crate::TransactionValidator::validate_transaction`]
    pub fn validate_protocol_one(
        &self,
        origin: TransactionOrigin,
        transaction: crate::BasePooledTransaction,
    ) -> TransactionValidationOutcome {
        let mut state: Option<StateProviderBox> = None;
        self.validate_protocol_one_with_provider(origin, transaction, &mut state, || {
            self.client.latest()
        })
    }
    ...
    pub fn validate_protocol_with_state(
        &self,
        origin: TransactionOrigin,
        transaction: crate::BasePooledTransaction,
        state: &mut Option<Box<dyn AccountInfoReader + Send>>,
    ) -> TransactionValidationOutcome {
        self.validate_protocol_one_with_provider(origin, transaction, state, || {
    ...
    fn validate_protocol_one_with_provider<P, F>(
        &self,
        origin: TransactionOrigin,
        transaction: crate::BasePooledTransaction,
    ...
    pub fn validate_protocol_with_state_provider(
        &self,
        origin: TransactionOrigin,
        transaction: crate::BasePooledTransaction,
    ...
    pub fn validate_stateless(
        ...
        match transaction.ty() {
            ...
            // Recognize supported formats; fork-specific checks follow below.
            LEGACY_TX_TYPE_ID | EIP2930_TX_TYPE_ID | EIP1559_TX_TYPE_ID | EIP7702_TX_TYPE_ID => {}
    ...
    ///
    /// Checks sender account balance, nonce, and bytecode. The
    ...
    fn validate_batch(
        ...
        transactions
            .into_iter()
            .map(|(origin, tx)| {
                self.validate_protocol_one_with_provider(origin, tx, &mut provider, || {
                    self.client.latest()
                })
            })
            .collect()
    ...
    pub fn update_protocol_head(&self, new_tip_block: &base_common_types_chain::Header) {
        // update all forks
...
impl BaseTransactionValidatorBuilder {
    ...
    pub fn build(self) -> BaseTransactionValidator {
        ...
        BaseTransactionValidator {
            client,
            chain_id,
            fork_tracker: Arc::new(fork_tracker),
            block_gas_limit: Arc::new(block_gas_limit),
            tx_fee_cap,
            minimum_priority_fee,
        ...
        .initialize_block_info()
    }

    /// Builds a [`BaseTransactionValidator`] and spawns validation tasks via the
    /// [`TransactionValidationTaskExecutor`]
    ///
...
pub fn ensure_intrinsic_gas(
    ...
    let eip2780 = fork_tracker.is_amsterdam_activated().then(|| {
        base_execution_evm_runtime::Eip2780TxInfo {
    ...
    let gas = base_execution_evm_runtime::gas::calculate_initial_tx_gas(
        spec_id,
        transaction.input(),
...
mod tests {
    ...
    use base_common_types_chain::{SignedTransaction, Transaction};
    use base_execution_evm_runtime::eip3860::MAX_INITCODE_SIZE;
    use base_execution_state_provider::test_utils::{ExtendedAccount, MockEthProvider};

    use super::*;
    use crate::{
        BaseOrdering, BasePooledTransaction, Pool, TransactionPool, error::PoolErrorKind,
    ...
    async fn validate_transaction() {
        ...
        let pool = Pool::new_test(validator, BaseOrdering::default(), Default::default());
    ...
    async fn invalid_on_priority_fee_lower_than_configured_minimum() {
        ...
        let local_outcome =
            validator_local.validate_protocol_one(TransactionOrigin::Local, transaction);
    ...
    async fn priority_fee_validation_applies_to_private_transactions() {
        ...
        let outcome = validator.validate_protocol_one(TransactionOrigin::Private, transaction);
        assert!(outcome.is_invalid());
