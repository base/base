...
use base_common_types_chain::{
    ...
    SignedChange, Transaction, constants::KECCAK_EMPTY,
};
use base_execution_evm_runtime::{
    AccountConfigState as AccountState, AccountConfigurationStorage, AccountInfo, ApplyError,
    AuthorizeError, BasePrecompileError, BaseSpecId, Bytecode, FeeCheck, IntrinsicGas,
    IntrinsicGasInput, JournalCheckpoint, L1BlockInfo, LockStatus, NonceError, NonceManagerStorage,
    NonceMode, NonceValidator, PrecompileStorageProvider, StorageCtx, TransactionAuthorizer,
    TxAuthError, validate_loaded_code_presence,
};
...
pub struct BaseTransactionValidator {
    ...
    /// L2 block.
    pub require_l1_data_gas_fee: bool,
    pub trusted_delegation_targets: Arc<AddressSet>,
    ...
    pub trusted_proxy_code_hashes: Arc<HashSet<B256>>,
    pub limit_class_cache: Arc<RwLock<LimitClassCache>>,
...
impl BaseTransactionValidator {
    ...
    pub fn trusted_proxy_code_hashes(targets: &AddressSet) -> HashSet<B256> {
        targets
            .iter()
...
impl BaseTransactionValidator {
    ...
    pub fn initialize_block_info(self) -> Self {
        ...
        if let Ok(Some(block)) =
            this.client().block_by_number_or_tag(alloy_eips::BlockNumberOrTag::Latest)
        {
    ...
    pub fn with_block_info(mut validator: Self, block_info: BaseL1BlockInfo) -> Self {
        validator.block_info = Arc::new(block_info);
        validator
    }
    ...
    /// See also [`TransactionValidator::validate_transaction`]
    ///
    /// This behaves the same as [`BaseTransactionValidator::validate_one_with_state`], but in
    ...
    fn validate_one_with_state_inner(
        ...
        let outcome = self.validate_protocol_with_state(origin, transaction, state);
    ...
    fn validate_eip8130_full(
        ...
        let local_chain_id = self.chain_spec().chain().id();
    ...
    fn validate_eip8130_create_freshness(
        state: &dyn StateProvider,
        sender: Address,
        account: &base_execution_evm_runtime::StoredAccount,
    ...
    fn account_has_code(
        state: &dyn StateProvider,
        address: Address,
    ) -> Result<bool, base_execution_state_types::ProviderError> {
        Ok(state
            .basic_account(&address)?
            .and_then(|account| account.bytecode_hash)
    ...
    fn validate_eip8130_structural(
        ...
        let limit = self.max_tx_input_bytes();
        if size > limit {
        ...
        signed.validate_admission_static(local_chain_id).map_err(|error| {
            let error = match error {
                base_common_types_chain::Eip8130StaticError::ChainIdMismatch => {
                    InvalidTransactionError::ChainIdMismatch
                }
                base_common_types_chain::Eip8130StaticError::TipAboveFeeCap => {
                    InvalidTransactionError::TipAboveFeeCap
                }
                base_common_types_chain::Eip8130StaticError::ZeroGasOrFee => {
...
impl TransactionValidator for BaseTransactionValidator {
    ...
    fn on_new_head_block(&self, new_tip_block: &SealedBlock) {
        self.update_protocol_head(new_tip_block.header());
...
mod tests {
    ...
    use base_common_chain_config::{BaseChainSpec, BaseChainSpecBuilder, ChainConfig};
    ...
    use base_execution_evm_runtime::{AccountChangeApplier, ConfigChangeAuthorizer};
    use base_execution_state_provider::test_utils::{ExtendedAccount, MockEthProvider};
    use base_execution_txpool::{
        BaseTransactionValidatorBuilder, TransactionOrigin, TransactionValidationOutcome,
    };
    ...
    fn rejects_eip8130_nonce_free_already_expired() {
        ...
        // "already expired".
        let validator = build_test_validator();
        let header = base_common_types_chain::Header { timestamp: 100, ..Default::default() };
