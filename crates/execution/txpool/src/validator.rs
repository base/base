    ...
    Evm: ConfigureEvm,
{
    ...
    /// `block.chainid` vs `0`). Authenticator-address bounds and actor-id
    /// uniqueness are enforced on both `Create.initial_actors` and
    /// `ConfigChange.actor_changes` via [`Self::validate_initial_actors`] and
    /// [`Self::validate_actor_changes`] respectively.
    ///
    ...
    fn validate_actor_changes(changes: &[SignedChange]) -> Result<(), InvalidPoolTransactionError> {
        ...
        for change in changes {
            ...
            // Reject duplicate targets in the same batch.
