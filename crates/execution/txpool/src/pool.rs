    ...
    S: BlobStore + Clone,
{
    ...
    fn validity_replacement_retry(
        ...
        replaced: Option<TxHash>,
        validated: &TransactionValidationOutcome<T>,
    ) -> Option<(TxHash, TransactionValidationOutcome<T>)> {
        ...
        Some((replaced, Self::clone_validated_outcome(validated)?))
    }
        ...
        F: FnMut(TransactionValidationOutcome<T>) -> PoolResult<R>,
    {
        ...
        let Some((replaced_hash, retry)) = retry else {
            return Err(error);
        };

        let mut removed = self.protocol_pool.remove_transactions(vec![replaced_hash]);
        let Some(original) = removed.pop() else {
            return Err(error);
        };
        let restore_origin = original.origin;
        let restore = Self::restore_validated_outcome(
            &retry,
            original.transaction.clone(),
            original.propagate,
        )
        .expect("EIP-1559 replacement has valid restoration metadata");
        debug!(
            transaction = %hash,
        ...
        match insert(retry) {
            ...
            Err(retry_error) => {
                // Reth reports `DiscardedOnInsert` only after the retry was accepted
                // and pool limits were enforced. Standard replacements do not restore
                // the old transaction in this case, so preserve that behavior.
                if matches!(
                    &retry_error.kind,
