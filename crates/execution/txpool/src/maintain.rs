...
use base_common_io::FsPathError;
    ...
    P: TransactionPool,
{
    ...
    let data = base_common_io::Files::read(file_path)?;
    ...
    base_common_io::Files::remove_file(file_path)?;
    Ok(())
}
    ...
    P: TransactionPool,
{
    ...
    match parent_dir.map(|_| base_common_io::Files::write(file_path, json_data)) {
        Ok(_) => {
...
pub async fn backup_local_transactions_task<P>(
    shutdown: base_common_runtime::shutdown::GracefulShutdown,
    pool: P,
...
mod tests {
    ...
    async fn test_save_local_txs_backup() {
        ...
        let tx = base_common_types_chain::BasePooledTransaction::decode_2718(&mut &tx_bytes[..])
            .unwrap();
