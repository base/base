...
mod tests {
    ...
    use crate::{TransactionValidity, ValidatedTransaction};

    /// One insert request, tagged by `nonce` so ordering assertions can name it.
    fn transaction(nonce: u64) -> InsertValidatedTransaction {
    ...
    fn tagged(nonce: u64) -> InsertValidatedTransaction {
        let mut request = transaction(nonce);
        request.transaction.extensions.validity = vec![crate::ValidityPredicate::BlockNumber {
            op: crate::ValidityOperator::Equal,
            value: alloy_primitives::U256::from(nonce),
        }];
        request
    }
    ...
    async fn queue_closure_drains_buffered_transactions_and_writes_extensions() {
        ...
        assert!(received.iter().all(|tx| tx["validity"][0]["type"] == "block_number"));
    ...
    async fn flush_buffer_chunks_transactions_at_max_batch_size() {
        ...
        let mut forwarder = forwarder::<InsertValidatedTransaction>(url, receiver, config(1, 2));
        for nonce in 0..5 {
    ...
    async fn run_forwards_an_isolated_request_without_waiting() {
        ...
        let forwarder = forwarder::<InsertValidatedTransaction>(url, receiver, config(0, 100));
    ...
    async fn a_batch_preserves_submission_order_across_request_kinds() {
        ...
        let insert = |nonce| TestRequest::Insert(Box::new(transaction(nonce).transaction));
