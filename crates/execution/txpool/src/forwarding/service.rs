...
impl TxForwardingService {
    ...
    {
        self.spawn_inner(pool, executor, false)
    }

    /// Starts forwarding with an extension payload on the transaction wire format.
    pub fn spawn_with_extensions<P>(self, pool: P, executor: &TaskExecutor) -> TxForwardingHandle
    where
        P: TransactionPool + Clone + Send + 'static,
    {
        self.spawn_inner(pool, executor, true)
    }
        ...
        P: TransactionPool + Clone + Send + 'static,
    {
        ...
        for builder_url in &self.config.builder_urls {
            ...
            // `TransactionValidity` is named explicitly: nothing else in this expression pins the queue's item type,
            // since both ends are generic over it.
            let mut reader = DestinationReader::<P>::new(
                pool.clone(),
                reader_config.clone(),
                sender,
            ...
            reader.include_validity = include_validity;
...
mod tests {
    ...
    use super::*;
    use crate::{ValidatedTransaction, forwarding::InsertValidatedTransaction};
