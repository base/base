...
use crate::{
    TransactionPool, TransactionValidity, ValidPoolTransaction, ValidatedTransaction,
    forwarding::forwarder::InsertValidatedTransaction,
};
...
/// async runtime.
pub(crate) struct DestinationReader<P: TransactionPool> {
    pool: P,
    pub include_validity: bool,
    config: ReaderConfig,
    ...
    P: TransactionPool + 'static,
{
    ...
    fn try_enqueue(
        ...
        let mut request = Self::to_wire(transaction);
        if !self.include_validity {
            request.transaction.extensions = TransactionValidity::default();
        }
        permit.send(request);
        Ok(())
    }
    ...
    fn to_wire(transaction: &Arc<ValidPoolTransaction>) -> InsertValidatedTransaction {
        ...
        InsertValidatedTransaction {
            transaction: ValidatedTransaction {
                ...
                extensions: TransactionValidity::extract(transaction),
            },
...
impl<P: TransactionPool> fmt::Debug for DestinationReader<P> {
