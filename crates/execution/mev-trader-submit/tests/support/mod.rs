...
use base_mev_trader::{BackrunHop, BackrunPlan, BackrunPlanDigest, ExactProtocol};
...
/// Build a two-hop WETH closed-loop [`BackrunPlan`] with the given hop protocols.
pub fn backrun_plan(protocols: [ExactProtocol; 2], victim: B256) -> BackrunPlan {
    BackrunPlan {
        ...
        victim,
        route: [
            BackrunHop { pool: POOL1, protocol: protocols[0], token_in: WETH, token_out: TOKEN },
            BackrunHop { pool: POOL2, protocol: protocols[1], token_in: TOKEN, token_out: WETH },
        ],
