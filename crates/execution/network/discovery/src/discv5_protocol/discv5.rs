...
//!
//! A [`Discv5Protocol`] struct needs to be created either with an [`crate::discv5_protocol::executor::Executor`] specified
//! in the [`Config`] via the [`crate::discv5_protocol::ConfigBuilder`] or in the presence of a tokio runtime that
//! has timing and io enabled.
//!
//! Once a [`Discv5Protocol`] struct has been created the service is started by running the [`Discv5Protocol::start`]
//! functions with a UDP socket. This will start a discv5 server in the background listening on the
//! specified UDP socket.
//!
//! The server can be shutdown using the [`Discv5Protocol::shutdown`] function.
...
lazy_static! {
    pub static ref PERMIT_BAN_LIST: RwLock<crate::discv5_protocol::PermitBanList> =
        RwLock::new(crate::discv5_protocol::PermitBanList::default());
}

pub(crate) mod test;

/// Events that can be produced by the `Discv5Protocol` event stream.
#[derive(Debug)]
#[non_exhaustive]
pub enum Event {
    ...
    NodeInserted { node_id: NodeId, replaced: Option<NodeId> },
    /// An ENR doesn't verify against the observed socket and node ID of the peer.
    UnverifiableEnr { enr: Enr, socket: SocketAddr, node_id: NodeId },
...
/// The main Discv5Protocol Service struct. This provides the user-level API for performing queries and
/// interacting with the underlying service.
pub struct Discv5Protocol {
    config: Config,
...
impl core::fmt::Debug for Discv5Protocol {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("Discv5Protocol")
...
impl Discv5Protocol {
    pub fn new(
        ...
        if config.executor.is_none() {
            config.executor =
                Some(Box::<crate::discv5_protocol::executor::TokioExecutor>::default());
        };
    ...
    pub fn shutdown(&mut self) {
        if let Some(exit) = self.service_exit.take() {
            if exit.send(()).is_err() {
                debug!("Discv5Protocol service already shutdown");
            }
    ...
    pub fn disconnect_node(&self, node_id: &NodeId) -> bool {
        let key = &kbucket::Key::from(*node_id);
        !matches!(
            self.kbuckets.write().update_node_status(key, ConnectionState::Disconnected, None),
    ...
    pub fn connected_peers(&self) -> usize {
        self.kbuckets.write().iter().filter(|entry| entry.status.is_connected()).count()
    }
    ...
    /// Identical to `Discv5Protocol::local_enr` except that this exposes the `Arc` itself.
    ///
    /// This is useful for synchronising views of the local ENR outside of `Discv5Protocol`.
    ...
    pub fn send_ping(
        ...
        async move {
            ...
            // await the response
            callback_recv.await.map_err(|e| RequestError::ChannelFailed(e.to_string()))?
        }
    }
    ...
    pub fn table_entries_id(&self) -> Vec<NodeId> {
        self.kbuckets.write().iter().map(|entry| *entry.node.key.preimage()).collect()
    }

    /// Returns an iterator over all the ENR's of nodes currently contained in the routing table.
    pub fn table_entries_enr(&self) -> Vec<Enr> {
        self.kbuckets.write().iter().map(|entry| entry.node.value.clone()).collect()
    }
    ...
    pub fn table_entries(&self) -> Vec<(NodeId, Enr, NodeStatus)> {
        self.kbuckets
            .write()
            .iter()
            .map(|entry| (*entry.node.key.preimage(), entry.node.value.clone(), entry.status))
            .collect()
    }
    ...
    /// ```
    /// use std::str::FromStr;
    /// use crate::discv5_protocol::{ConfigBuilder, Discv5Protocol, ListenConfig, Enr, enr::CombinedKey};
    ///
    ...
    /// let discv5: Discv5Protocol = Discv5Protocol::new(enr, sk, config).unwrap();
    ///
    ...
    pub fn request_enr(
        ...
        async move {
            ...
            channel
                ...
                .map_err(|_| RequestError::ChannelFailed("Service channel closed".into()))?;
            // await the response
            match callback_recv.await.map_err(|e| RequestError::ChannelFailed(e.to_string()))? {
                Ok(mut nodes) => {
                    // This must be for asking for an ENR
                    if nodes.len() > 1 {
                        warn!("Peer returned more than one ENR for itself. {}", node_contact);
                    }
                    nodes.pop().ok_or(RequestError::InvalidEnr("Peer did not return an ENR"))
                }
                Err(err) => Err(err),
            }
        }
    ...
    pub fn find_node(
        ...
        async move {
            ...
            callback_recv.await.map_err(|e| QueryError::ChannelFailed(e.to_string()))
        }
    }
    ...
    pub fn find_node_predicate(
        ...
        async move {
            ...
            let query_kind = QueryKind::Predicate { target_node, predicate, target_peer_no };
    ...
    /// Creates an event stream channel which can be polled to receive Discv5Protocol events.
    pub fn event_stream(
        &self,
        ...
        async move {
            ...
            channel.send(event).await.map_err(|_| Error::ServiceChannelClosed)?;
