...
/// Reads addresses assigned to local network interfaces.
#[derive(Debug)]
pub struct NetworkInterface;

impl NetworkInterface {
    /// Reads IP of OS interface with given name, if exists.
    pub fn resolve_ip(if_name: &str) -> Result<IpAddr, NetInterfaceError> {
        match if_addrs_0_14_0::get_if_addrs() {
            Ok(ifs) => {
...
mod tests {
    ...
    fn read_docker_if_addr() {
        ...
        let ip = NetworkInterface::resolve_ip(LOCALHOST_IF[0])
            .unwrap_or_else(|_| NetworkInterface::resolve_ip(LOCALHOST_IF[1]).unwrap());
