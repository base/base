...
use base_execution_eip8130_authorize::{AuthorizeError, ResolvedActor};
use base_execution_eip8130_state::AccountConfigurationStorage;
use base_execution_eip8130_tx::{ActorTxVerifier, ConfigChangeAuthorizer, TxActors, TxAuthError};
...
impl TransactionAuthorizer {
    ...
    pub fn authorize(
        ...
        //    used rather than re-reading the (still-unwritten) state.
        ...
        for change in &signed.tx().account_changes {
            ...
            let (base, applied) = if cc.chain_id == 0 {
                (multichain_base, &mut multichain_applied)
            } else {
                (local_base, &mut local_applied)
            };
...
mod tests {
    ...
    use base_execution_eip8130_tx::{ConfigChangeAuthorizer, Operation};
