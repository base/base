...
| `account_changes` | per config-change entry: its `auth` cost plus each mutated actor slot (`actor_config`, plus `policy_commitment`/`policy_manager` when the authorize carries a policy); per delegation entry: the `4,600` indicator deposit |
