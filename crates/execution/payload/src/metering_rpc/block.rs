...
use base_execution_evm_blocks::{BaseEvmConfig, BaseNextBlockEnvAttributes};
use base_execution_evm_runtime::State;
use base_execution_state_provider::{HeaderProvider, StateProviderFactory};
use eyre::{Result as EyreResult, eyre};

use crate::metering_rpc::types::{MeterBlockResponse, MeterBlockTransactions};
    ...
    P: StateProviderFactory + HeaderProvider,
{
    ...
    let recovered_transactions: Vec<_> = transactions
        .map(|tx| {
            ...
            Ok(base_common_types_chain::transaction::Recovered::new_unchecked(tx.clone(), signer))
        })
...
mod tests {
    ...
    async fn meter_block_invalid_transaction_signature() -> eyre::Result<()> {
        ...
        let signed_tx =
            base_common_types_chain::Signed::new_unchecked(tx, invalid_signature, B256::random());
