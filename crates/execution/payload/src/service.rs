...
type ResolvePayloadResult = (Option<PayloadFuture>, Option<PayloadJobEntry>);
...
impl PayloadStore {
    ...
    /// job, See [`BasicPayloadJob::resolve`].
    pub fn resolve_kind(
        &self,
        id: PayloadId,
        kind: PayloadKind,
...
pub struct PayloadBuilderService<St> {
    ...
    payload_jobs: Vec<PayloadJobEntry>,
...
impl<St> PayloadBuilderService<St> {
    ...
    fn resolve(&mut self, id: PayloadId, kind: PayloadKind) -> ResolvePayloadResult {
...
#[derive(Debug)]
pub struct PayloadJobEntry {
    /// The running payload job.
    pub job: BasicPayloadJob,
    id: PayloadId,
    span: Span,
    leases: Vec<PayloadBuilderLease>,
}
            ...
            async {
                ...
                ProviderTestUtils::insert_blocks(
                    ...
                    &[base_common_types_chain::RecoveredBlock::new(
                        BaseBlock { header: parent.clone(), body: Default::default() },
                        Vec::new(),
                        parent.hash_slow(),
                    )],
                );
