...
impl BasicPayloadJobGenerator {
    ...
    pub fn new_payload_job(
        ...
        id: PayloadId,
    ) -> Result<BasicPayloadJob, PayloadBuilderError> {
...
///
/// This type is a [`BasicPayloadJob`] and [`Future`] that terminates when the deadline is reached or
/// when the job is resolved: [`BasicPayloadJob::resolve`].
///
...
/// built and this future will wait to be resolved: [`BasicPayloadJob::resolve`] or terminated if the
/// deadline is reached.
#[derive(Debug)]
...
impl BasicPayloadJob {
    /// Returns the best payload built so far, or builds an empty payload.
    pub fn best_payload(&self) -> Result<BaseBuiltPayload, PayloadBuilderError> {
    ...
    /// Returns the attributes of this payload job.
    pub fn payload_attributes(&self) -> Result<BasePayloadBuilderAttributes, PayloadBuilderError> {
        Ok(self.config.attributes.clone())
    }

    /// Returns the timestamp of this payload job.
    pub fn payload_timestamp(&self) -> Result<u64, PayloadBuilderError> {
        Ok(self.config.attributes.timestamp())
    }

    /// Resolves the requested payload and indicates whether the job should continue.
    pub fn resolve_kind(
        &mut self,
        _kind: PayloadKind,
    ) -> (ResolveBestPayload, KeepPayloadJobAlive) {
        let best_payload = self.best_payload.payload().cloned();
