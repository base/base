...
pub(super) fn test_root_empty_trie(new_trie: fn() -> ArenaParallelSparseTrie) {
    let mut trie = (new_trie)();
    assert_eq!(trie.root(epoch(0)), EMPTY_ROOT_HASH, "empty trie should return EMPTY_ROOT_HASH");
}
...
pub(super) fn test_root_after_single_leaf_update(new_trie: fn() -> ArenaParallelSparseTrie) {
    let mut key_a = B256::ZERO;
    key_a.0[0] = 0x10;
    let mut key_b = B256::ZERO;
    key_b.0[0] = 0x20;
    ...
    let original_root = trie.root(epoch(0));
    ...
    let new_root = trie.root(epoch(0));
