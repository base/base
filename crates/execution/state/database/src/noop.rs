...
use base_execution_state_types::{
    AccountBeforeTx, AccountProof, AccountReader, BalProvider, BalStoreHandle,
    BlockBodyIndicesProvider, BlockHashReader, BlockIdReader, BlockNumReader, BlockReader,
    BlockReaderIdExt, BlockSource, BytecodeReader, ChangeSetReader, ExecutionOutcome,
    ExecutionWitnessMode, HashedPostState, HashedPostStateProvider, HashedStorage, HeaderProvider,
    MultiProof, MultiProofTargets, ProviderError, ProviderResult, PruneCheckpoint,
    PruneCheckpointReader, PruneModes, PruneSegment, ReceiptProvider, ReceiptProviderIdExt,
    StageCheckpoint, StageCheckpointReader, StageId, StateProofProvider, StateProviderBox,
    StateProviderFactory, StateRangeProviderFactory, StateRangeView, StateReader,
    StateRootProvider, StorageChangeSetReader, StorageMultiProof, StorageProof,
    StorageRootProvider, StorageSettingsCache, StoredBlockBodyIndices, TransactionVariant,
    TransactionsProvider, TrieInput, TryIntoHistoricalStateProvider, updates::TrieUpdates,
};

use crate::{DBProvider, DatabaseProviderFactory, DatabaseProviderROFactory, DbTxProvider, TxMock};
...
impl StorageChangeSetReader for NoopProvider {
    fn storage_changeset(
        ...
        Vec<(crate::models::BlockNumberAddress, base_execution_state_types::StorageEntry)>,
    > {
        Ok(Vec::default())
    }
...
base_execution_state_types::impl_state_database!([] NoopProvider where []);

impl base_execution_state_types::StateReadProvider for NoopProvider {
    fn storage(
        &self,
        _account: Address,
...
impl StorageSettingsCache for NoopProvider {
    fn cached_storage_settings(&self) -> crate::models::StorageSettings {
        crate::models::StorageSettings::default()
    }

    fn set_storage_settings_cache(&self, _settings: crate::models::StorageSettings) {}
}

#[cfg(test)]
mod tests {
    use alloy_primitives::B256;
    use base_execution_state_types::{BalProvider, GetBlockAccessListLimit};

    use super::NoopProvider;
