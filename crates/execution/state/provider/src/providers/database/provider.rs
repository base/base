...
pub type DatabaseProviderRO = DatabaseProvider<
    base_execution_state_database::mdbx::tx::Tx<base_execution_state_database::mdbx::RO>,
>;
...
impl<TX: DbTx + 'static> DatabaseProvider<TX> {
    ...
    ///
    /// Historical state reads ignore `RocksDB` history entries above their MDBX-visible tip,
...
impl<TX> RocksDBProviderFactory for DatabaseProvider<TX> {
...
impl<TX: Debug + Send> ChainSpecProvider for DatabaseProvider<TX> {
...
impl<TX: DbTx + DbTxMut + 'static> DatabaseProvider<TX> {
    ...
    fn static_file_write_ctx(
        ...
        Ok(StaticFileWriteCtx {
            write_senders: self.prune_modes.sender_recovery.is_none_or(|m| !m.is_full()),
    ...
    fn save_blocks_inner(
        ...
        let rocksdb_enabled = rocksdb_ctx.is_some();
        ...
        if rocksdb_enabled {
            timings.rocksdb = rocksdb_result.ok_or_else(|| {
                ProviderError::Database(base_execution_state_database::DatabaseError::Other(
    ...
    /// Writes MDBX-only indices for a block.
    ///
    /// Static file data (headers, transactions, senders, and unfiltered receipts) is written separately.
...
impl<TX: DbTx + 'static> DatabaseProvider<TX> {
    ...
    /// Populate a [`BundleStateInit`] and [`RevertsInit`] from changesets and current state.
...
impl<TX: DbTxMut + DbTx + 'static> DatabaseProvider<TX> {}
...
impl<TX: DbTx + 'static> AccountExtReader for DatabaseProvider<TX> {
...
impl<TX: DbTx> StorageChangeSetReader for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> BlockHashReader for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> BlockNumReader for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> TransactionsProviderExt for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> TransactionsProvider for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> BlockBodyIndicesProvider for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> StorageReader for DatabaseProvider<TX> {
...
impl<TX: DbTxMut + DbTx + 'static> StateWriter for DatabaseProvider<TX> {
    ...
    fn write_state_reverts(
        ...
        if config.write_storage_changesets {
            ...
            for (block_index, mut storage_changes) in reverts.storage.into_iter().enumerate() {
                ...
                for PlainStorageRevert { address, storage_revert, .. } in storage_changes {
                    ...
                    // Wipe reverts already contain the plain slot keys supplied by execution.
    ...
    fn remove_state_above(&self, block: BlockNumber) -> ProviderResult<()> {
        ...
        let _storage_range = BlockNumberAddress::range(range.clone());
...
impl<TX: DbTxMut + DbTx + 'static> TrieWriter for DatabaseProvider<TX> {
...
impl<TX: DbTxMut + DbTx + 'static> StorageTrieWriter for DatabaseProvider<TX> {
...
impl<TX: DbTxMut + DbTx + 'static> HashingWriter for DatabaseProvider<TX> {
...
impl<TX: DbTxMut + DbTx + 'static> HistoryWriter for DatabaseProvider<TX> {
    ...
    fn insert_account_history_index(
        ...
        for (key, indices) in account_transitions {
            grouped.entry(key).or_default().extend(indices);
        }
        let rocksdb = self.rocksdb_provider();
        let prepared =
            prepare_history_shard_writes_parallel::<tables::AccountsHistory, _>(grouped, |key| {
                rocksdb.get::<tables::AccountsHistory>(key)
            })?;
    ...
    fn unwind_storage_history_indices(
        ...
        let batch = self.rocksdb_provider.unwind_storage_history_indices(&storage_changesets)?;
        self.pending_rocksdb_batches.lock().push(batch);
    ...
    fn insert_storage_history_index(
        ...
        let mut grouped = BTreeMap::<_, Vec<_>>::new();
        for (key, indices) in storage_transitions {
            grouped.entry(key).or_default().extend(indices);
        }
        let rocksdb = self.rocksdb_provider();
        let prepared =
            prepare_history_shard_writes_parallel::<tables::StoragesHistory, _>(grouped, |key| {
                rocksdb.get::<tables::StoragesHistory>(key)
            })?;
        self.with_rocksdb_batch(|mut batch| {
            for (key, shard) in prepared.into_writes() {
...
impl<TX: DbTx + 'static> PruneCheckpointReader for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> StatsReader for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> ChainStateBlockReader for DatabaseProvider<TX> {
...
impl<TX: DbTxMut> ChainStateBlockWriter for DatabaseProvider<TX> {
...
impl<TX: DbTx + 'static> DbTxProvider for DatabaseProvider<TX> {
    type Tx = TX;
...
impl<TX: DbTx> MetadataProvider for DatabaseProvider<TX> {
...
impl<TX: Send> StorageSettingsCache for DatabaseProvider<TX> {
...
impl<TX: Send> StoragePath for DatabaseProvider<TX> {
...
mod tests {
    ...
    fn test_write_trie_updates_sorted() {
        ...
        // Pre-populate account trie with data that will be deleted
        {
            ...
            let to_delete = PackedStoredNibbles(Nibbles::from_nibbles([0x3, 0x4]));
            cursor
                .upsert(
                    to_delete,
            ...
            let to_update = PackedStoredNibbles(Nibbles::from_nibbles([0x1, 0x2]));
            cursor
                .upsert(
                    to_update,
                    &BranchNodeCompact::new(
        ...
        {
            let tx = provider_rw.tx_ref();
            let mut storage_cursor = tx.cursor_dup_write::<tables::PackedStoragesTrie>().unwrap();
            ...
            storage_cursor
                .upsert(
                    ...
                    &PackedStorageTrieEntry {
                        nibbles: PackedStoredNibblesSubKey(Nibbles::from_nibbles([0x2, 0x0])),
            ...
            storage_cursor
                .upsert(
                    storage_address2,
                    &PackedStorageTrieEntry {
                        nibbles: PackedStoredNibblesSubKey(Nibbles::from_nibbles([0xa, 0xb])),
                        node: BranchNodeCompact::new(
            ...
            storage_cursor
                .upsert(
                    storage_address2,
                    &PackedStorageTrieEntry {
                        nibbles: PackedStoredNibblesSubKey(Nibbles::from_nibbles([0xc, 0xd])),
                        node: BranchNodeCompact::new(
        ...
        let tx = provider_rw.tx_ref();
        let mut cursor = tx.cursor_read::<tables::PackedAccountsTrie>().unwrap();
        ...
        let nibbles2 = PackedStoredNibbles(Nibbles::from_nibbles([0x3, 0x4]));
        ...
        let nibbles3 = PackedStoredNibbles(Nibbles::from_nibbles([0x5, 0x6]));
        ...
        let mut storage_cursor = tx.cursor_dup_read::<tables::PackedStoragesTrie>().unwrap();
