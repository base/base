...
impl TestBlockBuilder {
    ...
    /// Chainspec setter.
    pub fn with_chain_spec(mut self, chain_spec: BaseChainSpec) -> Self {
    ...
    pub fn single_tx_cost() -> U256 {
        U256::from(INITIAL_BASE_FEE * 21_000u64)
    }
    ...
    pub fn generate_random_block(&mut self, number: BlockNumber, parent_hash: B256) -> SealedBlock {
        ...
        let receipts = transactions
            ...
            .enumerate()
            .map(|(idx, _tx)| {
                Receipt::Eip1559(base_common_types_chain::Receipt::<alloy_primitives::Log> {
                    status: true.into(),
        ...
        let header = Header {
            number,
            parent_hash,
            gas_used: transactions.len() as u64 * 21_000u64,
            ...
            timestamp: number
                + self.chain_spec.fork(EthereumHardfork::Cancun).as_timestamp().unwrap(),
