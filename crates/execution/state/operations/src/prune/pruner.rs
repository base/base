...
use crate::{
    ...
    prune::{
        error::{BaseProofStoragePrunerResult, ProofStoragePrunerError, ProofStoragePrunerOutput},
        metrics::Metrics,
    },
};
    ...
    H: BlockHashReader,
{
    fn run_inner(&self) -> BaseProofStoragePrunerResult {
        let Some(((earliest_block, _), (latest_block, _))) = self
            ...
            return Ok(ProofStoragePrunerOutput::default());
        };
        ...
        let mut prune_output = ProofStoragePrunerOutput {
    ...
    fn prune_batch(
        ...
        start_block: u64,
        end_block: u64,
    ) -> Result<ProofStoragePrunerOutput, ProofStoragePrunerError> {
        ...
        if end_block == 0 {
            ...
            return Ok(ProofStoragePrunerOutput {
        ...
        let new_earliest_block_hash = self
            ...
            .ok_or(ProofStoragePrunerError::BlockNotFound(end_block))?;
        ...
        let batch_output =
            ProofStoragePrunerOutput { duration, start_block, end_block, write_counts };
        ...
        ($mod_name:ident, $storage:ident) => {
            mod $mod_name {
                ...
                async fn run_inner_where_latest_block_is_none() {
                    ...
                    assert_eq!(
                        out,
                        ProofStoragePrunerOutput::default(),
                        "should early-return default output"
                    );
                }
                ...
                async fn run_inner_interval_too_small_real_db() {
                    ...
                    let out = pruner.run_inner().expect("ok");
                    assert_eq!(out, ProofStoragePrunerOutput::default(), "no pruning should occur");
                }
            }
        };
    }
