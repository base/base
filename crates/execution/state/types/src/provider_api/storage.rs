...
use crate::{ProviderResult, StorageEntry};
...
pub trait StorageChangeSetReader: Send {
    ...
    ) -> ProviderResult<Vec<(crate::BlockNumberAddress, StorageEntry)>>;
    ...
    fn storage_block_changeset(
        &self,
        block_number: BlockNumber,
    ) -> ProviderResult<Vec<crate::StorageBeforeTx>> {
        self.storage_changeset(block_number).map(|changesets| {
            changesets
                .into_iter()
                .map(|(block_address, entry)| crate::StorageBeforeTx {
