...
macro_rules! base_precompile {
    ($id:expr, storage_features: $storage_features:expr, |$ctx:ident, $calldata:ident| $impl:expr $(,)?) => {{
        ::base_execution_evm_precompiles::DynPrecompile::new_stateful(
            ::revm_precompile::PrecompileId::Custom($id.into()),
            move |input| {
                if !input.is_direct_call() {
                    return ::base_execution_evm_precompiles::IntoEnginePrecompileResult::into_revm(
                        ::base_execution_evm_precompiles::BasePrecompileError::revert(
                            ::base_execution_evm_precompiles::DelegateCallNotAllowed {},
                        )
                ...
                let mut provider = ::base_execution_evm_precompiles::EvmPrecompileStorageProvider::new_with_storage_features(
                    input,
                    ::base_evm_context::GasParams::default(),
                    $storage_features,
                );

                ::base_execution_evm_precompiles::IntoEnginePrecompileResult::into_revm(
                    ::base_execution_evm_precompiles::StorageCtx::enter(&mut provider, |$ctx| $impl),
                )
            },
        )
    }};
...
macro_rules! decode_precompile_call {
    ($calldata:expr, $call_ty:ty $(,)?) => {{
        ...
        let selector = match calldata.get(..4) {
            ...
            None => {
                return Err(
                    ::base_execution_evm_precompiles::BasePrecompileError::UnknownFunctionSelector(
                        [0u8; 4],
                    ),
                );
            }
        ...
        match <$call_ty as ::alloy_sol_types::SolInterface>::abi_decode_validate(calldata) {
            ...
            {
                return Err(::base_execution_evm_precompiles::BasePrecompileError::AbiDecodeFailed {
                    selector,
...
macro_rules! reject_frozen_selector {
    () => {
        ::core::result::Result::Err(
            ::base_execution_evm_precompiles::BasePrecompileError::UnknownFunctionSelector([0u8; 4]),
        )
    };
}
...
mod tests {
    ...
    use base_execution_evm_precompiles::{BasePrecompileError, Result};
