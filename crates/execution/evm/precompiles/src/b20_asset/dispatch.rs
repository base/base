...
use base_execution_evm_precompiles::{BasePrecompileError, PrecompileResult, StorageCtx};
...
impl<S: AssetAccounting, A: PolicyAccounting> B20AssetToken<S, A> {
    ...
    pub fn route<O>(
        ...
        privileged: bool,
        observer: O,
    ) -> base_execution_evm_precompiles::Result<Bytes>
    where
        O: PrecompileCallObserver,
    {
...
mod tests {
    ...
    use base_common_chain_config::BaseUpgrade;
    use base_execution_evm_precompiles::{HashMapStorageProvider, Result, StorageCtx};
    ...
    fn announce_inner_system_error_propagates_unchanged() {
        ...
        assert_eq!(err, base_execution_evm_precompiles::BasePrecompileError::under_overflow());
    }
    ...
    fn announce_inner_ordinary_revert_wraps_as_internal_call_failed() {
        ...
        assert_eq!(
            err,
            base_execution_evm_precompiles::BasePrecompileError::revert(IB20Asset::InternalCallFailed {
                call: inner_call
            })
        );
    }
    ...
    fn route_v1_rejects_scheduled_selector_as_unknown() {
        ...
        assert_eq!(
            err,
            base_execution_evm_precompiles::BasePrecompileError::UnknownFunctionSelector(selector)
        );
    }
