...
pub(crate) fn p1_add_affine_bytes(
    a: G1Point,
    b: G1Point,
) -> Result<[u8; G1_LENGTH], crate::core_crypto::PrecompileHalt> {
    let (a_x, a_y) = a;
    let (b_x, b_y) = b;
    // Parse first point
...
pub(crate) fn p2_add_affine_bytes(
    a: G2Point,
    b: G2Point,
) -> Result<[u8; G2_LENGTH], crate::core_crypto::PrecompileHalt> {
...
pub(crate) fn p1_msm_bytes(
    point_scalar_pairs: impl Iterator<Item = Result<G1PointScalar, crate::core_crypto::PrecompileHalt>>,
) -> Result<[u8; G1_LENGTH], crate::core_crypto::PrecompileHalt> {
...
pub(crate) fn p2_msm_bytes(
    point_scalar_pairs: impl Iterator<Item = Result<G2PointScalar, crate::core_crypto::PrecompileHalt>>,
) -> Result<[u8; G2_LENGTH], crate::core_crypto::PrecompileHalt> {
    let (lower, _) = point_scalar_pairs.size_hint();
...
pub(crate) fn pairing_check_bytes(
    pairs: &[PairingPair],
) -> Result<bool, crate::core_crypto::PrecompileHalt> {
