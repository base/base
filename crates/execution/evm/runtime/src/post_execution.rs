...
pub fn build_result_gas(
    ...
    // phase).
    let state_gas =
        gas.state_gas_spent().saturating_add_unsigned(init_and_floor_gas.initial_state_gas).max(0)
            as u64;

    ResultGas::default()
        .with_total_gas_spent(
            gas.limit().saturating_sub(gas.remaining()).saturating_sub(gas.reservoir()),
        )
...
pub fn reimburse_caller<CTX: ContextTr>(
    ...
    context.journal_mut().load_account_mut(caller)?.incr_balance(
        U256::from(effective_gas_price.saturating_mul(reimbursable as u128)) + additional_refund,
    );
