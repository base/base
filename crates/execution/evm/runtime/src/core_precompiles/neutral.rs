...
use crate::{
    AccountInfo as RevmAccountInfo, CryptoPrecompileError as RevmPrecompileError,
    CryptoPrecompileHalt as RevmPrecompileHalt, CryptoPrecompileOutput as RevmPrecompileOutput,
    CryptoPrecompileStatus as RevmPrecompileStatus, KECCAK_EMPTY,
};

/// Engine-neutral account information.
///
/// Mirrors the fields the enshrined precompiles read; the closure callbacks in
/// [`crate::core_precompiles::StorageCtx`] only observe `balance`, `nonce`, `code_hash`, and
...
pub trait IntoEnginePrecompileResult {
    ...
    fn into_revm(self) -> crate::CryptoPrecompileResult;
}
