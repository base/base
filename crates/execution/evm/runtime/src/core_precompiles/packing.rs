...
//!
//! Storage layout is described by the [`Layout`](crate::core_precompiles::storage_provider::Layout) enum:
...
impl Word {
    ...
    pub fn extract_from_word<T: FromWord + StorableType>(
        ...
        if offset + bytes > 32 {
            return Err(crate::core_precompiles::error::BasePrecompileError::Fatal(format!(
...
mod tests {
    ...
    fn test_packed_at_multiple_types() -> Result<()> {
        let (mut storage, address) = crate::core_precompiles::hashmap::setup_storage();
