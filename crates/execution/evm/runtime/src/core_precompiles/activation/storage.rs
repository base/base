...
use crate::core_precompiles::{
    BasePrecompileError, Handler, IActivationRegistry, Mapping, PrecompileResult, Result,
};
...
impl ActivationRegistryStorage<'_> {
    ...
    /// [`crate::core_precompiles::PrecompileOutput::is_revert`] to distinguish an activated feature from an
    /// ABI revert.
...
mod tests {
    ...
    use crate::core_precompiles::{
        ActivationAdminConfig, ActivationFeature, ActivationRegistryStorage, BasePrecompileError,
        HashMapStorageProvider, IActivationRegistry, PrecompileOutput, Result, StorageCtx,
        StorageKey, activation::storage::slots,
    };
