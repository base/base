...
use crate::core_precompiles::{BasePrecompileError, Result};

/// A frozen wire (ABI) surface of the shared B-20 token interface.
///
/// Reached only through a token version's wire join (e.g. [`crate::core_precompiles::AssetVersion::abi`]). There is
...
mod tests {
    ...
    use crate::core_precompiles::BasePrecompileError;
