...
use crate::core_precompiles::Result;
...
impl TransferPolicyIds {
    ...
    pub fn read_individually<T: TokenAccounting + ?Sized>(accounting: &T) -> Result<Self> {
        Ok(Self {
            sender: accounting
                .policy_id(crate::core_precompiles::B20PolicyType::TransferSender.id())?,
            receiver: accounting
                .policy_id(crate::core_precompiles::B20PolicyType::TransferReceiver.id())?,
            executor: accounting
                .policy_id(crate::core_precompiles::B20PolicyType::TransferExecutor.id())?,
        })
    }
}
