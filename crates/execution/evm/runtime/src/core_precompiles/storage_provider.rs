...
use crate::{
    ...
    core_precompiles::error::{BasePrecompileError, Result},
    gas::{KECCAK256, KECCAK256WORD},
};

/// Ensures bytecode emptiness matches whether the account's code hash is empty.
///
/// This guards against providers representing a missing code lookup as empty
/// bytecode. It does not recompute or verify the bytecode hash.
pub fn validate_loaded_code_presence(expected_hash: B256, code: &Bytecode) -> Result<()> {
    let expected_empty = expected_hash == B256::ZERO || expected_hash == KECCAK256_EMPTY;
...
pub trait PrecompileStorageProvider {
    ...
    /// Executes a closure with the full bytecode for the given address.
...
pub trait ContractStorage<'a> {
    ...
    fn storage(&self) -> crate::core_precompiles::storage_ctx::StorageCtx<'a>;
...
pub trait StorableType {
    ...
    fn handle<'a>(
        ...
        ctx: LayoutCtx,
        address: Address,
        storage: crate::core_precompiles::storage_ctx::StorageCtx<'a>,
    ) -> Self::Handler<'a>;
}
    ...
    fn delete<S: StorageOps>(storage: &mut S, slot: U256, ctx: LayoutCtx) -> Result<()> {
        match ctx.packed_offset() {
            ...
            Some(offset) => {
                ...
                let cleared = crate::core_precompiles::packing::Word::delete_from_word(
...
impl<T: Packable> Storable for T {
    ...
    fn load<S: StorageOps>(storage: &S, slot: U256, ctx: LayoutCtx) -> Result<Self> {
        ...
        match ctx.packed_offset() {
            ...
            Some(offset) => {
                ...
                crate::core_precompiles::packing::Word::extract_from_word(
                    slot_value,
                    offset,
                    Self::BYTES,
                )
    ...
    fn store<S: StorageOps>(&self, storage: &mut S, slot: U256, ctx: LayoutCtx) -> Result<()> {
        ...
        match ctx.packed_offset() {
            ...
            Some(offset) => {
                let current = storage.load(slot)?;
                let updated = crate::core_precompiles::packing::Word::insert_into_word(
                    current,
                    self,
                    offset,
                    Self::BYTES,
