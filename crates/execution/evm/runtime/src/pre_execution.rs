...
>(
    ...
    if precompiles_changed || empty_warmed_precompiles {
        ...
        context.journal_mut().warm_precompiles(precompiles.warm_addresses());
    }
...
mod tests {
    ...
    fn rejects_transactions_when_sender_nonce_is_max() {
        let caller_info = AccountInfo { nonce: u64::MAX, ..AccountInfo::default() };
    ...
    fn allows_matching_non_max_nonce() {
        let caller_info = AccountInfo { nonce: 7, ..AccountInfo::default() };
