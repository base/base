...
impl OpCode {
    ...
    pub const fn name_by_op(opcode: u8) -> &'static str {
        if let Some(opcode) = Self::new(opcode) { opcode.as_str() } else { "Unknown" }
    }
    ...
    pub const fn info(&self) -> OpCodeInfo {
        if let Some(info) = OPCODE_INFO[self.0 as usize] { info } else { OpCodeInfo::unknown() }
    }
    ...
    ///
    /// <https://docs.rs/base-execution-evm-machine/latest/base_execution_evm_runtime/instructions/index.html>
    ///
