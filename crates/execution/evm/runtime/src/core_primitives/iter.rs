use crate::core_primitives::{Bytecode, OpCode, opcode};
...
impl<'a> BytecodeIterator<'a> {
    ...
    pub fn new(bytecode: &'a Bytecode) -> Self {
        let bytes = if bytecode.is_legacy() { bytecode.original_byte_slice() } else { &[] };
        Self { bytes: bytes.iter(), start: bytes.as_ptr() }
    }
    ...
    pub fn position(&self) -> usize {
        ...
        unsafe { self.bytes.as_slice().as_ptr().offset_from_unsigned(self.start) }
    }
...
impl Iterator for BytecodeIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.bytes.next().copied().inspect(|&current| self.skip_immediate(current))
    }
...
mod tests {
    use super::*;
    use crate::core_primitives::Bytes;
    ...
    fn test_bytecode_with_various_push_sizes() {
        ...
        assert_eq!(opcodes, vec![opcode::PUSH1, opcode::PUSH2, opcode::PUSH3, opcode::STOP]);
    }
