...
impl<CTX> Inspector<CTX> for TestInspector {
    ...
    fn call(&mut self, _ctx: &mut CTX, inputs: &mut CallInputs) -> Option<CallOutcome> {
        self.call_depth += 1;
        self.events.push(InspectorEvent::Call { inputs: inputs.clone(), outcome: None });
        None
    }
    ...
    fn create(&mut self, _ctx: &mut CTX, inputs: &mut CreateInputs) -> Option<CreateOutcome> {
        self.events.push(InspectorEvent::Create { inputs: inputs.clone(), outcome: None });
        None
    }
    ...
    fn selfdestruct(&mut self, contract: Address, beneficiary: Address, value: U256) {
        self.events.push(InspectorEvent::Selfdestruct { address: contract, beneficiary, value });
    }
}
...
pub mod default_tests {
        ...
        F: FnMut(Bytes) -> Result<TestInspector, String>,
    {
        ...
        // Test LOG0
        let log_test =
            Bytes::from(vec![opcode::PUSH1, 0x20, opcode::PUSH1, 0x00, opcode::LOG0, opcode::STOP]);

        match execute(log_test) {
            Ok(inspector) => {
                let has_log = inspector.events.iter().any(|e| matches!(e, InspectorEvent::Log(_)));
                if !has_log {
...
mod tests {
    ...
    fn test_push_opcodes_and_stack_operations() {
        ...
        let step_events: Vec<_> = events
            .iter()
            .filter_map(|e| if let InspectorEvent::Step(record) = e { Some(record) } else { None })
            .collect();
    ...
    fn test_jump_and_jumpi_control_flow() {
        ...
        let jump_event = step_events.iter().find(|e| e.opcode_name == "JUMP").unwrap();
        ...
        let jumpi_event = step_events.iter().find(|e| e.opcode_name == "JUMPI").unwrap();
    ...
    fn test_create_opcodes() {
        ...
        assert!(!create_events.is_empty(), "Should have recorded CREATE events");
    ...
    fn test_log_operations() {
        ...
        let log_events: Vec<_> = events
            .iter()
            .filter_map(|e| if let InspectorEvent::Log(log) = e { Some(log) } else { None })
            .collect();
    ...
    fn test_eip7708_selfdestruct_transfer_log_is_inspected() {
        ...
        let result = evm
            .inspect_one_tx(TxEnv::builder_for_bench().gas_limit(100_000).gas_price(0).build_fill())
            .unwrap();
        assert!(result.is_success());
    ...
    fn test_selfdestruct() {
        ...
        let selfdestruct_events: Vec<_> = events
            .iter()
            .filter_map(|e| {
                if let InspectorEvent::Selfdestruct { address, beneficiary, value } = e {
        ...
        assert_eq!(selfdestruct_events.len(), 1, "Should have recorded SELFDESTRUCT event");
    ...
    fn test_comprehensive_inspector_integration() {
        ...
        let step_count = events.iter().filter(|e| matches!(e, InspectorEvent::Step(_))).count();
        let log_count = events.iter().filter(|e| matches!(e, InspectorEvent::Log(_))).count();
        ...
        let mstore_event = step_events.iter().find(|e| e.opcode_name == "MSTORE").unwrap();
        assert!(mstore_event.after.as_ref().unwrap().memory_size > 0);

        // Verify conditional jump worked correctly
        let jumpi_event = step_events.iter().find(|e| e.opcode_name == "JUMPI").unwrap();
        assert_eq!(jumpi_event.after.as_ref().unwrap().pc, 0x17, "Should have jumped to JUMPDEST");
    }
    ...
    fn test_inspect_tx_finalizes_journal_on_error() {
        ...
        assert!(result.is_err(), "tx with stale nonce must error so the error path is exercised");
