...
use crate::{
    Address, B256, Bytecode, EvmState, PRECOMPILE3, StorageKey, StorageValue, TransientStorage,
    U256,
};
...
impl JournalEntry {
    ...
    pub fn revert(
        ...
        match self {
            ...
            JournalEntry::StorageWarmed { address, key } => {
                state.get_mut(&address).unwrap().storage.get_mut(&key).unwrap().mark_cold();
            }
            JournalEntry::StorageChanged { address, key, had_value } => {
                state.get_mut(&address).unwrap().storage.get_mut(&key).unwrap().present_value =
                    had_value;
            }
            JournalEntry::TransientStorageChange { address, key, had_value } => {
