...
use crate::{
    Account, Address, AddressMap, B256, Bytecode, Database, EvmStorageSlot, HashSet, KECCAK_EMPTY,
    StorageKey, StorageValue, TransactionId, U256,
    core_machine::{
...
impl<'a, DB: Database> JournaledAccount<'a, DB> {
    ...
    pub const fn new(
        ...
        Self { address, account, journal_entries, access_list, transaction_id, db }
    }
        ...
        let (slot, is_cold) = match self.account.storage.entry(key) {
            ...
            Entry::Vacant(vac) => {
                ...
                let value = if is_newly_created {
                    ...
                    self.db.storage_by_account_id(self.address, account_id, key)?
                } else {
    ...
    pub fn sstore_concrete_error(
        ...
        if slot.present_value != new {
            ...
            // add journal entry.
            self.journal_entries.push(JournalEntry::storage_changed(
                self.address,
                key,
                previous_value,
            ));
...
impl<'a, DB: Database> JournaledAccountTr for JournaledAccount<'a, DB> {
    ...
    fn set_code(&mut self, code_hash: B256, code: Bytecode) {
        ...
        self.journal_entries.push(JournalEntry::code_changed(
            self.address,
            had_code_hash,
            had_code,
        ));
