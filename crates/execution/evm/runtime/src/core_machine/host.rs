...
use crate::{
    Address, B256, Bytecode, Bytes, Log, StorageKey, StorageValue, U256,
    core_machine::{
        StateLoad,
        cfg::GasParams,
...
pub trait Host {
    ...
    fn load_account_delegated(&mut self, address: Address) -> Option<StateLoad<AccountLoad>> {
        let account = self.load_account_info_skip_cold_load(address, true, false).ok()?;

        let mut account_load = StateLoad::new(
            AccountLoad { is_delegate_account_cold: None, is_empty: account.is_empty },
            account.is_cold,
        );
        ...
        if let Some(address) = account.code.as_ref().and_then(Bytecode::eip7702_address) {
            let delegate_account =
                self.load_account_info_skip_cold_load(address, true, false).ok()?;
    ...
    fn load_account_code(&mut self, address: Address) -> Option<StateLoad<Bytes>> {
        self.load_account_info_skip_cold_load(address, true, false).ok().map(|load| {
            load.into_state_load(|i| {
                i.code.as_ref().map(|b| b.original_bytes()).unwrap_or_default()
            })
        })
    }
    ...
    fn load_account_code_hash(&mut self, address: Address) -> Option<StateLoad<B256>> {
        self.load_account_info_skip_cold_load(address, false, false).ok().map(|load| {
            load.into_state_load(|i| if i.is_empty() { B256::ZERO } else { i.code_hash })
        })
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::{AccountInfo, Address, Bytecode, U256, hardfork::SpecId};
    ...
    impl Eip7702Host {
        fn new() -> Self {
            ...
            let delegated_info =
                AccountInfo::new(U256::from(1), 1, B256::ZERO, Bytecode::new_eip7702(delegate));
            Self { dummy: DummyHost::new(SpecId::PRAGUE), delegated, delegate, delegated_info }
        }
    }
    ...
    impl Host for Eip7702Host {
        ...
        fn sstore_skip_cold_load(
            ...
            self.dummy.sstore_skip_cold_load(address, key, value, skip_cold_load)
        }
        fn sload_skip_cold_load(
            &mut self,
            address: Address,
            ...
            self.dummy.sload_skip_cold_load(address, key, skip_cold_load)
        }
    ...
    fn load_account_delegated_keeps_caller_is_empty_not_delegate() {
        ...
        let load = host.load_account_delegated(host.delegated).expect("delegated account loads");

        assert!(load.data.is_delegate_account_cold.is_some(), "delegate account must be loaded");
        assert!(!load.data.is_empty, "is_empty must stay false for the non-empty EIP-7702 account");
    }
}
