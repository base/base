...
use crate::{
    Bytes, U256,
    core_machine::{
        CallInput, CallInputs, CallScheme, CallValue, CreateInputs, CreateScheme, Host,
        InstructionContext as Ictx, InstructionExecResult as Result, InstructionResult,
        InterpreterAction, instructions::utility::IntoAddress, interpreter_action::FrameInput,
    },
    hardfork::SpecId,
};
...
pub fn create<const IS_CREATE2: bool, H: Host + ?Sized>(context: Ictx<'_, H>) -> Result {
    ...
    if len != 0 {
        ...
        if context.interpreter.runtime_flag.spec_id.is_enabled_in(SpecId::SHANGHAI) {
            ...
            gas!(context.interpreter, context.host.gas_params().initcode_cost(len));
        }
    ...
    let scheme = if IS_CREATE2 {
        ...
        gas!(context.interpreter, context.host.gas_params().create2_cost(len));
    ...
    if context.host.is_amsterdam_eip8037_enabled() {
        ...
        let caller_info = context.host.load_account_info_skip_cold_load(caller, false, false)?;
        ...
        let destination_alive =
            !context.host.load_account_info_skip_cold_load(created_address, false, false)?.is_empty;
        if !destination_alive {
            state_gas!(context.interpreter, context.host.gas_params().create_state_gas());
...
pub fn call<const KIND: u8, H: Host + ?Sized>(mut context: Ictx<'_, H>) -> Result {
    use crate::opcode::{CALL, CALLCODE, DELEGATECALL, STATICCALL};
