...
use crate::{
    ...
    core_machine::{
        AccountInfoLoad, Gas, Host, InstructionContext as Ictx, InstructionExecResult as Result,
        InstructionResult, LoadError, SStoreResult, StateLoad,
...
pub fn extcodecopy<H: Host + ?Sized>(context: Ictx<'_, H>) -> Result {
    ...
    gas!(context.interpreter, context.host.gas_params().extcodecopy(len));
...
pub fn sload<H: Host + ?Sized>(context: Ictx<'_, H>) -> Result {
    ...
    if spec_id.is_enabled_in(BERLIN) {
        ...
        let storage = context.host.sload_skip_cold_load(target, *index, skip_cold)?;
        if storage.is_cold {
        ...
        *index = storage.data;
        ...
        let storage =
            context.host.sload(target, *index).ok_or(InstructionResult::FatalExternalError)?;
        *index = storage.data;
    };
    Ok(())
}
    ...
    ) -> Result,
{
    ...
    gas!(context.interpreter, context.host.gas_params().sstore_static_gas());
    ...
    H: Host + ?Sized,
{
    ...
    if context.host.is_amsterdam_eip8037_enabled() {
        ...
        let refill = context.host.gas_params().sstore_state_gas_refill(&state_load.data);
        if refill > 0 {
    ...
    context
        .interpreter
        .gas
        .record_refund(context.host.gas_params().sstore_refund(is_istanbul, &state_load.data));
    Ok(())
}
...
pub fn log<const N: usize, H: Host + ?Sized>(context: Ictx<'_, H>) -> Result {
    ...
    gas!(context.interpreter, context.host.gas_params().log_cost(N as u8, len as u64));
    let data = if len == 0 {
        Bytes::new()
    } else {
...
pub fn selfdestruct<H: Host + ?Sized>(context: Ictx<'_, H>) -> Result {
    ...
    gas!(
        context.interpreter,
        context.host.gas_params().selfdestruct_cost(should_charge_topup, res.is_cold)
    );

    // State gas for new account creation (EIP-8037)
    if context.host.is_amsterdam_eip8037_enabled() && should_charge_topup {
        state_gas!(context.interpreter, context.host.gas_params().new_account_state_gas());
    }
