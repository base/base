...
use crate::{
    Address, B256, Bytecode, U256,
    core_machine::{
        GasParams, Host, InstructionContext as Ictx, InstructionResult, LoadError,
...
pub fn load_acc_and_calc_gas<H: Host + ?Sized>(
    ...
    // Transfer value cost
    if transfers_value {
        gas!(context.interpreter, context.host.gas_params().transfer_value_cost());
    }
    ...
    Ok((gas_limit, bytecode, code_hash, charged_new_account_state_gas))
}
