...
use crate::{
    U256,
    core_machine::{InstructionContext as Ictx, InstructionExecResult as Result},
};
...
mod tests {
    ...
    fn test_shift_left() {
        ...
        for test in test_cases {
            ...
            let context = Ictx { host: &mut DummyHost::default(), interpreter: &mut interpreter };
            let _ = shl(context);
    ...
    fn test_clz() {
        ...
        for test in test_cases {
            assert!(interpreter.stack.push(test.value));
            let context = Ictx { host: &mut host, interpreter: &mut interpreter };
            let _ = clz(context);
            let res = interpreter.stack.pop().unwrap();
            assert_eq!(
                res, test.expected,
