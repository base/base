//! Module containing the [`JournalInner`] that is part of [`crate::core_machine::Journal`].
use core::mem;
use std::vec::Vec;

use super::warm_addresses::WarmAddresses;
use crate::{
    Account, Address, B256, Bytecode, Bytes, Database, EvmState, HashMap, KECCAK_EMPTY, Log,
    LogData, StorageKey, StorageValue, TransactionId, TransientStorage, U256,
    core_machine::{
        AccountLoad, JournalCheckpoint, JournalEntry, JournalLoadError, JournaledAccount,
        JournaledAccountTr, SStoreResult, SelfDestructResult, SelfdestructionRevertStatus,
...
impl JournalInner {
    ...
    pub fn checkpoint_revert(&mut self, checkpoint: JournalCheckpoint) {
        ...
        self.selfdestructed_addresses.truncate(checkpoint.selfdestructed_i);
    ...
    pub fn load_account_delegated<DB: Database>(
        ...
        if let Some(address) = account.info.code.as_ref().and_then(Bytecode::eip7702_address) {
    ...
    pub fn sstore<DB: Database>(
        ...
        self.load_account_mut(db, address)?.sstore_concrete_error(key, new, skip_cold_load)
    }
    ...
    pub fn tstore(&mut self, address: Address, key: StorageKey, new: StorageValue) {
        let had_value = if new.is_zero() {
            ...
            // insert values
            let previous_value =
                self.transient_storage.insert_value(address, key, new).unwrap_or_default();
...
mod tests {
    use super::*;
    use crate::{AccountInfo, EmptyDB, HashSet, U256, address};
