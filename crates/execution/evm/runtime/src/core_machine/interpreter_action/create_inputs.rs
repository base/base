...
use crate::{Address, B256, Bytes, U256, core_machine::CreateScheme, keccak256};
...
pub struct CreateInputs {
    ...
    /// exist at access time). Propagated onto [`crate::core_machine::CreateOutcome`] so the
...
impl CreateInputs {
    ...
    pub fn created_address(&self, nonce: u64) -> Address {
        *self.cached_address.get_or_init(|| match self.scheme {
            ...
            CreateScheme::Create2 { salt } => {
                self.caller.create2(salt.to_be_bytes(), self.init_code_hash())
            }
    ...
    pub fn init_code_hash(&self) -> B256 {
        *self.cached_init_code_hash.get_or_init(|| keccak256(self.init_code.as_ref()))
    }
