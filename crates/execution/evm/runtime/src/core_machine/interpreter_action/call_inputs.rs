...
use crate::{
    Address, B256, Bytecode, Bytes, U256,
    core_machine::{ContextTr, LocalContextTr},
};
...
pub enum CallInput {
    ...
    Bytes(Bytes),
    /// The Range points to the SharedMemory buffer. Buffer can be found in [`crate::core_machine::LocalContextTr::shared_memory_buffer_slice`] function.
    ...
    /// recommendation is to fetch buffer at first Inspector call and clone it from [`crate::core_machine::LocalContextTr::shared_memory_buffer_slice`] function.
...
impl CallInput {
    ...
    pub fn as_bytes_memory<'a>(
        &'a self,
        memory: &'a crate::core_machine::SharedMemory,
    ) -> impl core::ops::Deref<Target = [u8]> + 'a {
        match self {
            Self::Bytes(bytes) => CallInputRef::Bytes(bytes.as_ref()),
            Self::SharedBuffer(range) => {
...
mod tests {
    ...
    impl TestLocalContext {
        fn new(data: Vec<u8>) -> Self {
            Self { buffer: Rc::new(RefCell::new(data)) }
        }
    }
