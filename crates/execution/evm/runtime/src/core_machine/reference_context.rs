//! This module contains [`ReferenceContext`] struct and implements [`ContextTr`] trait for it.
...
use crate::{
    ...
    core_machine::{
        ...
        StateLoad, Transaction, TransactionType, journal::Journal, tx::TxEnv,
    },
    hardfork::SpecId,
...
pub struct ReferenceContext<TX = TxEnv, CFG = CfgEnv, DB: Database = EmptyDB, CHAIN = ()> {
    /// Block information.
    pub block: BlockEnv,
...
impl<TX: Transaction, DB: Database, CFG: Cfg, CHAIN> ContextTr
    for ReferenceContext<TX, CFG, DB, CHAIN>
{
    type Tx = TX;
    type Cfg = CFG;
    type Db = DB;
...
impl<TX: Transaction + Default, DB: Database, CHAIN: Default, SPEC: Default + Into<SpecId> + Clone>
    ReferenceContext<TX, CfgEnv<SPEC>, DB, CHAIN>
{
...
impl<TX, CFG, DB, CHAIN> ReferenceContext<TX, CFG, DB, CHAIN>
where
    TX: Transaction,
    CFG: Cfg,
    DB: Database,
{
    ...
    ///
    /// This will create a new [`Journal`] object.
    pub fn with_db<ODB: Database>(self, db: ODB) -> ReferenceContext<TX, CFG, ODB, CHAIN> {
        let mut journaled_state = Journal::new(db);
    ...
    pub fn with_ref_db<ODB: DatabaseRef>(
        self,
        db: ODB,
    ) -> ReferenceContext<TX, CFG, WrapDatabaseRef<ODB>, CHAIN> {
    ...
    pub fn with_block(self, block: BlockEnv) -> ReferenceContext<TX, CFG, DB, CHAIN> {
        ReferenceContext {
            tx: self.tx,
            block,
            cfg: self.cfg,
    ...
    pub fn with_tx<OTX: Transaction>(self, tx: OTX) -> ReferenceContext<OTX, CFG, DB, CHAIN> {
        ReferenceContext {
            tx,
            block: self.block,
            cfg: self.cfg,
    ...
    pub fn with_chain<OC>(self, chain: OC) -> ReferenceContext<TX, CFG, DB, OC> {
        ReferenceContext {
            tx: self.tx,
            block: self.block,
            cfg: self.cfg,
    ...
    /// Creates a new context with a new chain type.
    pub fn with_cfg<OCFG: Cfg>(mut self, cfg: OCFG) -> ReferenceContext<TX, OCFG, DB, CHAIN> {
...
impl<TX: Transaction, CFG: Cfg, DB: Database, CHAIN> Host for ReferenceContext<TX, CFG, DB, CHAIN> {
    /* Block */
