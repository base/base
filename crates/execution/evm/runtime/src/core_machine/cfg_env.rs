...
use crate::{
    core_machine::{Cfg, GasParams},
    eip170, eip3860, eip7825, eip7954,
    hardfork::SpecId,
};
...
pub struct CfgEnv<SPEC = SpecId> {
    ...
    ///
    /// Default values for Cancun is [`crate::eip4844::BLOB_BASE_FEE_UPDATE_FRACTION_CANCUN`]
    /// and for Prague is [`crate::eip4844::BLOB_BASE_FEE_UPDATE_FRACTION_PRAGUE`].
    ...
    /// [OutOfGasError::Memory][crate::core_machine::OutOfGasError::Memory] cannot be resized.
    ///
    ...
    pub fn blob_base_fee_update_fraction(&self) -> u64 {
        self.blob_base_fee_update_fraction.unwrap_or_else(|| {
            ...
            if spec.is_enabled_in(SpecId::PRAGUE) {
                crate::eip4844::BLOB_BASE_FEE_UPDATE_FRACTION_PRAGUE
            } else {
                crate::eip4844::BLOB_BASE_FEE_UPDATE_FRACTION_CANCUN
            }
        })
    }
...
impl<SPEC: Into<SpecId> + Clone> Cfg for CfgEnv<SPEC> {
    ...
    fn tx_gas_limit_cap(&self) -> u64 {
        self.tx_gas_limit_cap.unwrap_or(if self.spec.clone().into().is_enabled_in(SpecId::OSAKA) {
    ...
    fn max_initcode_size(&self) -> usize {
        self.limit_contract_initcode_size
            .or_else(|| self.limit_contract_code_size.map(|size| size.saturating_mul(2)))
