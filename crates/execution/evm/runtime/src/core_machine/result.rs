...
use crate::{
    Address, Bytes, DBErrorMarker, EvmState, Log, U256,
    core_machine::{context::ContextError, transaction::TransactionError},
};
...
impl ResultGas {
    ...
    pub const fn new(total_gas_spent: u64, refunded: u64, floor_gas: u64) -> Self {
        Self { total_gas_spent, refunded, floor_gas, state_gas_spent: 0 }
    }
    ...
    pub const fn new_with_state_gas(
        ...
        Self { total_gas_spent, refunded, floor_gas, state_gas_spent }
    }
    ...
    pub const fn block_regular_gas_used(&self) -> u64 {
        max(self.total_gas_spent().saturating_sub(self.state_gas_spent_final()), self.floor_gas())
    }
...
impl fmt::Display for ResultGas {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Gas used: {}, total spent: {}", self.tx_gas_used(), self.total_gas_spent())?;
        if self.refunded > 0 {
...
impl<HaltReasonTy> ExecutionResult<HaltReasonTy> {
    ...
    {
        match self {
            Self::Success { reason, gas, logs, output } => {
            ...
            Self::Halt { reason, gas, logs } => {
                ExecutionResult::Halt { reason: op(reason), gas, logs }
            }
        }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TxGasLimitGreaterThanCap { gas_limit, cap } => {
                write!(f, "transaction gas limit ({gas_limit}) is greater than the cap ({cap})")
            }
            Self::CallGasCostMoreThanGasLimit { initial_gas, gas_limit } => {
                write!(f, "call gas cost ({initial_gas}) exceeds the gas limit ({gas_limit})")
            }
            Self::GasFloorMoreThanGasLimit { gas_floor, gas_limit } => {
                write!(f, "gas floor ({gas_floor}) exceeds the gas limit ({gas_limit})")
            }
            ...
            Self::BlobGasPriceGreaterThanMax { block_blob_gas_price, tx_max_fee_per_blob_gas } => {
                write!(
                    f,
...
/// Errors related to misconfiguration of a [`crate::core_machine::Block`].
...
pub enum SuccessReason {
    /// Stop [`crate::opcode::STOP`] opcode.
    Stop,
    /// Return [`crate::opcode::RETURN`] opcode.
    Return,
...
pub enum HaltReason {
    ...
    StackOverflow,
    /// Invalid memory or storage offset for [`crate::opcode::RETURNDATACOPY`].
    OutOfOffset,
...
mod tests {
    ...
    fn test_execution_result_display() {
        ...
        let result: ExecutionResult<HaltReason> = ExecutionResult::Revert {
            gas: ResultGas::default().with_total_gas_spent(100000).with_refunded(100000),
            logs: vec![],
        ...
        let result: ExecutionResult<HaltReason> = ExecutionResult::Halt {
            ...
            gas: ResultGas::default().with_total_gas_spent(1000000).with_refunded(1000000),
            logs: vec![],
        };
        assert_eq!(
            result.to_string(),
    ...
    fn test_result_gas_display() {
        ...
        // With refund
        assert_eq!(
            ResultGas::default().with_total_gas_spent(50000).with_refunded(10000).to_string(),
    ...
    fn test_result_gas_used_and_remaining() {
        let gas = ResultGas::default().with_total_gas_spent(100).with_refunded(30);
        ...
        let gas = ResultGas::default().with_total_gas_spent(10).with_refunded(50);
    ...
    fn test_final_refunded_with_floor_gas() {
        ...
        let gas = ResultGas::default().with_total_gas_spent(50000).with_refunded(10000);
    ...
    fn test_block_regular_gas_used_no_floor_no_refund() {
        ...
        // With state gas.
        let gas = ResultGas::default().with_total_gas_spent(100_000).with_state_gas_spent(30_000);
        ...
        let gas = ResultGas::default().with_total_gas_spent(20_000).with_state_gas_spent(30_000);
