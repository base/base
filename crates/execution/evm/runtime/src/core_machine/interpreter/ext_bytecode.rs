...
use crate::{
    B256, Bytecode,
    core_machine::{InstructionResult, InterpreterAction},
    utils::read_u16,
};
...
impl ExtBytecode {
    /// Returns current bytecode original length. Used in [`crate::opcode::CODESIZE`] opcode.
    pub fn bytecode_len(&self) -> usize {
        self.base.len()
    }

    /// Returns current bytecode original slice. Used in [`crate::opcode::CODECOPY`] opcode.
