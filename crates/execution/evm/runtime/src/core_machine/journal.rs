...
use crate::{
    Account, Address, AddressMap, AddressSet, B256, Bytecode, Database, EvmState, HashSet, Log,
    StorageKey, StorageValue, U256,
    core_machine::{
        AccountInfoLoad, AccountLoad, JournalCheckpoint, JournalLoadError, JournalTr,
        JournaledAccount, SStoreResult, SelfDestructResult, StateLoad, TransferError,
    },
    hardfork::SpecId,
};
...
impl<DB: Database> JournalTr for Journal<DB> {
    ...
    fn set_eip7708_config(&mut self, disabled: bool, eip8246_delayed_clear_disabled: bool) {
        self.inner.set_eip7708_config(disabled, eip8246_delayed_clear_disabled);
    }
    ...
    fn load_account_mut_skip_cold_load(
        ...
        self.inner.load_account_mut_optional(&mut self.database, address, skip_cold_load)
    }
