...
use crate::{
    Address, AddressMap, B256Map, Bytecode, HashMap,
...
impl CacheState {
        ...
        T: IntoIterator<Item = (Address, Cow<'a, Account>)>,
    {
        evm_state.into_iter().filter_map(move |(address, account)| {
            inspect(&address, &account);
            self.apply_account_state(address, account).map(move |transition| (address, transition))
        })
    }
