...
use crate::{
    B256, Bytecode, U256,
    core_memory::{AccountInfo, EvmStorage},
    hash_map,
};
    ...
    pub fn update(&mut self, other: TransitionAccount<Option<Cow<'_, EvmStorage>>>) {
        ...
        let storage = other.storage.into_iter().flat_map(|storage| match storage {
            Cow::Borrowed(storage) => {
                Either::Left(storage.iter().map(|(k, v)| (Cow::Borrowed(k), Cow::Borrowed(v))))
            }
            Cow::Owned(storage) => {
                Either::Right(storage.into_iter().map(|(k, v)| (Cow::Owned(k), Cow::Owned(v))))
            }
        });
...
impl<S> TransitionAccount<S> {
    ...
    where
        F: FnOnce(S) -> N,
    {
        let Self { info, status, previous_info, previous_status, storage, storage_was_destroyed } =
            self;
        TransitionAccount {
            info,
            status,
