...
use crate::{
    HashMap, StorageKey, StorageValue, U256,
    core_memory::{Account, AccountInfo, EvmStorage},
};
...
impl CacheAccount {
    ...
    pub fn storage_slot(&self, slot: StorageKey) -> Option<StorageValue> {
        self.account.as_ref().and_then(|a| a.storage.get(&slot).cloned())
    }
    ...
    pub fn newly_created<'a>(
        ...
        self.account = Some(PlainAccount { info, storage: new_bundle_storage });
        transition_account
    }
    ...
    pub fn change<'a>(
        ...
        this_storage.extend(
            storage.iter().filter_map(|(k, s)| s.is_changed().then_some((*k, s.present_value))),
        );
        let changed_account = PlainAccount { info, storage: this_storage };
