...
use crate::{BLOCK_HASH_HISTORY, alloy_primitives::B256};
...
impl BlockHashCache {
    ...
    #[inline]
    pub fn new() -> Self {
        Self { hashes: Box::new([(None, B256::ZERO); BLOCK_HASH_HISTORY_USIZE]) }
    }
    ...
    pub fn get(&self, block_number: u64) -> Option<B256> {
        ...
        if Some(block_number) == stored_block_number { Some(stored_hash) } else { None }
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (u64, B256)> + '_ {
        self.hashes.iter().filter_map(|(block_number, hash)| block_number.map(|n| (n, *hash)))
    }
