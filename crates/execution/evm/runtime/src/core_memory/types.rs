...
use crate::{Address, AddressMap, StorageKey, StorageKeyMap, StorageValue};
...
impl TransientStorage {
    ...
    /// storage `key`.
    ///
    /// Returns [`StorageValue::ZERO`](crate::StorageValue) (the default) if
    /// the slot was never set.
    #[inline]
    pub fn get_value(&self, address: Address, key: StorageKey) -> StorageValue {
        self.0.get(&address).and_then(|slots| slots.get(&key)).copied().unwrap_or_default()
    }
