...
use crate::{
    Address, AddressIndexMap, StorageKey, StorageValue,
    core_memory::{Account, AccountId, AccountInfo},
};
...
impl Bal {
    ...
    pub fn new() -> Self {
        Self { accounts: AddressIndexMap::default() }
    }
    ...
    pub fn account_storage(
        ...
        let Some(storage_value) = bal_account.storage.get(address, key, bal_index)? else {
            return Err(BalError::SlotNotFound { address: *address, slot: key });
        };
...
mod tests {
    ...
    use super::*;
    use crate::{B256, Bytecode, Bytes, U256};
