...
use crate::{
    ...
    StorageValue, U256, U256Map,
    core_memory::{
        Account, AccountInfo, BENCH_CALLER, BENCH_CALLER_BALANCE, BENCH_TARGET,
        BENCH_TARGET_BALANCE, Bytecode, Database, DatabaseCommit, DatabaseRef, EmptyDB,
    },
    hash_map::Entry,
};
...
impl<ExtDb> CacheDB<CacheDB<ExtDb>> {
    ...
    pub fn flatten(self) -> CacheDB<ExtDb> {
        let CacheDB {
            cache: Cache { accounts, contracts, logs, block_hashes }, db: mut inner, ..
        } = self;
    ...
    pub fn pretty_print(&self) -> String {
        ...
        for (address, db_account) in accounts {
            ...
            if !db_account.storage.is_empty() {
                output.push_str(&format!("    storage: {} slots\n", db_account.storage.len()));
        ...
        if !self.cache.block_hashes.is_empty() {
            output.push_str(&format!("  block_hashes: {} total\n", self.cache.block_hashes.len()));
...
impl Database for BenchmarkDB {
    ...
    fn code_by_hash(&mut self, code_hash: B256) -> Result<Bytecode, Self::Error> {
        if code_hash == self.1 { Ok(self.0.clone()) } else { Ok(Bytecode::default()) }
    }
...
mod tests {
    ...
    use crate::{
        Address, HashMap, StorageKey, StorageValue,
        core_memory::{
            Account, AccountInfo, Database, DatabaseCommit, EvmStorageSlot, TransactionId,
        },
    };
    ...
    fn test_replace_account_storage() {
        ...
        let mut new_state = CacheDB::new(init_state);
        new_state.replace_account_storage(account, HashMap::from_iter([(key1, value1)])).unwrap();
    ...
    fn commit_iter_applies_repeated_account_updates_in_order() {
        ...
        let first = Account::from(AccountInfo { nonce: 1, ..Default::default() })
            .with_touched_mark()
            .with_storage(
                [(
                    key,
        ...
        let second =
            Account::from(AccountInfo { nonce: 2, ..Default::default() }).with_touched_mark();
    ...
    fn test_pretty_print_cachedb() {
        use crate::{B256, Bytes, Log, LogData, U256};

        let account = Address::with_last_byte(55);
        let mut cachedb = CacheDB::new(EmptyDB::default());
        cachedb.insert_account_info(account, AccountInfo { nonce: 7, ..Default::default() });
        ...
        cachedb.cache.block_hashes.insert(U256::from(123u64), B256::from([1u8; 32]));
    ...
    fn test_serialize_deserialize_cachedb() {
        ...
        assert_eq!(deserialized.cache.accounts.get(&account).unwrap().info.nonce, nonce);
    }
}
