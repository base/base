...
use crate::{
    Address, AddressMap, B256, StorageKey, StorageValue, U256, address,
    core_memory::{Account, AccountId, AccountInfo, Bytecode, ErasedError, TransactionId},
};
    ...
    fn increment_balances(
        ...
        let transitions = balances
            ...
            .map(|(address, balance)| {
                ...
                original_account.info.balance =
                    original_account.info.balance.saturating_add(U256::from(balance));
...
mod tests {
    ...
    fn wrappers_forward_commit_iter() {
        ...
        db.commit_iter(&mut changes().into_iter());
        let ::either::Either::Left(db) = db else { unreachable!() };
        ...
        let mut db = crate::core_memory::BalDatabase::new(MockDb::default()).with_bal_builder();
