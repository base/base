...
use crate::{
    Address, B256, Bytecode, BytecodeDecodeError, StorageKey, StorageValue, U256,
    core_memory::{
        Account, AccountInfo, EvmStorage,
        bal::{BalError, BlockAccessIndex, writes::BalWrites},
    },
};
...
impl AccountBal {
    ...
    pub fn update(&mut self, bal_index: BlockAccessIndex, account: &Account) {
        if account.is_selfdestructed_locally() {
            ...
            self.account_info.update(bal_index, &account.original_info(), &empty_info);
        ...
        self.account_info.update(bal_index, &account.original_info(), &account.info);
