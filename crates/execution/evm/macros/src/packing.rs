...
pub(crate) fn gen_constants_from_ir(fields: &[LayoutField<'_>], gen_location: bool) -> TokenStream {
    ...
    for field in fields {
        ...
        let slots_to_end = quote! {
            ::alloy_primitives::U256::from_limbs([<#ty as ::base_execution_evm_runtime::StorableType>::SLOTS as u64, 0, 0, 0])
                .saturating_sub(::alloy_primitives::U256::ONE)
        };

        let bytes_expr = quote! { <#ty as ::base_execution_evm_runtime::StorableType>::BYTES };
        ...
        if gen_location {
            ...
            constants.extend(quote! {
                #[doc = #loc_doc]
                pub const #loc_const: ::base_execution_evm_runtime::FieldLocation =
                    ::base_execution_evm_runtime::FieldLocation::new(#slot_const.as_limbs()[0] as usize, #offset_const, #bytes_expr);
            });
        }
...
fn gen_auto_slot_expr(
    ...
    for candidate in previous_auto_fields.iter().filter(|candidate| {
        ...
        if candidate.assigned_slot.allows_type_namespace() {
            ...
            output = (
                quote! {
                    if <#candidate_ty as ::base_execution_evm_runtime::StorableType>::HAS_STORAGE_NAMESPACE {
                        #fallback_slot
                    } else {
                        #candidate_slot
                    }
    ...
    if field.assigned_slot.allows_type_namespace() {
        ...
        (
            quote! {
                if <#field_ty as ::base_execution_evm_runtime::StorableType>::HAS_STORAGE_NAMESPACE {
                    <#field_ty as ::base_execution_evm_runtime::StorableType>::STORAGE_NAMESPACE_ROOT
                        .checked_add(#slots_to_end).expect("slot overflow")
                        .saturating_sub(#slots_to_end)
                } else {
                    #normal_slot
...
pub(crate) fn gen_slot_packing_logic(
    ...
    let prev_layout_slots = quote! {
        ::alloy_primitives::U256::from_limbs([<#prev_ty as ::base_execution_evm_runtime::StorableType>::SLOTS as u64, 0, 0, 0])
    };
    let curr_slots_to_end = quote! {
        ::alloy_primitives::U256::from_limbs([<#curr_ty as ::base_execution_evm_runtime::StorableType>::SLOTS as u64, 0, 0, 0])
            .saturating_sub(::alloy_primitives::U256::ONE)
    };

    let can_pack_expr = quote! {
        #prev_offset_expr
            + <#prev_ty as ::base_execution_evm_runtime::StorableType>::BYTES
            + <#curr_ty as ::base_execution_evm_runtime::StorableType>::BYTES <= 32
    };
    ...
    let offset_expr = quote! {{
        if #can_pack_expr { #prev_offset_expr + <#prev_ty as ::base_execution_evm_runtime::StorableType>::BYTES } else { 0 }
    }};
...
pub(crate) fn gen_layout_ctx_expr(
    ...
    if !is_manual_slot && let Some(shares_slot_check) = shares_slot_check {
        quote! {
            {
                if #shares_slot_check && <#ty as ::base_execution_evm_runtime::StorableType>::IS_PACKABLE {
                    ::base_execution_evm_runtime::LayoutCtx::packed(#offset_const_ref)
                } else {
        ...
        quote! { ::base_execution_evm_runtime::LayoutCtx::FULL }
    }
}
...
pub(crate) fn gen_collision_check_fn(
    ...
    fn gen_slot_count_expr(ty: &Type) -> TokenStream {
        quote! { ::alloy_primitives::U256::from_limbs([<#ty as ::base_execution_evm_runtime::StorableType>::SLOTS as u64, 0, 0, 0]) }
    }
        ...
        checks.extend(quote! {
            {
                ...
                let no_overlap = if slot == other_slot {
                    let byte_end = #offset_const + <#field_ty as ::base_execution_evm_runtime::StorableType>::BYTES;
                    let other_byte_end = #other_offset_const + <#other_ty as ::base_execution_evm_runtime::StorableType>::BYTES;
