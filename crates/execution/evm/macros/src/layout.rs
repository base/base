...
pub(crate) fn gen_handler_field_decl(field: &LayoutField<'_>) -> proc_macro2::TokenStream {
    ...
    let handler_type = match &field.kind {
        FieldKind::Direct(ty) => {
            quote! { <#ty as ::base_execution_evm_runtime::StorableType>::Handler<'a> }
        }
        FieldKind::Mapping { key, value } => {
            quote! { <::base_execution_evm_runtime::Mapping<#key, #value> as ::base_execution_evm_runtime::StorableType>::Handler<'a> }
        }
    };
...
pub(crate) fn gen_handler_field_init(
    ...
    match &field.kind {
        FieldKind::Direct(ty) => {
            ...
            quote! {
                #field_name: <#ty as ::base_execution_evm_runtime::StorableType>::handle(
        ...
        FieldKind::Mapping { key, value } => {
            quote! {
                #field_name: <::base_execution_evm_runtime::Mapping<#key, #value> as ::base_execution_evm_runtime::StorableType>::handle(
                    #slot_expr, ::base_execution_evm_runtime::LayoutCtx::FULL, address, storage
                )
            }
        }
    }
...
pub(crate) fn gen_constructor(
    ...
    let new_fn = address.map(|addr| {
        quote! {
            ...
            pub fn new(storage: ::base_execution_evm_runtime::StorageCtx<'a>) -> Self {
    ...
    quote! {
        impl<'a> #name<'a> {
            ...
            #[inline(always)]
            fn __initialize(&mut self) -> ::base_execution_evm_runtime::Result<()> {
                let bytecode = ::base_execution_evm_runtime::Bytecode::new_legacy(::alloy_primitives::Bytes::from_static(&[0xef]));
                self.storage.set_code(self.address, bytecode)?;
                Ok(())
            }

            #[inline(always)]
            fn emit_event(&mut self, event: impl ::alloy_primitives::IntoLogData) -> ::base_execution_evm_runtime::Result<()> {
...
pub(crate) fn gen_contract_storage_impl(name: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl<'a> ::base_execution_evm_runtime::ContractStorage<'a> for #name<'a> {
            #[inline(always)]
            ...
            #[inline(always)]
            fn storage(&self) -> ::base_execution_evm_runtime::StorageCtx<'a> {
                self.storage
            }
        }
    }
