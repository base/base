...
impl UpgradeSignalMetrics {
    ...
    pub fn record_signal(layer: UpgradeSignalMetricLayer, signal: &UpgradeSignal) {
        ...
        Self::activation_timestamp(layer, hardfork_id.clone())
            .set(signal.activation_timestamp as f64);
        Self::expected_protocol_version(layer, hardfork_id.clone())
            .set(Self::protocol_version_to_f64(signal.protocol_version));
        Self::last_l1_read_block(layer, hardfork_id).set(signal.l1_block_number as f64);
    }

    /// Records a failed L1 read for one hardfork ID.
    pub fn record_l1_read_error(layer: UpgradeSignalMetricLayer, hardfork_id: BaseUpgrade) {
        Self::init();
        Self::l1_read_errors_total(layer.label(), hardfork_id.contract_id().to_string())
            .increment(1);
    }

    /// Records a failed L1 read for one hardfork ID across all enabled layers.
    pub fn record_l1_read_error_for_layers(
        ...
        Self::init();
        for layer in layers {
            Self::record_l1_read_error(*layer, hardfork_id);
        }
    }

    /// Records failed L1 reads for all configured hardfork IDs.
    pub fn record_l1_read_errors(layer: UpgradeSignalMetricLayer, hardfork_ids: &[BaseUpgrade]) {
        Self::init();
    ...
    /// Records failed L1 reads for all configured hardfork IDs across all enabled layers.
    pub fn record_l1_read_errors_for_layers(
        ...
        Self::init();
        for layer in layers {
            Self::record_l1_read_errors(*layer, hardfork_ids);
        }
    }

    /// Records a live L1 signal value update for one hardfork ID.
    pub fn record_signal_update(layer: UpgradeSignalMetricLayer, hardfork_id: BaseUpgrade) {
        Self::init();
        Self::signal_updates_total(layer.label(), hardfork_id.contract_id().to_string())
            .increment(1);
    }
