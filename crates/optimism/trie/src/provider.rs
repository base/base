...
fn state_root_error_to_database_error(err: StateRootError) -> DatabaseError {
    match err {
        StateRootError::Database(err) |
        StateRootError::StorageRootError(StorageRootError::Database(err)) => err,
        StateRootError::PrefixSetLoadError(err) => DatabaseError::Other(err.to_string()),
    }
}

fn storage_root_error_to_database_error(err: StorageRootError) -> DatabaseError {
    match err {
    ...
    for OpProofsStateProviderRef<'a, Storage>
{
    fn state_root(&self, state: HashedPostState) -> ProviderResult<B256> {
        StateRoot::overlay_root(self.storage, self.block_number, state)
            .map_err(|err| ProviderError::Database(state_root_error_to_database_error(err)))
    }
    ...
    for OpProofsStateProviderRef<'a, Storage>
{
    fn storage_root(&self, address: Address, storage: HashedStorage) -> ProviderResult<B256> {
        StorageRoot::overlay_root(self.storage, self.block_number, address, storage)
            .map_err(|err| ProviderError::Database(storage_root_error_to_database_error(err)))
    }
