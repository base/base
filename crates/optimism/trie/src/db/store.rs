...
impl MdbxProofsStorage {
    ...
    fn store_trie_updates_for_block(
        ...
        let account_trie_keys = self.persist_history_batch(
            tx,
            block_number,
            sorted_trie_updates.account_nodes_ref().into_iter().cloned(),
            append_mode,
        )?;
        ...
        for (hashed_address, nodes) in sorted_trie_updates.storage_tries_ref().iter() {
            ...
            let keys = self.persist_history_batch(
                tx,
                block_number,
                nodes.storage_nodes_ref().into_iter().map(|(path, node)| (*hashed_address, *path, node.clone())),
                append_mode,
            )?;
            storage_trie_keys.extend(keys);
        }
