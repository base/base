...
mod tests {
    ...
    #[test]
    fn test_slot_delete_clears_value() -> crate::error::Result<()> {
        let (mut storage, address) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            let slot_num = U256::from(50u64);
            let value = U256::from(42u64);
            let mut slot = Slot::<U256>::new(slot_num, address, ctx);
            slot.write(value)?;
            assert_eq!(slot.read()?, value);
            slot.delete()?;
            assert_eq!(slot.read()?, U256::ZERO);
            Ok(())
        })
    }

    #[test]
    fn test_slot_overwrite_updates_value() -> crate::error::Result<()> {
        let (mut storage, address) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            let slot_num = U256::from(51u64);
            let mut slot = Slot::<U256>::new(slot_num, address, ctx);
            slot.write(U256::from(1u64))?;
            assert_eq!(slot.read()?, U256::from(1u64));
            slot.write(U256::from(2u64))?;
            assert_eq!(slot.read()?, U256::from(2u64));
            Ok(())
        })
    }
}
