...
mod tests {
    ...
    #[test]
    fn test_set_duplicate_insert_is_idempotent() {
        let (mut storage, contract_addr) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            assert!(handler.insert(a).unwrap()); // first insert: newly added
            assert!(!handler.insert(a).unwrap()); // duplicate: already present
            assert_eq!(handler.len().unwrap(), 1);
            assert!(handler.contains(&a).unwrap());
        });
    }
    ...
    #[test]
    fn test_set_remove_nonexistent_is_idempotent() {
        let (mut storage, contract_addr) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            // Removing an element never inserted must return false and not panic.
            assert!(!handler.remove(&a).unwrap());
    ...
    #[test]
    fn test_set_multiple_element_types() {
        let (mut storage, contract_addr) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            let base = U256::from(900u64);
            let mut handler = SetHandler::<U256>::new(base, contract_addr, ctx);

            let values: Vec<U256> = (1u64..=5).map(U256::from).collect();
            for &v in &values {
                assert!(handler.insert(v).unwrap());
            }
            assert_eq!(handler.len().unwrap(), 5);
            for v in &values {
                assert!(handler.contains(v).unwrap());
            }

            // Remove the first two elements and verify the set shrinks correctly.
            assert!(handler.remove(&values[0]).unwrap());
            assert!(handler.remove(&values[1]).unwrap());

            assert_eq!(handler.len().unwrap(), 3);
            assert!(!handler.contains(&values[0]).unwrap());
            assert!(!handler.contains(&values[1]).unwrap());
            for v in &values[2..] {
                assert!(handler.contains(v).unwrap());
            }
        });
