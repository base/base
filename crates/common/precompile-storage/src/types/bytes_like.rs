...
mod tests {
    ...
    #[test]
    fn test_bytes_overwrite_shorter_value() {
        let (mut storage, address) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            let base = U256::from(500u64);
            // 40 bytes uses long encoding (bit 0 = 1 in the base slot).
            let long_bytes = Bytes::from(vec![0xAAu8; 40]);
            // 5 bytes uses short (inline) encoding.
            let short_bytes = Bytes::from(vec![0xBBu8; 5]);

            let mut handler = BytesLikeHandler::<Bytes>::new(base, address, ctx);
            handler.write(long_bytes).unwrap();
            assert_eq!(handler.len().unwrap(), 40);

            // Overwrite the long value with a short one.
            handler.write(short_bytes.clone()).unwrap();
            let loaded = handler.read().unwrap();
            assert_eq!(loaded, short_bytes);
    ...
    fn test_empty_bytes_roundtrip() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            let mut handler = BytesLikeHandler::<Bytes>::new(base, address, ctx);
            handler.write(empty.clone()).unwrap();
            let loaded = handler.read().unwrap();
