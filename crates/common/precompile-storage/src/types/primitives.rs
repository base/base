...
mod tests {
    ...
    #[test]
    fn test_u256_primitive_boundaries() {
        let (mut storage, address) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            let base = U256::from(200u64);

            let mut zero_slot = U256::handle(base, LayoutCtx::FULL, address, ctx);
            zero_slot.write(U256::ZERO).unwrap();
            assert_eq!(zero_slot.read().unwrap(), U256::ZERO);

            let mut max_slot = U256::handle(base + U256::ONE, LayoutCtx::FULL, address, ctx);
            max_slot.write(U256::MAX).unwrap();
            assert_eq!(max_slot.read().unwrap(), U256::MAX);

            // 2^128 — straddles the boundary between the two 128-bit limb halves.
            let mid = U256::from_limbs([0u64, 0u64, 1u64, 0u64]);
            let mut mid_slot = U256::handle(base + U256::from(2u64), LayoutCtx::FULL, address, ctx);
            mid_slot.write(mid).unwrap();
            assert_eq!(mid_slot.read().unwrap(), mid);
        });
    }

    #[test]
    fn test_u64_primitive_roundtrip() {
        let (mut storage, address) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            let base = U256::from(300u64);
            let value: u64 = u64::MAX - 1;
            let mut slot = u64::handle(base, LayoutCtx::FULL, address, ctx);
    ...
    #[test]
    fn test_u8_primitive_packing() {
        let (mut storage, address) = setup_storage();
        StorageCtx::enter(&mut storage, |ctx| {
            let base = U256::from(400u64);
            // Two u8 values packed at byte offsets 0 and 1 within the same slot.
            let mut slot0 = Slot::<u8>::new_with_ctx(base, LayoutCtx::packed(0), address, ctx);
            let mut slot1 = Slot::<u8>::new_with_ctx(base, LayoutCtx::packed(1), address, ctx);

            slot0.write(0xAAu8).unwrap();
            slot1.write(0xBBu8).unwrap();

            // Neither write must overwrite the other's byte.
            assert_eq!(slot0.read().unwrap(), 0xAAu8);
            assert_eq!(slot1.read().unwrap(), 0xBBu8);
        });
    }
