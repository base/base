...
mod tests {
    ...
    #[test]
    fn test_mapping_read_write_delete() {
        let address = Address::from([0x10; 20]);
        let base_slot = U256::from(10u64);
        let (mut storage, _) = crate::hashmap::setup_storage();
        crate::StorageCtx::enter(&mut storage, |ctx| {
            let key = Address::from([0xAA; 20]);
            let value = U256::from(12345u64);
            let mut mapping = MappingHandler::<Address, U256>::new(base_slot, address, ctx);

            mapping.at_mut(&key).write(value).unwrap();
            assert_eq!(mapping.at(&key).read().unwrap(), value);

            mapping.at_mut(&key).delete().unwrap();
            assert_eq!(mapping.at(&key).read().unwrap(), U256::ZERO);
        });
    }

    #[test]
    fn test_mapping_independent_keys_do_not_collide() {
        let address = Address::from([0x10; 20]);
        let base_slot = U256::from(11u64);
        let (mut storage, _) = crate::hashmap::setup_storage();
        crate::StorageCtx::enter(&mut storage, |ctx| {
            let key1 = Address::from([0x11; 20]);
            let key2 = Address::from([0x22; 20]);
            let val1 = U256::from(111u64);
            let val2 = U256::from(222u64);
            let mut mapping = MappingHandler::<Address, U256>::new(base_slot, address, ctx);

            mapping.at_mut(&key1).write(val1).unwrap();
            mapping.at_mut(&key2).write(val2).unwrap();

            assert_eq!(mapping.at(&key1).read().unwrap(), val1);
            assert_eq!(mapping.at(&key2).read().unwrap(), val2);
        });
    }

    #[test]
    fn test_mapping_handler_write_and_read() {
        let address = Address::from([0x10; 20]);
        let base_slot = U256::from(12u64);
        let (mut storage, _) = crate::hashmap::setup_storage();
        crate::StorageCtx::enter(&mut storage, |ctx| {
            let key = Address::from([0xBB; 20]);
            let value = U256::from(99999u64);
            let mut mapping = MappingHandler::<Address, U256>::new(base_slot, address, ctx);

            // Write via the at_mut handler interface and read back via at.
            mapping.at_mut(&key).write(value).unwrap();
            assert_eq!(mapping.at(&key).read().unwrap(), value);

            // The cached handler returns a stable slot for repeated access.
            assert_eq!(mapping.at(&key).slot(), mapping.at(&key).slot());
        });
    }
}
