...
pub trait StorageOps {
    ...
    /// Computes keccak256 and charges the appropriate gas.
    ///
    /// Implementations backed by a live EVM context (e.g. [`crate::types::Slot`]) delegate
    /// to [`crate::storage_ctx::StorageCtx::metered_keccak256`] so the cost is deducted from
    /// the gas meter. Every implementor must be explicit — there is no default — to prevent
    /// unmetered keccak from silently slipping through.
...
pub trait StorageKey: sealed::OnlyPrimitives {
    ...
    ///
    /// The default packs `key_bytes` right-aligned into a 64-byte buffer followed by
    /// `slot` in big-endian — matching Solidity's mapping key encoding for fixed-size
    /// primitive types. `String` overrides this to use raw bytes without padding.
    ...
    /// Computes `keccak256(lpad32(key) ‖ slot_be32)` without gas metering.
    ///
    /// Used only for tests and off-chain verification. Production mapping access goes
    /// through [`MappingHandler::at`] / [`MappingHandler::at_mut`], which routes
    /// keccak through the gas meter via `StorageOps::metered_keccak256`.
    #[doc(hidden)]
