...
#[test]
fn namespace_id_case_sensitive_produces_distinct_roots() {
    // Namespace IDs differing only by case must hash to different ERC-7201 roots.
    let upper = erc7201_root("b20.Policy");
    let lower = erc7201_root("b20.policy");

    assert_ne!(upper, lower, "namespace IDs differing only by case must produce distinct roots");
    assert_ne!(upper, U256::ZERO, "upper-case root must be non-zero");
    assert_ne!(lower, U256::ZERO, "lower-case root must be non-zero");
}
