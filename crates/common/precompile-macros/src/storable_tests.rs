    ...
    let tests: Vec<_> = RUST_INT_SIZES.iter().map(|&size| {
        ...
        quote! {
            ...
            fn #test_name(value in any::<#type_name>(), base_slot in arb_safe_slot()) {
                ...
                ::base_common_precompiles::StorageCtx::enter(&mut storage, |ctx| {
                    let mut slot = ::base_common_precompiles::Slot::<#type_name>::new(base_slot, address, ctx);
    ...
    let tests: Vec<_> = ALLOY_INT_SIZES.iter().map(|&size| {
        ...
        quote! {
            ...
            fn #test_name(value in #arb_fn(), base_slot in arb_safe_slot()) {
                let (mut storage, address) = setup_storage();
                ::base_common_precompiles::StorageCtx::enter(&mut storage, |ctx| {
                    let mut slot = ::base_common_precompiles::Slot::<::alloy_primitives::aliases::#type_name>::new(base_slot, address, ctx);
                    slot.write(value).unwrap();
                    let loaded = slot.read().unwrap();
                    assert_eq!(value, loaded, concat!(#label, " roundtrip failed"));
                    slot.delete().unwrap();
    ...
    let tests: Vec<_> = FIXED_BYTES_SIZES.iter().map(|&size| {
        ...
        quote! {
            ...
            fn #test_name(value in #arb_fn(), base_slot in arb_safe_slot()) {
                let (mut storage, address) = setup_storage();
                ::base_common_precompiles::StorageCtx::enter(&mut storage, |ctx| {
                    let mut slot = ::base_common_precompiles::Slot::<::alloy_primitives::FixedBytes<#size>>::new(base_slot, address, ctx);
                    slot.write(value).unwrap();
                    let loaded = slot.read().unwrap();
                    assert_eq!(
                        value, loaded,
