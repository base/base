...
fn gen_storable_layout_impl(type_path: &TokenStream, byte_count: usize) -> TokenStream {
    quote! {
        impl ::base_execution_evm_precompiles::StorableType for #type_path {
            const LAYOUT: ::base_execution_evm_precompiles::Layout = ::base_execution_evm_precompiles::Layout::Bytes(#byte_count);
...
fn gen_storage_key_impl(type_path: &TokenStream, strategy: &StorageKeyStrategy) -> TokenStream {
    ...
    quote! {
        impl ::base_execution_evm_precompiles::StorageKey for #type_path {
            #[inline]
...
fn gen_to_word_impl(type_path: &TokenStream, strategy: &StorableConversionStrategy) -> TokenStream {
    match strategy {
        StorableConversionStrategy::UnsignedRust => quote! {
            impl ::base_execution_evm_precompiles::FromWord for #type_path {
                #[inline]
                fn to_word(&self) -> ::alloy_primitives::U256 {
                    ::alloy_primitives::U256::from(*self)
                }
                #[inline]
                fn from_word(word: ::alloy_primitives::U256) -> ::base_execution_evm_precompiles::Result<Self> {
                    word.try_into().map_err(|_| ::base_execution_evm_precompiles::BasePrecompileError::under_overflow())
                }
            }
        },
            ...
            impl ::base_execution_evm_precompiles::FromWord for #type_path {
                ...
                fn from_word(word: ::alloy_primitives::U256) -> ::base_execution_evm_precompiles::Result<Self> {
                    if word > ::alloy_primitives::U256::from(::alloy_primitives::aliases::#ty::MAX) {
                        return Err(::base_execution_evm_precompiles::BasePrecompileError::under_overflow());
                    }
                    Ok(word.to::<Self>())
                }
            }
            ...
            impl ::base_execution_evm_precompiles::FromWord for #type_path {
                ...
                fn from_word(word: ::alloy_primitives::U256) -> ::base_execution_evm_precompiles::Result<Self> {
                    let unsigned: #unsigned_type = word.try_into()
                        .map_err(|_| ::base_execution_evm_precompiles::BasePrecompileError::under_overflow())?;
                    Ok(unsigned as Self)
                }
            }
        },
...
fn gen_complete_impl_set(config: &TypeConfig) -> TokenStream {
    ...
    let full_word_storable_impl = if config.byte_count < 32 {
        quote! {
            impl ::base_execution_evm_precompiles::sealed::OnlyPrimitives for #type_path {}
            impl ::base_execution_evm_precompiles::Packable for #type_path {}
        }
    } else {
        quote! {
            impl ::base_execution_evm_precompiles::sealed::OnlyPrimitives for #type_path {}
            impl ::base_execution_evm_precompiles::Storable for #type_path {
                #[inline]
                fn load<S: ::base_execution_evm_precompiles::StorageOps>(
                    ...
                    storage.load(slot).and_then(<Self as ::base_execution_evm_precompiles::FromWord>::from_word)
                }
                #[inline]
                fn store<S: ::base_execution_evm_precompiles::StorageOps>(
                    ...
                    storage.store(slot, <Self as ::base_execution_evm_precompiles::FromWord>::to_word(self))
                }
            }
        }
    };
...
fn gen_array_impl(config: &ArrayConfig) -> TokenStream {
    ...
    let slot_count_expr = if *elem_is_packable {
        quote! { ::base_execution_evm_precompiles::calc_packed_slot_count(#array_size, #elem_byte_count) }
    } else {
        quote! { #array_size }
    };
    ...
    quote! {
        impl ::base_execution_evm_precompiles::StorableType for [#elem_type; #array_size] {
            const LAYOUT: ::base_execution_evm_precompiles::Layout = ::base_execution_evm_precompiles::Layout::Slots(#slot_count_expr);
            type Handler<'a> = ::base_execution_evm_precompiles::ArrayHandler<'a, #elem_type, #array_size>;
            ...
            fn handle<'a>(
                ...
                debug_assert_eq!(ctx, ::base_execution_evm_precompiles::LayoutCtx::FULL, "Arrays cannot be packed");
                Self::Handler::new(slot, address, storage)
            }
        }

        impl ::base_execution_evm_precompiles::Storable for [#elem_type; #array_size] {
            #[inline]
            fn load<S: ::base_execution_evm_precompiles::StorageOps>(storage: &S, slot: ::alloy_primitives::U256, ctx: ::base_execution_evm_precompiles::LayoutCtx) -> ::base_execution_evm_precompiles::Result<Self> {
                debug_assert_eq!(ctx, ::base_execution_evm_precompiles::LayoutCtx::FULL, "Arrays can only be loaded with LayoutCtx::FULL");
                use ::base_execution_evm_precompiles::{Word, calc_element_slot, calc_element_offset};
                let base_slot = slot;
                #load_impl
            }

            #[inline]
            fn store<S: ::base_execution_evm_precompiles::StorageOps>(&self, storage: &mut S, slot: ::alloy_primitives::U256, ctx: ::base_execution_evm_precompiles::LayoutCtx) -> ::base_execution_evm_precompiles::Result<()> {
                debug_assert_eq!(ctx, ::base_execution_evm_precompiles::LayoutCtx::FULL, "Arrays can only be stored with LayoutCtx::FULL");
                use ::base_execution_evm_precompiles::{Word, calc_element_slot, calc_element_offset};
                let base_slot = slot;
                #store_impl
            }
    ...
    quote! {
        ...
        for i in 0..#array_size {
            ...
            let slot_addr = base_slot
                ...
                .ok_or(::base_execution_evm_precompiles::BasePrecompileError::SlotOverflow)?;
...
fn gen_packed_array_store(array_size: &usize, elem_byte_count: &usize) -> TokenStream {
    quote! {
        let slot_count = ::base_execution_evm_precompiles::calc_packed_slot_count(#array_size, #elem_byte_count);
    ...
    quote! {
        ...
        for i in 0..#array_size {
            let elem_slot = base_slot
                ...
                .ok_or(::base_execution_evm_precompiles::BasePrecompileError::SlotOverflow)?;
            result[i] = ::base_execution_evm_precompiles::Storable::load(storage, elem_slot, ::base_execution_evm_precompiles::LayoutCtx::FULL)?;
        }
        Ok(result)
    }
}
    ...
    quote! {
        for (i, elem) in self.iter().enumerate() {
            let elem_slot = base_slot
                .checked_add(::alloy_primitives::U256::from(i))
                .ok_or(::base_execution_evm_precompiles::BasePrecompileError::SlotOverflow)?;
            ::base_execution_evm_precompiles::Storable::store(elem, storage, elem_slot, ::base_execution_evm_precompiles::LayoutCtx::FULL)?;
        }
        Ok(())
    }
}
...
fn gen_struct_array_impl(struct_type: &TokenStream, array_size: usize) -> TokenStream {
    ...
    quote! {
        mod #mod_ident {
            use super::*;
            pub const ELEM_SLOTS: usize = <#struct_type as ::base_execution_evm_precompiles::StorableType>::SLOTS;
            pub const ARRAY_LEN: usize = #array_size;
            pub const SLOT_COUNT: usize = ARRAY_LEN * ELEM_SLOTS;
        }

        impl ::base_execution_evm_precompiles::StorableType for [#struct_type; #array_size] {
        ...
        impl ::base_execution_evm_precompiles::Storable for [#struct_type; #array_size] {
            #[inline]
            fn load<S: ::base_execution_evm_precompiles::StorageOps>(storage: &S, slot: ::alloy_primitives::U256, ctx: ::base_execution_evm_precompiles::LayoutCtx) -> ::base_execution_evm_precompiles::Result<Self> {
                debug_assert_eq!(ctx, ::base_execution_evm_precompiles::LayoutCtx::FULL, "Struct arrays can only be loaded with LayoutCtx::FULL");
                let base_slot = slot;
                #load_impl
            }
            ...
            #[inline]
            fn store<S: ::base_execution_evm_precompiles::StorageOps>(&self, storage: &mut S, slot: ::alloy_primitives::U256, ctx: ::base_execution_evm_precompiles::LayoutCtx) -> ::base_execution_evm_precompiles::Result<()> {
                debug_assert_eq!(ctx, ::base_execution_evm_precompiles::LayoutCtx::FULL, "Struct arrays can only be stored with LayoutCtx::FULL");
                let base_slot = slot;
                #store_impl
            }
        }
        ...
        for i in 0..#array_size {
            let elem_slot = base_slot.checked_add(
                ::alloy_primitives::U256::from(i).checked_mul(
                    ::alloy_primitives::U256::from(<#struct_type as ::base_execution_evm_precompiles::StorableType>::SLOTS)
                ).ok_or(::base_execution_evm_precompiles::BasePrecompileError::SlotOverflow)?
            ).ok_or(::base_execution_evm_precompiles::BasePrecompileError::SlotOverflow)?;
            result[i] = <#struct_type as ::base_execution_evm_precompiles::Storable>::load(storage, elem_slot, ::base_execution_evm_precompiles::LayoutCtx::FULL)?;
        }
        Ok(result)
    }
}
...
fn gen_struct_array_store(struct_type: &TokenStream) -> TokenStream {
    quote! {
        for (i, elem) in self.iter().enumerate() {
            ...
            <#struct_type as ::base_execution_evm_precompiles::Storable>::store(elem, storage, elem_slot, ::base_execution_evm_precompiles::LayoutCtx::FULL)?;
        }
        Ok(())
    }
}
