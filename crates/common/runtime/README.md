...
```sh
cargo test -p base-common-runtime --features test-utils,time,rayon
```

base-common-runtime provides an async runtime abstraction for deterministic testing of base
...
control over scheduling and time. base-common-runtime follows the same pattern but is scoped
...
- `BatchDriver` (`base-batcher-service`) — uses `runtime.cancelled()` as a shutdown signal
  and `runtime.sleep(drain_timeout)` to bound the drain phase after cancellation.
- `PollingBlockSource` (`base-batcher-service`) — uses `runtime.sleep(poll_interval)`
...
```rust
use base_common_runtime::{TokioRuntime, Cancellation};
...
```rust,ignore
use base_common_runtime::{AsyncRuntime, Clock};
...
```rust,ignore
use base_common_runtime::deterministic::{Config, Runner};
use base_common_runtime::{Clock, Spawner};
use std::time::Duration;
