...
base_common_l1::define_tx_manager_cli!("CUSTOM_PREFIX");
...
fn try_from_default_matches_config_default() {
    let config = base_common_l1::TxManagerConfig::try_from(TxManagerCli::default())
        .expect("default CLI should convert successfully");
    assert_eq!(config, base_common_l1::TxManagerConfig::default());
}
