...
base_common_l1::define_signer_cli!("TEST_SIGNER");
...
fn local_signer_accepts_key_with_and_without_0x() {
    ...
    for key in [prefixed.as_str(), TEST_KEY] {
        ...
        let config = base_common_l1::SignerConfig::try_from(cli.signer).unwrap();
        assert!(matches!(config, base_common_l1::SignerConfig::Local(..)));
    }
}
...
fn remote_signer() {
    ...
    let config = base_common_l1::SignerConfig::try_from(cli.signer).unwrap();
    assert!(matches!(config, base_common_l1::SignerConfig::Remote { .. }));
}

#[test]
fn no_signer_returns_error() {
    let cli = TestCli::try_parse_from(["test"]).unwrap();
    let err = base_common_l1::SignerConfig::try_from(cli.signer).unwrap_err();
    assert!(matches!(err, base_common_l1::ConfigError::InvalidValue { field: "signer", .. }));
}
...
fn invalid_hex_returns_config_error() {
    ...
    let result = base_common_l1::SignerConfig::try_from(cli.signer);
    assert!(matches!(
        result.unwrap_err(),
        base_common_l1::ConfigError::InvalidValue { field: "private-key", .. }
    ));
}
...
fn endpoint_without_host_returns_config_error() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        base_common_l1::ConfigError::InvalidValue { field: "signer-endpoint", .. }
    ));
}
