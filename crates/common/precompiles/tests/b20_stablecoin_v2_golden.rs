...
//! Because every other op's logic and storage layout are identical to V1, and storage is
//! append-only, this suite reuses V1's pinned roots and gas-footprint counts verbatim for every
//! behavior-preserving op — following the precedent set by `b20_policy_v2_golden.rs` and
//! `b20_asset_v2_golden.rs` — while still locking V2's behavior independently: a future edit to
//! V2 that changes state, events, or gas must re-bless these roots, and can never silently
//! diverge under V1's frozen pins. `seizeWithMemo` gets its own fresh roots.
//!
...
//! `B20StablecoinStorage` over `HashMapStorageProvider`, with an `FakePolicyAccounting` for
//! deterministic allow/block decisions. Each case asserts:
...
//
// Reused verbatim from `b20_stablecoin_v1_golden.rs`: every op below carries V1's unmodified
// body at V2, and storage is append-only, so dispatching through V2 at Cobalt yields the same
// snapshot as V1.
