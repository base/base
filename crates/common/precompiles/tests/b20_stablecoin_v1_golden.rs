...
fn domain_separator(storage: &mut HashMapStorageProvider) -> B256 {
    StorageCtx::enter(storage, |ctx| {
        let token = StablecoinContractContext::with_storage_and_policy(
            B20StablecoinStorage::from_address(TOKEN, ctx),
...
fn resolver_maps_forks_to_versions() {
    assert_eq!(StablecoinVersionResolver::from_base_upgrade(BaseUpgrade::Azul), None);
    assert_eq!(
        StablecoinVersionResolver::from_base_upgrade(BaseUpgrade::Beryl),
        Some(StablecoinVersion::V1)
    );
    assert_eq!(
        StablecoinVersionResolver::from_base_upgrade(BaseUpgrade::Cobalt),
        Some(StablecoinVersion::V1)
    );
}
