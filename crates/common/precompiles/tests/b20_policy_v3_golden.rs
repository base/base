...
//! `BaseUpgrade::Denim` -> `PolicyVersion::V3`) over `HashMapStorageProvider` configured with
//! `StorageFeatures::Cobalt` (the production storage config at Denim).
//! Each case asserts:
...
//!    --test b20_policy_v3_golden -- --nocapture` and copy the printed `GOLDEN_ROOT` values.
...
fn call_policy(
    ...
    StorageCtx::enter(storage, |ctx| {
        PolicyRegistryStorage::new(ctx).dispatch(ctx, &calldata, BaseUpgrade::Denim)
    })
