...
//! storage config at Denim), with a `FakePolicyAccounting` for deterministic allow/block
...
//!    --test b20_asset_v3_golden -- --nocapture` and copy the printed `GOLDEN_ROOT` values.
...
/// Drives one op through the resolver-gated (`Denim` -> V3) unprivileged path.
fn op(
    ...
    StorageCtx::enter(storage, |ctx| {
        let version =
            AssetVersions::from_base_upgrade(BaseUpgrade::Denim).expect("Denim activates V3");
...
/// Drives one op through V3 with factory-init privilege (guards skipped).
fn op_privileged(
    storage: &mut HashMapStorageProvider,
    caller: Address,
    policy: FakePolicyAccounting,
    ...
    StorageCtx::enter(storage, |ctx| {
        ...
        .route(ctx, &calldata, AssetVersion::V3, true, NoopPrecompileCallObserver)
    })
}
...
/// Returns an uninitialized Stablecoin address in the structural B-20 range.
fn b20_prefix_address() -> Address {
    B20Variant::Stablecoin.compute_address(ALICE, B256::repeat_byte(0x42)).0
}
...
#[test]
fn resolver_resolves_v3_from_denim() {
...
fn golden_dispatch_no_observer_wrapper_reverts_uninitialized() {
    ...
    let out = StorageCtx::enter(&mut s, |ctx| {
        ...
        .dispatch(ctx, &calldata, BaseUpgrade::Denim)
    })
    .expect("dispatch must not fatally error");
    assert!(out.is_revert());
}
...
#[test]
fn golden_batch_mint_reverts_b20_prefix_recipient() {
    let mut s = fresh();
    let receiver = b20_prefix_address();
    seed(&mut s, |t| give_role(t, B20TokenRole::Mint.id(), ALICE));
    let mut policy = FakePolicyAccounting::new();
    ...
    let err = op(
        ...
        ALICE,
        policy,
        IB20Asset::batchMintCall { recipients: vec![BOB, receiver], amounts: vec![u(1), u(1)] }
            .abi_encode(),
    )
    .unwrap_err();
    assert_eq!(err, BasePrecompileError::revert(IB20::InvalidReceiver { receiver }));
...
/// V3/Denim (inherited from V2/Cobalt) with a cheap, calldata-size-independent rejection instead
/// of paying the owned decoder's O(aliases · `tail_bytes`) diagnostic construction. Contrast with
...
#[test]
fn golden_announce_malformed_id_short_circuits_at_v3() {
    let mut s = fresh();
...
fn v2_op_coverage_checklist(call: IB20::IB20Calls, ext: IB20Asset::IB20AssetCalls) {
    ...
    match ext {
        ...
        SC::batchMint(_) => covered(&[
            ...
            golden_batch_mint_reverts_b20_prefix_recipient,
