//! Golden tests pinning Stablecoin **V3** behavior of the B-20 precompile.
//!
...
//! (`BaseUpgrade::Denim` -> `StablecoinVersion::V3`) against the real EVM-backed
//! `B20StablecoinStorage` over `HashMapStorageProvider` configured with `StorageFeatures::Cobalt`
//! (the production storage config at Denim), with a `FakePolicyAccounting` for deterministic
...
//!    --test b20_stablecoin_v3_golden -- --nocapture` and copy the printed `GOLDEN_ROOT` values.
...
const LOGIC: StablecoinV3 = StablecoinV3;
...
// V1 suite runs Legacy, so an op that triggers Cobalt dynamic tail cleanup (name/symbol/contract-URI)
// can diverge from its V1 pin. Re-blessing reflects the true Denim snapshot for each.
...
fn op(
    ...
    StorageCtx::enter(storage, |ctx| {
        let version =
            StablecoinVersions::from_base_upgrade(BaseUpgrade::Denim).expect("Denim activates V3");
...
fn op_privileged(
    ...
    StorageCtx::enter(storage, |ctx| {
        ...
        .route(ctx, &calldata, StablecoinVersion::V3, true, NoopPrecompileCallObserver)
    })
}
...
#[test]
fn resolver_activates_v3_from_denim() {
    assert_eq!(
