...
impl<S: BasePrecompileSpec> BasePrecompiles<S> {
    ...
    pub fn new_with_spec(spec: S) -> Self {
        let precompiles = match spec.upgrade() {
            ...
            // Denim, Everest, and Zenith introduce no new precompiles, so they inherit the Cobalt mapping
            ...
            | BaseUpgrade::Denim
            | BaseUpgrade::Everest
            | BaseUpgrade::Zenith => Self::cobalt(),
...
mod tests {
    ...
    use crate::{
        ...
        NonceManagerStorage, PolicyRegistryStorage, bls12_381, bn254_pair,
    };
    ...
    // Static table only. Factory, lookup, registries, and nonce manager are
    // registered later by install() / install_with_observer().
    #[rstest]
