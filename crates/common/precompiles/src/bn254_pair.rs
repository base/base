...
mod tests {
    ...
    fn test_bn254_pair_granite() {
      ...
      ",
        ...
        .unwrap();
        assert!(run_pair_granite(&bad_input, 260_000).is_err());

        // Input length longer than 112687
        let over_limit = vec![1u8; 587 * bn254::PAIR_ELEMENT_LEN];
        assert!(matches!(run_pair_granite(&over_limit, 260_000), Err(PrecompileError::Fatal(_))));
    }
    ...
    fn test_bn254_pair_jovian_bad_input_len() {
        ...
        assert!(matches!(run_pair_jovian(&input, u64::MAX), Err(PrecompileError::Fatal(_))));
    }
}
