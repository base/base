//! Version 3 of the `PolicyRegistry` precompile logic, activated at Denim.
...
impl<S: PolicyAccounting> PolicyRegistryLogic<S> for PolicyRegistryV3 {
    fn create_policy(
        &self,
        storage: &mut S,
        admin: Address,
...
mod tests {
    ...
    const LOGIC: PolicyRegistryV3 = PolicyRegistryV3;
    ...
    fn always_allow_id_authorizes_any_account() {
        let rt = initialized();
        assert!(is_authorized(&rt, PolicyRegistryV3::ALWAYS_ALLOW_ID, ALICE));
        assert!(is_authorized(&rt, PolicyRegistryV3::ALWAYS_ALLOW_ID, BOB));
    }

    #[test]
    fn always_block_id_rejects_any_account() {
        let rt = initialized();
        assert!(!is_authorized(&rt, PolicyRegistryV3::ALWAYS_BLOCK_ID, ALICE));
        assert!(!is_authorized(&rt, PolicyRegistryV3::ALWAYS_BLOCK_ID, BOB));
    }
    ...
    fn unknown_allowlist_policy_id_does_not_authorize_account() {
        let unknown_allowlist = PolicyRegistryV3::make_id(PolicyType::ALLOWLIST as u8, 9999);
        let rt = initialized();
    ...
    fn first_create_policy_initializes_builtins_and_starts_counter_at_two() {
        ...
        assert_eq!(id & PolicyRegistryV3::COUNTER_MASK, 2);
        assert!(LOGIC.policy_exists(&rt, PolicyRegistryV3::ALWAYS_ALLOW_ID).unwrap());
        assert!(LOGIC.policy_exists(&rt, PolicyRegistryV3::ALWAYS_BLOCK_ID).unwrap());
    ...
    fn ensure_initialized_and_get_counter_is_idempotent() {
        ...
        assert_eq!(rt.next_counter, PolicyRegistryV3::BUILTIN_POLICY_COUNT);
    }
    ...
    fn create_policy_ids_encode_type_in_top_byte_and_increment_counter() {
        ...
        assert_eq!(id1 & PolicyRegistryV3::COUNTER_MASK, 2);
        assert_eq!(id2 & PolicyRegistryV3::COUNTER_MASK, 3);
    }

    #[test]
    fn create_policy_at_counter_mask_reverts_with_under_overflow() {
        let mut rt = initialized();
        rt.next_counter = PolicyRegistryV3::COUNTER_MASK;
    ...
    fn create_policy_at_counter_mask_minus_one_consumes_last_slot_then_reverts() {
        let mut rt = initialized();
        rt.next_counter = PolicyRegistryV3::COUNTER_MASK - 1;
        let id = LOGIC.create_policy(&mut rt, ADMIN, PolicyType::ALLOWLIST).unwrap();
        assert_eq!(id & PolicyRegistryV3::COUNTER_MASK, PolicyRegistryV3::COUNTER_MASK - 1);
        let err = LOGIC.create_policy(&mut rt, ADMIN, PolicyType::ALLOWLIST).unwrap_err();
        assert_eq!(err, BasePrecompileError::under_overflow());
    }
    ...
    fn update_allowlist_too_many_accounts_reverts() {
        let mut rt = initialized();
        let id = create_allowlist(&mut rt);
        let accounts = many_accounts(PolicyRegistryV3::MAX_ACCOUNTS_PER_BATCH + 1);
        ...
        assert_eq!(
            ...
            BasePrecompileError::revert(IPolicyRegistry::BatchSizeTooLarge {
                maxBatchSize: U256::from(PolicyRegistryV3::MAX_ACCOUNTS_PER_BATCH),
            })
        );
    }
    ...
    fn update_allowlist_max_batch_size_succeeds() {
        let mut rt = initialized();
        let id = create_allowlist(&mut rt);
        let accounts = many_accounts(PolicyRegistryV3::MAX_ACCOUNTS_PER_BATCH);
    ...
    fn builtin_policies_reject_admin_mutations() {
        let mut rt = initialized();
        for policy_id in [PolicyRegistryV3::ALWAYS_ALLOW_ID, PolicyRegistryV3::ALWAYS_BLOCK_ID] {
    ...
    fn get_policy_admin_builtin_ids_return_zero_address() {
        let rt = initialized();
        assert_eq!(
            LOGIC.get_policy_admin(&rt, PolicyRegistryV3::ALWAYS_ALLOW_ID).unwrap(),
            Address::ZERO
        );
        assert_eq!(
            LOGIC.get_policy_admin(&rt, PolicyRegistryV3::ALWAYS_BLOCK_ID).unwrap(),
            Address::ZERO
        );
    }
    ...
    fn pending_policy_admin_builtin_ids_return_zero_address() {
        let rt = initialized();
        assert_eq!(
            LOGIC.pending_policy_admin(&rt, PolicyRegistryV3::ALWAYS_ALLOW_ID).unwrap(),
            Address::ZERO
        );
        assert_eq!(
            LOGIC.pending_policy_admin(&rt, PolicyRegistryV3::ALWAYS_BLOCK_ID).unwrap(),
            Address::ZERO
        );
    }
    ...
    fn pending_policy_admin_counter_one_blocklist_reads_staged_slot() {
        ...
        let counter_one_blocklist = PolicyRegistryV3::make_id(PolicyType::BLOCKLIST as u8, 1);
        assert_ne!(counter_one_blocklist, PolicyRegistryV3::ALWAYS_BLOCK_ID);
        let mut rt = initialized();
    ...
    fn pending_policy_admin_nonexistent_well_formed_policy_returns_zero_address() {
        let rt = initialized();
        let nonexistent = PolicyRegistryV3::make_id(0, 999);
    ...
    fn composite_policy_child_ids_returns_empty_for_non_composites() {
        ...
        let malformed = PolicyRegistryV3::make_id(9, 1);
        let uncreated_union = PolicyRegistryV3::make_id(PolicyType::UNION as u8, 999);

        for policy_id in [
            simple,
            PolicyRegistryV3::ALWAYS_ALLOW_ID,
            PolicyRegistryV3::ALWAYS_BLOCK_ID,
            malformed,
            uncreated_union,
        ] {
            assert!(
    ...
    fn create_composite_nonexistent_child_reverts_policy_not_found() {
        let mut rt = initialized();
        let a = create_allowlist(&mut rt);
        let missing = PolicyRegistryV3::make_id(PolicyType::ALLOWLIST as u8, 999);
        let err = LOGIC
    ...
    fn create_composite_builtin_child_reverts_invalid_child() {
        ...
        let err = LOGIC
            .create_composite_policy(
                ...
                ADMIN,
                PolicyType::UNION,
                vec![a, PolicyRegistryV3::ALWAYS_ALLOW_ID],
            )
            .unwrap_err();
        assert_eq!(
            err,
            BasePrecompileError::revert(IPolicyRegistry::InvalidChildPolicy {
                childPolicyId: PolicyRegistryV3::ALWAYS_ALLOW_ID,
            })
        );
    }
    ...
    fn create_composite_nonexistent_child_precedes_invalid_child() {
        ...
        let err = LOGIC
            .create_composite_policy(
                ...
                ADMIN,
                PolicyType::UNION,
                vec![PolicyRegistryV3::ALWAYS_ALLOW_ID, missing],
            )
            .unwrap_err();
        assert_eq!(err, BasePrecompileError::revert(IPolicyRegistry::PolicyNotFound {}));
    }
    ...
    fn update_composite_nonexistent_reverts_policy_not_found() {
        ...
        let a = create_allowlist(&mut rt);
        let b = create_allowlist(&mut rt);
        let missing = PolicyRegistryV3::make_id(PolicyType::UNION as u8, 999);
    ...
    fn update_composite_invalid_child_reverts() {
        ...
        set_caller(&mut rt, ADMIN);
        let err = LOGIC
            .update_composite(&mut rt, id, vec![a, PolicyRegistryV3::ALWAYS_BLOCK_ID])
            .unwrap_err();
        assert_eq!(
            err,
            BasePrecompileError::revert(IPolicyRegistry::InvalidChildPolicy {
