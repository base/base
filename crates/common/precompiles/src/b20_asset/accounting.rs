...
pub trait AssetAccounting: TokenAccounting {
    ...
    /// Returns the extra metadata value for `identifier_type`, or an empty string if unset.
    fn extra_metadata(&self, identifier_type: &str) -> Result<String>;
    /// Writes (or removes when `value` is empty) the extra metadata for `identifier_type`.
