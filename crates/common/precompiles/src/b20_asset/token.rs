...
use crate::{
    ...
    Configurable,
    IB20::{self},
    IB20Asset, Mintable, Pausable, Permittable, Policy, RoleManaged, Token, Transferable, Version,
};
...
pub struct B20AssetToken<S: AssetAccounting, P: Policy> {
    accounting: S,
    policy: P,
    /// The fork-versioned behavior generation this token instance runs. Resolved once (from the
    /// fork captured at precompile install) and read by the versioned dispatch arms. It is a plain
    /// value, not a type parameter — see `b20_asset/logic.rs`.
    version: Version,
}
...
impl<S: AssetAccounting, P: Policy> B20AssetToken<S, P> {
    ...
    /// Creates a `B20AssetToken` running the genesis behavior ([`Version::V1`], today's behavior).
    ///
    /// Used by direct-trait-method tests and any caller that does not resolve a fork.
    pub const fn with_storage_and_policy(accounting: S, policy: P) -> Self {
        Self::with_storage_and_policy_versioned(accounting, policy, Version::V1)
    }

    /// Creates a `B20AssetToken` running the behavior generation `version` (resolved from the
    /// active fork by [`crate::for_upgrade`]).
    pub const fn with_storage_and_policy_versioned(
        accounting: S,
        policy: P,
        version: Version,
    ) -> Self {
        Self { accounting, policy, version }
    }

    /// The fork-versioned behavior generation this token runs.
