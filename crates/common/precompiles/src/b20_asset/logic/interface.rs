...
use crate::{
    AssetAccounting, AssetContractContext, Eip712Domain, IB20, PermitArgs, PolicyAccounting, Token,
};
...
pub trait B20AssetLogic<S: AssetAccounting, A: PolicyAccounting> {
    ...
    fn is_initialized(&self, ctx: &AssetContractContext<S, A>) -> Result<bool> {
        ctx.accounting().is_initialized()
    }

    /// Returns the token name.
    fn name(&self, ctx: &AssetContractContext<S, A>) -> Result<String> {
        ctx.accounting().name()
    }

    /// Returns the token symbol.
    fn symbol(&self, ctx: &AssetContractContext<S, A>) -> Result<String> {
        ctx.accounting().symbol()
    }

    /// Returns the custom decimal precision configured for this asset ctx.
    fn decimals(&self, ctx: &AssetContractContext<S, A>) -> Result<u8> {
        AssetAccounting::decimals(ctx.accounting())
    }

    /// Returns the total token supply currently in circulation.
    fn total_supply(&self, ctx: &AssetContractContext<S, A>) -> Result<U256> {
        ctx.accounting().total_supply()
    }

    /// Returns the token balance of `account`.
    fn balance_of(&self, ctx: &AssetContractContext<S, A>, account: Address) -> Result<U256> {
    ...
    fn supply_cap(&self, ctx: &AssetContractContext<S, A>) -> Result<U256> {
        ctx.accounting().supply_cap()
    }

    /// Returns the current EIP-2612 permit nonce for `owner`.
    fn nonce(&self, ctx: &AssetContractContext<S, A>, owner: Address) -> Result<U256> {
        ctx.accounting().nonce(owner)
    }

    /// Returns the off-chain metadata URI for this token (ERC-7572).
    fn contract_uri(&self, ctx: &AssetContractContext<S, A>) -> Result<String> {
    ...
    fn role_admin(&self, ctx: &AssetContractContext<S, A>, role: B256) -> Result<B256> {
        ctx.accounting().role_admin(role)
    }

    /// Returns the current multiplier, scaled to WAD.
    fn multiplier(&self, ctx: &AssetContractContext<S, A>) -> Result<U256> {
        ctx.accounting().multiplier()
    }

    /// Returns the extra-metadata value for `key`, or the empty string if unset.
    fn extra_metadata(&self, ctx: &AssetContractContext<S, A>, key: &str) -> Result<String> {
        ctx.accounting().extra_metadata(key)
    }

    /// Returns whether announcement `id` has already been consumed.
    fn is_announcement_id_used(&self, ctx: &AssetContractContext<S, A>, id: &str) -> Result<bool> {
    ...
    fn scaled_balance_of(&self, ctx: &AssetContractContext<S, A>, account: Address)
    -> Result<U256>;
