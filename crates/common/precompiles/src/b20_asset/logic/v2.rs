...
use alloy_primitives::{Address, B256, FixedBytes, U256, U512, b256, keccak256};
...
impl AssetV2 {
    ...
    /// for balance-derived reads (`balanceOfUI`, `totalSupplyUI`). Caller-supplied conversion
    /// inputs to `toUIAmount` / `fromUIAmount` are not bounded by the supply cap, so those
    /// helpers widen the intermediate product to `U512` and revert only when the final
    /// quotient exceeds `U256`.
...
mod tests {
    ...
    fn to_scaled_balance_succeeds_when_intermediate_product_overflows_u256() {
        ...
        let expected = U256::from(1u64) << 201;
        assert_eq!(LOGIC.to_scaled_balance(&tok, balance).unwrap(), expected);
    }
    ...
    fn to_scaled_balance_reverts_only_on_genuine_quotient_overflow() {
        ...
        assert_eq!(
            LOGIC.to_scaled_balance(&tok, U256::MAX).unwrap_err(),
    ...
    fn to_raw_balance_succeeds_when_intermediate_product_overflows_u256() {
        ...
        tok.accounting_mut().set_multiplier(B20AssetStorage::WAD * U256::from(2u64)).unwrap();
        let balance = U256::from(1u64) << 200;
        let expected = U256::from(1u64) << 199;
        assert_eq!(LOGIC.to_raw_balance(&tok, balance).unwrap(), expected);
    }
    ...
    fn to_raw_balance_reverts_only_on_genuine_quotient_overflow() {
        ...
        assert_eq!(
            LOGIC.to_raw_balance(&tok, U256::MAX).unwrap_err(),
            BasePrecompileError::under_overflow()
        );
    }
    ...
    fn total_supply_ui_succeeds_when_intermediate_product_overflows_u256() {
        ...
        let expected = U256::from(1u64) << 201;
        assert_eq!(LOGIC.total_supply_ui(&tok).unwrap(), expected);
    }
