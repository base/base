...
impl B20AssetPrecompile {
    /// Returns a [`DynPrecompile`] that dispatches to asset B-20 logic at
    ...
    /// Returns a [`DynPrecompile`] that observes and dispatches to asset B-20 logic at
    /// `token_address`, gated to the version active at `upgrade`.
    ...
    {
        base_precompile!(alloc::format!("B20Asset@{token_address}"), |ctx, calldata| {
