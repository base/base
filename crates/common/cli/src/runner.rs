...
use base_common_runtime::{PanickedTaskError, TaskExecutor};
...
impl CliRunner {
    ...
    pub fn try_default_runtime() -> Result<Self, base_common_runtime::RuntimeBuildError> {
        Self::try_with_runtime_config(base_common_runtime::RuntimeConfig::default())
    }

    /// Creates a new [`CliRunner`] with the given [`RuntimeConfig`](base_common_runtime::RuntimeConfig).
    ...
    /// Returns a clone of the underlying [`Runtime`](base_common_runtime::Runtime).
    pub fn runtime(&self) -> base_common_runtime::Runtime {
        self.runtime.clone()
    }
    ...
    where
        ...
        E: Send
            ...
            + std::fmt::Display
            + From<std::io::Error>
            + From<base_common_runtime::PanickedTaskError>
            + 'static,
    {
...
where
    F: Future<Output = Result<(), E>>,
    E: Send + Sync + From<base_common_runtime::PanickedTaskError> + 'static,
{
    let fut = pin!(fut);
    tokio::select! {
...
/// Shut down the given [`Runtime`](base_common_runtime::Runtime), and wait for it if `wait` is set.
///
...
pub fn runtime_shutdown(rt: base_common_runtime::Runtime, wait: bool) {
