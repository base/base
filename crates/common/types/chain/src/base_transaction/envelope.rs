...
impl BaseTxEnvelope {
    ...
    /// `From<BasePooledTransaction> for base_common_types_chain::PooledTransaction` from panicking.
    pub fn try_into_eth_pooled(
        self,
    ) -> Result<base_common_types_chain::transaction::PooledTransaction, ValueError<Self>> {
        match self {
...
#[cfg(feature = "k256")]
impl base_common_types_chain::transaction::SignerRecoverable for BaseTxEnvelope {
    fn recover_signer(
        &self,
    ) -> Result<alloy_primitives::Address, base_common_types_chain::crypto::RecoveryError> {
        ...
        base_common_types_chain::crypto::secp256k1::recover_signer(signature, signature_hash)
    }
    ...
    fn recover_signer_unchecked(
        ...
        base_common_types_chain::crypto::secp256k1::recover_signer_unchecked(
            signature,
            signature_hash,
        )
    }
    ...
    fn recover_unchecked_with_buf(
        ...
        match self {
            Self::Legacy(tx) => {
                base_common_types_chain::transaction::SignerRecoverable::recover_unchecked_with_buf(
                    tx, buf,
                )
            }
            Self::Eip2930(tx) => {
...
mod tests {
    ...
    fn eip8130_compact_roundtrip() {
        use base_common_types_chain::Typed2718;
