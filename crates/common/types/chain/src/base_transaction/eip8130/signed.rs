...
use base_common_types_chain::{InMemorySize, Transaction, Typed2718};
...
impl Eip8130Signed {
    ...
    pub fn recover_eoa_sender(
        &self,
    ) -> Result<Option<Address>, base_common_types_chain::crypto::RecoveryError> {
        self.recover_eoa_sender_with(base_common_types_chain::crypto::secp256k1::recover_signer)
    }
    ...
    pub fn recover_sender(
        ...
        self.recover_eoa_sender()?.ok_or_else(base_common_types_chain::crypto::RecoveryError::new)
    }
    ...
    pub fn recover_sender_unchecked(
        ...
        self.recover_eoa_sender_unchecked()?
            .ok_or_else(base_common_types_chain::crypto::RecoveryError::new)
    }
    ...
    fn recover_eoa_sender_with(
        ...
        if raw.len() != 65 || !matches!(raw[64], 27 | 28) {
            return Err(base_common_types_chain::crypto::RecoveryError::new());
        }
        let signature = alloy_primitives::Signature::try_from(raw)
            .map_err(|_| base_common_types_chain::crypto::RecoveryError::new())?;
