...
use base_common_types_chain::EthereumReceipt as AlloyEthereumReceipt;
use bytes::Buf;
...
impl<T: Compact> Compact for AlloyEthereumReceipt<T> {
        ...
        B: bytes::BufMut + AsMut<[u8]>,
    {
        ...
        buf.put_slice(&flags);
        if zstd {
            crate::StorageCodec::with_receipt_compressor(|compressor| {
    ...
    fn from_compact(buf: &[u8], _len: usize) -> (Self, &[u8]) {
        ...
        if flags.__zstd() != 0 {
            crate::StorageCodec::with_receipt_decompressor(|decompressor| {
