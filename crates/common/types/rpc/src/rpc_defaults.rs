...
/// Default resource limits for RPC servers.
#[derive(Debug)]
pub struct RpcServerDefaults;

#[cfg(feature = "std")]
impl RpcServerDefaults {
    ...
    pub fn max_tracing_requests() -> usize {
        // We reserve 2 cores for the rest of the system
        const RESERVED: usize = 2;

        std::thread::available_parallelism()
            .map_or(25, |cpus| core::cmp::max(cpus.get().saturating_sub(RESERVED), RESERVED))
    }
}
