...
pub struct TxEip8130 {
    ...
    /// The bound is exclusive — the transaction is invalid when
    /// `block.timestamp * 1000 >= valid_before` (valid only strictly before it);
    /// `0` means no expiry, and it MUST be non-zero for nonce-free
    /// (`nonce_key == NONCE_KEY_MAX`) transactions.
