...
use crate::{
    ...
    base_transaction::{BaseTransactionInfo, DepositInfo, Eip8130Signed, TxEip8130},
};
...
#[envelope(alloy_consensus = crate, tx_type_name = OpTxType, typed = BaseTypedTransaction, serde_cfg(feature = "serde"), arbitrary_cfg(feature = "arbitrary"))]
...
impl BaseTxEnvelope {
    ...
    /// `From<BasePooledTransaction> for base_common_consensus::PooledTransaction` from panicking.
    pub fn try_into_eth_pooled(
        self,
    ) -> Result<base_common_consensus::transaction::PooledTransaction, ValueError<Self>> {
        match self {
...
#[cfg(feature = "k256")]
impl base_common_consensus::transaction::SignerRecoverable for BaseTxEnvelope {
    fn recover_signer(
        &self,
    ) -> Result<alloy_primitives::Address, base_common_consensus::crypto::RecoveryError> {
        ...
        base_common_consensus::crypto::secp256k1::recover_signer(signature, signature_hash)
    }
    ...
    fn recover_signer_unchecked(
        ...
        base_common_consensus::crypto::secp256k1::recover_signer_unchecked(
            signature,
            signature_hash,
        )
    }
    ...
    fn recover_unchecked_with_buf(
        ...
        match self {
            Self::Legacy(tx) => {
                base_common_consensus::transaction::SignerRecoverable::recover_unchecked_with_buf(
                    tx, buf,
                )
            }
            Self::Eip2930(tx) => {
...
pub(super) mod serde_bincode_compat {
    ...
    use crate::{base_transaction::Eip8130Signed, serde_bincode_compat::TxDeposit};
...
mod tests {
    ...
    use base_common_consensus::{SignableTransaction, Transaction};
    ...
    fn eip8130_compact_roundtrip() {
        use base_common_consensus::Typed2718;
