...
use base_common_consensus::{InMemorySize, Transaction, Typed2718};

use crate::base_transaction::eip8130::{constants::Eip8130Constants, tx::TxEip8130};
...
impl Eip8130Signed {
    ...
    pub fn recover_eoa_sender(
        &self,
    ) -> Result<Option<Address>, base_common_consensus::crypto::RecoveryError> {
        self.recover_eoa_sender_with(base_common_consensus::crypto::secp256k1::recover_signer)
    }
    ...
    #[cfg(feature = "k256")]
    pub fn recover_sender(&self) -> Result<Address, base_common_consensus::crypto::RecoveryError> {
        if let Some(addr) = self.explicit_sender() {
            return Ok(addr);
        }
        self.recover_eoa_sender()?.ok_or_else(base_common_consensus::crypto::RecoveryError::new)
    }
    ...
    pub fn recover_sender_unchecked(
        &self,
    ) -> Result<Address, base_common_consensus::crypto::RecoveryError> {
        if let Some(addr) = self.explicit_sender() {
            return Ok(addr);
        }
        self.recover_eoa_sender_unchecked()?
            .ok_or_else(base_common_consensus::crypto::RecoveryError::new)
    }
    ...
    fn recover_eoa_sender_with(
        ...
        if raw.len() != 65 || !matches!(raw[64], 27 | 28) {
            return Err(base_common_consensus::crypto::RecoveryError::new());
        }
        let signature = alloy_primitives::Signature::try_from(raw)
            .map_err(|_| base_common_consensus::crypto::RecoveryError::new())?;
...
mod tests {
    ...
    use super::*;
    use crate::base_transaction::eip8130::{
