...
use super::{
    BlockLogs, BlockLogsFut, CanonicalEvent, WatchLogsFrom, WatchLogsFromStream,
    watch_canonical_blocks_from::FixedBuf,
};
...
fn block_log_event<N: Network>(
    ...
    if removed { CanonicalEvent::Removed(block_logs) } else { CanonicalEvent::Added(block_logs) }
}
...
mod tests {
    ...
    async fn next_event(
        stream: &mut WatchCanonicalLogsFromStream<crate::Ethereum>,
    ) -> CanonicalEvent<BlockLogs<crate::Ethereum>> {
        timeout(Duration::from_secs(1), stream.next()).await.unwrap().unwrap().unwrap()
    }

    fn assert_added(
        event: CanonicalEvent<BlockLogs<crate::Ethereum>>,
        number: u64,
        hash_last_byte: u8,
        log_count: usize,
    ) {
