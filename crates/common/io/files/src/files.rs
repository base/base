...
/// File operations with path-aware errors.
...
impl Files {
    ...
    pub fn remove_file_if_exists(path: impl AsRef<Path>) -> Result<()> {
        match Self::remove_file(&path) {
            Err(FsPathError::RemoveFile { source, .. })
        ...
        E: Into<Box<dyn core::error::Error + Send + Sync>>,
    {
        ...
        Self::rename(&tmp_path, file_path)?;
...
mod tests {
    ...
    use super::{Files, FsPathError};

    #[test]
    fn atomic_write_replaces_destination() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("state.json");
        Files::write(&path, b"old").unwrap();
        Files::atomic_write_file(&path, |file| file.write_all(b"new")).unwrap();
        assert_eq!(Files::read(&path).unwrap(), b"new");
        assert!(!path.with_extension("tmp").exists());
    }

    #[test]
    fn failed_atomic_write_preserves_destination() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("state.json");
        Files::write(&path, b"old").unwrap();
        let result = Files::atomic_write_file(&path, |file| {
            file.write_all(b"partial")?;
            Err::<(), _>(io::Error::other("write failed"))
        });
        assert!(matches!(result, Err(FsPathError::Write { .. })));
        assert_eq!(Files::read(&path).unwrap(), b"old");
        assert!(!path.with_extension("tmp").exists());
    }

    #[test]
    fn missing_read_retains_path_and_cause() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("missing");
        let error = Files::read(&path).unwrap_err();
        match error {
            FsPathError::Read { path: failed_path, source } => {
                assert_eq!(failed_path, path);
                assert_eq!(source.kind(), io::ErrorKind::NotFound);
            }
            error => panic!("unexpected error: {error}"),
        }
        Files::remove_file_if_exists(&path).unwrap();
    }
}
