...
impl BaseExecutionPayloadV4 {
    /// Converts [`ExecutionPayloadV3`] to [`BaseExecutionPayloadV4`] using the given L2 withdrawals
    /// root.
    ///
    ...
    /// Converts [`BaseExecutionPayloadV4`] to [`Block`] with raw transactions.
    ///
    ...
    /// Converts [`BaseExecutionPayloadV4`] to [`Block`].
    ///
    /// This performs the same conversion as the underlying V3 payload, but inserts the L2
    /// withdrawals root.
    ///
    ...
    /// Converts [`BaseExecutionPayloadV4`] to [`Block`] with a custom transaction mapper.
    ///
    /// This performs the same conversion as the underlying V3 payload, but inserts the L2
    /// withdrawals root.
    ///
...
#[cfg(feature = "std")]
impl ssz::Encode for BaseExecutionPayloadV4 {
    fn is_ssz_fixed_len() -> bool {
        false
    }
