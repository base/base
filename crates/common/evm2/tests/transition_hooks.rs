...
//!
//! Validates the Canyon create2-deployer, Denim `BaseTime`, and Everest EIP-8130 system-account
...
//! reference (`base-common-evm`'s `canyon`/`everest`/`base_time` modules) is in the shape of the
...
#[test]
fn everest_plants_stub_on_codeless_nonce_manager() {
    let mut executor = executor(BaseUpgrade::Everest, 1000, db_with_valid_proxy());
    executor.apply_transition_hooks(&schedule(BaseUpgrade::Everest, 1000)).expect("hooks apply");
    let (mut evm, _, _) = executor.finish();
...
#[test]
fn everest_does_not_overwrite_existing_code() {
    ...
    let mut executor = executor(BaseUpgrade::Everest, 1000, db);
    executor.apply_transition_hooks(&schedule(BaseUpgrade::Everest, 1000)).expect("hooks apply");
    let (mut evm, _, _) = executor.finish();
...
#[test]
fn everest_is_skipped_before_activation() {
    let mut executor = executor(BaseUpgrade::Isthmus, 1000, db_with_valid_proxy());
    // Everest is scheduled far in the future; at ts=1000 it is inactive.
    executor.apply_transition_hooks(&schedule(BaseUpgrade::Everest, 10_000)).expect("hooks apply");
    let (mut evm, _, _) = executor.finish();

    assert_eq!(code_hash(&mut evm, NONCE_MANAGER), KECCAK256_EMPTY, "no stub before Everest");
}
