#!/bin/bash
# Fixed deployment for newer Forge versions
set -e
...
echo "[1/3] Compiling..."
forge build Flat.sol --force --optimize --optimizer-runs 200 2>&1 | grep -v "Warning"
...
elif [ -f "out/SimpleAccountFactory.json" ]; then
    BYTECODE=$(jq -r '.bytecode.object' out/SimpleAccountFactory.json)
else
    # Find the JSON file
    JSON_FILE=$(find . -name "SimpleAccountFactory.json" 2>/dev/null | head -1)
    if [ -n "$JSON_FILE" ]; then
        echo "Found at: $JSON_FILE"
        BYTECODE=$(jq -r '.bytecode.object' "$JSON_FILE")
    fi
fi

if [ -z "$BYTECODE" ] || [ "$BYTECODE" == "null" ]; then
    echo "❌ Could not extract bytecode"
    echo "Looking for files..."
    find . -name "*.json" -path "*/out/*" 2>/dev/null || echo "No JSON files found"
