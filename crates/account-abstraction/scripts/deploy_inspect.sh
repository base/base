#!/bin/bash
# Deploy using forge inspect to get bytecode directly
set -e
...
echo "[1/2] Getting bytecode with forge inspect..."

# Use forge inspect to get bytecode directly
BYTECODE=$(forge inspect Deploy.sol:SimpleAccountFactory bytecode --optimize --optimizer-runs 200 2>/dev/null)

if [ -z "$BYTECODE" ] || [ "$BYTECODE" == "null" ]; then
    echo "❌ Could not get bytecode"
    rm -f Deploy.sol
    exit 1
fi
...
echo "Factory: $FACTORY_ADDRESS"
echo "Wallet:  $WALLET"
echo "Saved:   deployment-info.json"
echo ""
echo "Next steps:"
echo ""
echo "1. Fund the wallet:"
