...
use base_consensus_batch::{BatchValidationProvider, L2BlockInfo, to_system_config};
use base_consensus_derive::{L2ChainProvider, PipelineError, PipelineErrorKind, ResetError};
use base_execution_state_provider::{
    BlockReaderIdExt, StateProviderFactory, providers::BlockchainProvider,
};
use base_execution_state_types::{HashedStorage, ProviderError};
...
pub enum LocalL2Error {
    ...
    #[error(transparent)]
    BlockInfo(#[from] base_consensus_batch::FromBlockError),
...
mod tests {
    ...
    async fn unset_safety_labels_are_optional_until_consensus_assigns_them() {
        ...
        base_execution_state_operations::init::init_genesis(&factory).unwrap();
