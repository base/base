...
use base_common_types_chain::{AccountChange, Call, Eip8130Signed, TxEip8130};
...
impl SpanBatchEip8130TransactionData {
    ...
    /// Returns the encoded length of the transaction fields.
    ...
    pub fn join_auth(authenticator: &Bytes, proof: &Bytes, configured: bool) -> Bytes {
        if !configured {
            proof.clone()
...
mod tests {
    ...
    use base_common_types_chain::Delegation;
