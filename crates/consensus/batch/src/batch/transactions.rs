...
use base_common_types_chain::{BaseTxEnvelope, OpTxType, Transaction};

use crate::{
    SpanBatchBits, SpanBatchEip8130TransactionData, SpanBatchElement, SpanBatchError,
    SpanBatchTransactionData, SpanDecodingError, SpanTransactionReader,
};
...
impl SpanBatchTransactions {
    ...
    pub fn decode_tx_data(&mut self, r: &mut &[u8]) -> Result<(), SpanBatchError> {
        ...
        // length information.
        for _ in 0..self.total_block_tx_count {
            let (tx_data_item, tx_type) = SpanTransactionReader::read(r)?;
    ...
    pub fn decode_eip8130_proof(r: &mut &[u8]) -> Result<Bytes, SpanBatchError> {
