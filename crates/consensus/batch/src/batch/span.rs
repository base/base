...
use crate::{
    BatchDropReason, BatchValidationProvider, BatchValidity, BlockInfo, L2BlockInfo,
    L2BlockInfoDecoder, SingleBatch, SpanBatchBits, SpanBatchElement, SpanBatchError,
...
impl SpanBatch {
    ...
    pub fn append_singular_batch(
        ...
        let epoch_hash = singular_batch.epoch_hash;
        let parent_hash = singular_batch.parent_hash;
    ...
    pub async fn check_batch<BV: BatchValidationProvider>(
        ...
        for (i, batch) in self.batches.iter().enumerate() {
            ...
            if batch_epoch < l2_safe_head.l1_origin.number {
                warn!(
                    target: "batch_span",
                    batch_epoch, safe_head_epoch = ?l2_safe_head.l1_origin,
            ...
            let Some((offset, l1_origin)) =
                ...
                warn!(
                    target: "batch_span",
                    batch_epoch, batch_timestamp, "unable to find L1 origin for batch"
                );
            ...
            if batch_timestamp < l1_origin.timestamp {
                warn!(
                    target: "batch_span",
                    l2_timestamp = batch_timestamp, l1_timestamp = l1_origin.timestamp, origin = ?l1_origin.id(),
            ...
            if batch_timestamp > l1_origin.timestamp + max_drift {
                if batch.transactions.is_empty() {
                    ...
                    warn!(
                        target: "batch_span",
                        max_time = l1_origin.timestamp + max_drift,
            ...
            for (i, tx) in batch.transactions.iter().enumerate() {
                if tx.is_empty() {
                    warn!(
                        target: "batch_span",
                        tx_index = i, "transaction data must not be empty, but found empty tx"
                    );
                ...
                if tx.as_ref().first() == Some(&(OpTxType::Deposit as u8)) {
                    warn!(
                        target: "batch_span",
                        tx_index = i, "sequencers may not embed any deposits into batch data"
                    );
        ...
        if self.starting_timestamp() < next_timestamp {
            for i in 0..(l2_safe_head.block_info.number - parent_num) {
                ...
                for (j, batch_tx) in batch_txs.iter().enumerate() {
                    let mut buf = Vec::new();
                ...
                let safe_block_ref = match L2BlockInfoDecoder::from_block_and_genesis(
                    &safe_block_payload,
                    &cfg.genesis,
                ) {
                    Ok(r) => r,
                    Err(e) => {
                        warn!(
                            target: "batch_span",
                            hash = %safe_block_payload.header.hash_slow(), error = %e,
                ...
                if safe_block_ref.l1_origin.number != self.batches[i as usize].epoch_num {
                    warn!(
                        safe_origin = safe_block_ref.l1_origin.number,
                        batch_origin = self.batches[i as usize].epoch_num,
    ...
    pub async fn check_batch_prefix<BF: BatchValidationProvider>(
        ...
        if starting_epoch_num == batch_origin.number + 1 {
            if l1_origins.len() < 2 {
                info!(
                    target: "batch_span",
                    epoch = ?epoch.id(), "eager batch cannot advance epoch without more L1 blocks"
                );
                return (BatchValidity::Undecided, None);
            }
        ...
        if !cfg.is_delta_active(batch_origin.timestamp) {
            warn!(
                target: "batch_span",
                origin = ?batch_origin.id(), timestamp = batch_origin.timestamp, "received span batch before Delta upgrade"
            );
        ...
        if self.starting_timestamp() > next_timestamp {
            warn!(
                target: "batch_span",
                timestamp = self.starting_timestamp(), next_timestamp, "received out-of-order batch for future processing"
            );

            // After holocene is activated, gaps are disallowed.
            if cfg.is_holocene_active(inclusion_block.timestamp) {
                return (BatchValidity::Drop(BatchDropReason::FutureTimestamp), None);
            }
    ...
    async fn test_check_batch_overlapping_blocks_tx_mismatch() {
        ...
        let mut fetcher: TestBatchValidator = TestBatchValidator {
            base_blocks: vec![BaseBlock {
                header: Header { number: 9, ..Default::default() },
                body: base_common_types_chain::BlockBody {
                    transactions: vec![base_common_types_chain::BaseTxEnvelope::Eip1559(
    ...
    async fn test_check_batch_with_eip7702_tx() {
        ...
        let second = SpanBatchElement {
            ...
            transactions: vec![Bytes::copy_from_slice(&[
                base_common_types_chain::TxType::Eip7702 as u8
            ])],
        };
        let third =
            SpanBatchElement { epoch_num: 11, timestamp: 20, transactions: vec![filler_bytes] };
