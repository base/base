    ...
    Gate: ProofGate + 'static,
{
    ...
    async fn update_safe_and_finalized(
        ...
        let local_safe = local.block_info(BlockNumberOrTag::Safe).await?.unwrap_or_default();
        let local_finalized =
            local.block_info(BlockNumberOrTag::Finalized).await?.unwrap_or_default();
        ...
        let source_finalized = source
            ...
            .min(safe_limit);
        let finalized = if source_finalized >= local_finalized.block_info.number {
