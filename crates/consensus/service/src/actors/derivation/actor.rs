    ...
    async fn handle_derivation_actor_request(
        ...
        match request_type {
            ...
            DerivationActorRequest::ProcessEngineSafeHeadUpdateRequest(safe_head) => {
                ...
                if *safe_head == self.derivation_state_machine.last_confirmed_safe_head() {
                    if self.derivation_state_machine.current_state()
                    ...
                    debug!(
                        target: "derivation",
                        safe_head = ?*safe_head,
                        derivation_state = ?self.derivation_state_machine,
                        "Ignoring duplicate safe head confirmation from engine"
                    );
                    return Ok(());
                }
...
mod tests {
    ...
    use async_trait::async_trait;
    use base_consensus_derive::PipelineError;
    use base_common_genesis::{RollupConfig, SystemConfig};
    use base_consensus_safedb::{SafeDBError, SafeHeadListener};
    ...
    struct TestSafeHeadListener {
        updates: Mutex<Vec<(L2BlockInfo, BlockInfo)>>,
    }

    #[async_trait]
    impl SafeHeadListener for TestSafeHeadListener {
        async fn safe_head_updated(
            &self,
            safe_head: L2BlockInfo,
            l1_block: BlockInfo,
        ) -> Result<(), SafeDBError> {
            self.updates.lock().unwrap().push((safe_head, l1_block));
            Ok(())
        }
    ...
    impl Iterator for PanicPipeline {
    ...
    impl base_consensus_derive::OriginProvider for PanicPipeline {
    ...
    impl Pipeline for PanicPipeline {
        ...
        async fn step(&mut self, _cursor: L2BlockInfo) -> StepResult {
            panic!("duplicate confirmations must not retry derivation")
        }
        ...
        async fn system_config_by_number(
            ...
            panic!("duplicate confirmations must not query system config")
        }
    }

    #[async_trait]
    impl SignalReceiver for PanicPipeline {
        async fn signal(&mut self, _signal: Signal) -> base_consensus_derive::PipelineResult<()> {
            Ok(())
        }
    }

    #[derive(Debug, Default)]
    struct YieldPipeline {
        rollup_config: RollupConfig,
    }

    impl Iterator for YieldPipeline {
        type Item = AttributesWithParent;
    ...
    impl base_consensus_derive::OriginProvider for YieldPipeline {
        fn origin(&self) -> Option<BlockInfo> {
            None
        }
    }
    ...
    #[async_trait]
    impl Pipeline for YieldPipeline {
        fn peek(&self) -> Option<&AttributesWithParent> {
            None
        }
    ...
    #[async_trait]
    impl SignalReceiver for YieldPipeline {
        async fn signal(&mut self, _signal: Signal) -> base_consensus_derive::PipelineResult<()> {
            Ok(())
        }
    }

    fn l2_block(number: u64, hash_byte: u8) -> L2BlockInfo {
        L2BlockInfo {
            ...
            l1_origin: BlockNumHash { number: number.saturating_sub(1), hash: B256::ZERO },
            seq_num: 0,
        }
    }
    ...
    #[tokio::test]
    async fn duplicate_safe_head_confirmation_is_ignored_before_side_effects() {
        let safe_head_listener = Arc::new(TestSafeHeadListener::default());
        let (engine_tx, _engine_rx) = mpsc::channel(1);
        let (derivation_origin_tx, _derivation_origin_rx) = watch::channel(None);
        let (_inbound_tx, inbound_rx) = mpsc::channel(1);
        let mut actor = DerivationActor::new(
            super::super::engine_client::QueuedDerivationEngineClient::new(engine_tx),
            CancellationToken::new(),
            inbound_rx,
            PanicPipeline::default(),
            safe_head_listener.clone(),
            derivation_origin_tx,
        );

        let confirmed_safe_head = l2_block(10, 0x10);
        let pending_derived_from = l1_block(99, 0x99);
        actor.derivation_state_machine.confirmed_safe_head = confirmed_safe_head;
        actor.derivation_state_machine.state = DerivationState::AwaitingSafeHeadConfirmation;
        actor.pending_derived_from = Some(pending_derived_from);

        actor
            .handle_derivation_actor_request(
                DerivationActorRequest::ProcessEngineSafeHeadUpdateRequest(Box::new(
                    confirmed_safe_head,
                )),
            )
            .await
        ...
        assert_eq!(actor.derivation_state_machine.last_confirmed_safe_head(), confirmed_safe_head);
        assert_eq!(actor.pending_derived_from, Some(pending_derived_from));
        assert!(safe_head_listener.updates.lock().unwrap().is_empty());
    }

    #[tokio::test]
    async fn duplicate_safe_head_confirmation_after_signal_wakes_derivation_without_side_effects() {
        let safe_head_listener = Arc::new(TestSafeHeadListener::default());
        let (engine_tx, _engine_rx) = mpsc::channel(1);
        let (derivation_origin_tx, _derivation_origin_rx) = watch::channel(None);
        let (_inbound_tx, inbound_rx) = mpsc::channel(1);
        ...
        let pending_derived_from = l1_block(99, 0x99);
        actor.derivation_state_machine.confirmed_safe_head = confirmed_safe_head;
        actor.derivation_state_machine.state = DerivationState::AwaitingUpdateAfterSignal;
        actor.pending_derived_from = Some(pending_derived_from);
