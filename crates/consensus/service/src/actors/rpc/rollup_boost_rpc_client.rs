...
use base_consensus_rpc::{
    RollupBoostAdminClient, RollupBoostHealthzApiServer, RollupBoostHealthzResponse,
};
use jsonrpsee::{
    core::RpcResult,
        ...
        self.engine_actor_request_tx
            .send(EngineActorRequest::RpcRequest(Box::new(
                EngineRpcRequest::RollupBoostHealthRequest(Box::new(
                    base_consensus_rpc::RollupBoostHealthQuery { sender: health_tx },
                )),
            )))
            .await
...
impl RollupBoostAdminClient for RollupBoostAdminApiClient {
        ...
        engine_actor_request_tx
            .send(EngineActorRequest::RpcRequest(Box::new(
                EngineRpcRequest::RollupBoostAdminRequest(Box::new(
                    base_consensus_rpc::RollupBoostAdminQuery::SetExecutionMode {
    ...
    async fn get_execution_mode(&self) -> RpcResult<GetExecutionModeResponse> {
        ...
        engine_actor_request_tx
            .send(EngineActorRequest::RpcRequest(Box::new(
                EngineRpcRequest::RollupBoostAdminRequest(Box::new(
                    base_consensus_rpc::RollupBoostAdminQuery::GetExecutionMode { sender: mode_tx },
                )),
            )))
            .await
            .map_err(|_| ErrorObject::from(ErrorCode::InternalError))?;
