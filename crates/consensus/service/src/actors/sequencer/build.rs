...
impl<A: AttributesBuilder, O: OriginSelector, E: SequencerEngineClient> PayloadBuilder<A, O, E> {
    ...
    /// `timestamp_from_block_number` using the child block number; otherwise advances
    ...
    fn plan_next_timestamp(
        ...
        let (timestamp, timestamp_millis_part) = self
            .rollup_config
            .timestamp_from_block_number(child_block_number)
            .ok_or(SequencerTimestampPlannerError::TimestampOverflow)?;
        if self.rollup_config.is_zombie_active(timestamp) {
