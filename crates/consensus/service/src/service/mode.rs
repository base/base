...
/// [`NodeMode::into_operating_mode`] to attach runtime configuration and obtain a
/// [`NodeOperatingMode`].
#[derive(
    Debug,
    Default,
...
impl NodeMode {
    ...
    pub const fn try_into_operating_mode(
        ...
        match self {
            ...
            Self::Sequencer => Ok(NodeOperatingMode::Sequencer),
            Self::ShadowSequencer => match shadow_blocks_per_cycle {
                ...
                None => Err("--mode shadow-sequencer requires --sequencer.shadow-blocks-per-cycle"),
            },
