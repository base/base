...
use base_protocol::{
    BaseTimeUpdateTx, BlockInfo, L1BlockInfoBedrockBaseFields as _, L1BlockInfoTx, L2BlockInfo,
};
    ...
    pub fn new(l1_info: L1BlockInfoTx) -> Self {
        Self {
            origin: BlockInfo::new(
                ...
                B256::ZERO,
                match l1_info {
                    L1BlockInfoTx::Bedrock(info) => info.time(),
                    L1BlockInfoTx::Ecotone(info) => info.time(),
                    L1BlockInfoTx::Isthmus(info) => info.time(),
                    L1BlockInfoTx::Jovian(info) => info.time(),
                },
            ),
        }
    }
...
impl<E: EngineClient + 'static> StandaloneSequencerNode<E> {
    ...
    pub async fn start_with_cancellation(
        ...
        let processor = EngineProcessor::new_l1_free(
