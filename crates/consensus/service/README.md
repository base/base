...
`ProcessSafeL2SignalRequest` takes a `SafeL2SignalRequest`: `Derived` pairs `AttributesWithParent` with a confirmation oneshot owned by `AwaitingSafeHead`, and `Delegated` promotes a safe `L2BlockInfo` without a waiter. Attributes are forwarded to `engine_api::forkchoice_updated`. After the drain that runs a derived consolidate, the engine completes that oneshot and skips the matching mailbox event. Delegated consolidates publish `ProcessEngineSafeHeadUpdateRequest` so idle wait-states can advance their cursor.
...
The derivation actor drives the `OnlinePipeline` from `base-consensus-derive` and translates its output into engine requests. Idle wait-states are distinct types (`AwaitingELSync`, `AwaitingL1Data`, `AwaitingSafeHead`, `AwaitingSignal`, `AwaitingUpdateAfterSignal`) dispatched by `Idle`. `Deriving` is a transient work phase and is never stored across an inbound `recv`. `DerivationState` is a `Copy` observability snapshot for logs, metrics, and tests.

The actor starts in `AwaitingELSync`. It waits until the engine sends a `ProcessEngineSyncCompletionRequest` via `QueuedEngineDerivationClient::notify_sync_completed()`, then enters `Deriving`.

In `Deriving` the actor calls `pipeline.step()` in a loop. Each call either returns `PreparedAttributes` — a set of `AttributesWithParent` ready to send to the engine — or returns an error. On `PreparedAttributes` the actor creates a confirmation oneshot, transitions to `AwaitingSafeHead`, enqueues the attributes in the `L2Finalizer`, records the L1 inclusion block as `pending_derived_from`, and sends `SafeL2SignalRequest::Derived` to the engine. On `NotEnoughData` it yields and transitions to `AwaitingL1Data`. On a reset error (reorg detected or Holocene activation) it sends a `ProcessEngineSignalRequest` and transitions to `AwaitingSignal`.

`AwaitingSafeHead` leaves only when that oneshot completes. Mailbox `ProcessEngineSafeHeadUpdateRequest` events are absorbed there and in other idle wait-states; they advance `confirmed_safe_head` when the incoming number is greater (or replaces the same height) and never crash the actor. A reset or activation signal installs that signal's `l2_safe_head` as the derivation cursor (including rewinds) before entering `AwaitingUpdateAfterSignal`, which then waits for L1 data or an engine safe-head update before deriving again.
