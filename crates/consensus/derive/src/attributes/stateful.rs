...
use base_consensus_batch::{BaseTimeUpdateTx, Deposits, L1BlockInfoTx, L2BlockInfo};
use tracing::warn;
    ...
    L2P: L2ChainProvider + Debug + Send,
{
    ...
    async fn prepare_payload_attributes(
        ...
        Ok(BasePayloadAttributes {
            ...
            min_base_fee: Some(sys_config.min_base_fee.unwrap_or_default()), /* Default to zero if not
                                                                              * set at Jovian */
        })
    }
}
...
mod tests {
    ...
    use base_common_chain_config::{
        BaseUpgradeConfig, ChainGenesis, Sepolia, SystemConfig, SystemConfigUpdate,
        SystemConfigUpdateKind, UpgradeConfig,
    };
    use base_common_types_chain::{BaseTxEnvelope, Header, SystemAddresses};
    use base_consensus_batch::{BlockInfo, DepositDecodeError};
    ...
    #[tokio::test]
    async fn test_prepare_payload_uses_azul_baseline_without_old_fork_schedule() {
        let block_time = 10_u64;
        let timestamp = 100_u64;
        let cfg = Arc::new(RollupConfig {
            block_time,
