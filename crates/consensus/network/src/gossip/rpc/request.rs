...
use crate::{
    BaseEnr, Discv5Handler,
    gossip::{ConnectionGate, GossipDriver, Metrics},
};
...
pub enum P2pRpcRequest {
    ...
    /// Dumps the node's discovery table from the [`base_consensus_network::Discv5Driver`].
    DiscoveryTable(Sender<Vec<String>>),
    /// Returns the current peer count for both the
    /// - Discovery Service ([`base_consensus_network::Discv5Driver`])
