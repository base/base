...
use base_consensus_node::{EngineConfig, FollowNode, L1Config, NodeMode, RemoteL2Client};
...
impl ConsensusFollowNodeArgs {
    ...
    async fn follow_node(
        ...
        let rpc_builder: RpcBuilder = Option::<RpcBuilder>::from(self.config.rpc_flags.clone())
            .ok_or_else(|| eyre::eyre!("follow mode requires RPC"))?;
