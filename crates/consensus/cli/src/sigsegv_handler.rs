...
fn backtrace_stderr(buffer: &[*mut libc::c_void]) {
    ...
    // SAFETY: `buffer` is a valid slice of stack frame pointers, `size` fits in i32 for
    // typical stack depths, and `STDERR_FILENO` is a valid open file descriptor.
...
impl fmt::Write for RawStderr {
    fn write_str(&mut self, s: &str) -> Result<(), fmt::Error> {
        // SAFETY: `STDERR_FILENO` is a valid open file descriptor and `s` is a valid
        // UTF-8 string with a correctly computed length.
...
extern "C" fn print_stack_trace(_: libc::c_int) {
    ...
    // SAFETY: `stack_trace` is a valid mutable array of pointer-sized values and
    // `MAX_FRAMES` fits in i32. The returned slice is bounded by the valid depth.
    let stack = unsafe {
...
pub fn install() {
    // SAFETY: We configure a valid alternate signal stack and register `print_stack_trace`,
    // which has the correct `extern "C" fn(c_int)` signature for a SIGSEGV handler.
    unsafe {
