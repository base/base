...
//!
//! - **QueuedDerivationEngineClient**: Wraps the engine channel sender, allowing
//!   the `DerivationActor` to send consolidation signals to the engine.
//!
//! - **QueuedNetworkEngineClient**: Wraps the engine channel sender, allowing
//!   the `NetworkActor` to send unsafe blocks from P2P to the engine.
//!
//! ```text
//! ┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
...
//! └────────┬────────┘     └────────┬────────┘     └────────┬────────┘
//!          │                       │                       │
//!          │ Queued*EngineClient   │ Queued*EngineClient   │
//!          └───────────┬───────────┴───────────┬───────────┘
//!                      ▼                       ▼
...
use base_engine_actor::{DirectEngineActor, DirectEngineApi, EngineContext, NodeActor};
...
///
/// # Engine Channel Clients
///
/// Kona's actors communicate with the engine via channel-based clients:
///
/// - [`QueuedDerivationEngineClient`] - Used by `DerivationActor` to send consolidation
///   signals and finalization updates to the engine.
/// - [`QueuedNetworkEngineClient`] - Used by `NetworkActor` to send unsafe blocks
///   received via P2P gossip to the engine.
///
/// These clients wrap an `mpsc::Sender<EngineActorRequest>` and can be created from
/// the engine channel sender returned by [`Self::start_with_clients`].
#[derive(Debug)]
...
impl<E: DirectEngineApi + std::fmt::Debug + 'static> UnifiedRollupNode<E> {
    ...
    ///
    /// // Use clients.derivation_client to wire up DerivationActor
    /// // Use clients.network_client to wire up NetworkActor
    ///
    /// // Wait for shutdown
    /// handle.await?;
    /// ```
    pub async fn start_with_clients(
        ...
        // Create context channels for engine actor
        let (sync_complete_tx, _sync_complete_rx) = tokio::sync::oneshot::channel();
        let (derivation_signal_tx, _derivation_signal_rx) = mpsc::channel(self.channel_buffer_size);
        let (engine_l2_safe_head_tx, _engine_l2_safe_head_rx) =
        ...
        // Create the queued engine clients for wiring other actors
        let derivation_client =
            QueuedDerivationEngineClient { engine_actor_request_tx: engine_tx.clone() };
        let network_client =
            QueuedNetworkEngineClient { engine_actor_request_tx: engine_tx.clone() };

        let clients = EngineClients { derivation_client, network_client, engine_tx };
