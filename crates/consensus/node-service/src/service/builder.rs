//! Contains the builder for the [`BaseNode`].
...
/// The [`BaseNodeBuilder`] is used to construct a [`BaseNode`] service.
#[derive(Debug)]
...
impl BaseNodeBuilder {
    /// Creates a new [`BaseNodeBuilder`] with the given [`RollupConfig`].
    pub fn new(
        config: RollupConfig,
    ...
    /// Sets the [`RpcBuilder`] on the [`BaseNodeBuilder`].
    ...
    /// Assembles the [`BaseNode`] service.
    ///
    /// ## Panics
    ///
    /// Panics if:
