# `base-consensus-driver-service`

<a href="https://crates.io/crates/base-consensus-driver-service"><img src="https://img.shields.io/crates/v/base-consensus-driver-service.svg" alt="base-consensus-driver-service crate"></a>
...
Fourth, the derivation actor is created. In normal mode this is a `DerivationActor` wrapping an `OnlinePipeline` from `base-consensus-derive-pipeline`. In delegation mode this is a `DelegateDerivationActor` that polls an external sync-status RPC endpoint instead of running the pipeline. Both communicate to the engine through a `QueuedDerivationEngineClient` that wraps `mpsc::Sender<EngineActorRequest>`.
...
The derivation actor drives the `OnlinePipeline` from `base-consensus-derive-pipeline` and translates its output into engine requests. It maintains a `DerivationStateMachine` with six states.
