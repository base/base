...
pub enum DerivationState {
    ...
    /// A reorg or some other inconsistency was detected, necessitating a [`base_consensus_derive_pipeline::Signal`] to
    /// be processed before continuing derivation.
    AwaitingSignal,
    /// After receiving a [`base_consensus_derive_pipeline::Signal`], we need an update of L1 data or a new engine
...
pub enum DerivationStateUpdate {
    ...
    /// A [`base_consensus_derive_pipeline::Signal`] is necessary to update the derivation pipeline in order to
    /// continue.
    SignalNeeded,
    /// A [`base_consensus_derive_pipeline::Signal`] has been received and processed.
    SignalProcessed,
}
...
///
/// ## Signal handling
/// Certain conditions require a [`base_consensus_derive_pipeline::Signal`] to be processed by the
/// [`base_consensus_derive_pipeline::Pipeline`], updating derivation state before continuing derivation. This struct
