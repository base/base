//! Contains error types for the [`crate::engine::SynchronizeTask`].

use base_common_types_payload::PayloadStatusEnum;
use thiserror::Error;

use crate::engine::{
    EngineClientError, EngineTaskError, task_queue::tasks::task::EngineTaskErrorSeverity,
};

/// An error that occurs when running the [`crate::engine::SynchronizeTask`].
#[derive(Debug, Error)]
