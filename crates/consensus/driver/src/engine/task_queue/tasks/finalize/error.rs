//! Contains error types for the [`crate::engine::FinalizeTask`].

use base_consensus_batch::FromBlockError;
use thiserror::Error;
...
/// An error that occurs when running the [`crate::engine::FinalizeTask`].
#[derive(Debug, Error)]
pub enum FinalizeTaskError {
    ...
    ///
    /// [`L2BlockInfo`]: base_consensus_batch::L2BlockInfo
    #[error(transparent)]
    FromBlock(#[from] FromBlockError),
    /// A temporary local read failure.
    #[error(transparent)]
    Local(#[from] crate::engine::EngineClientError),
