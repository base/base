...
use base_consensus_batch::{AttributesWithParent, BlockInfo, L1BlockInfoBedrock, L2BlockInfo};
...
async fn consolidate_does_not_crash_when_safe_behind_unsafe_and_attributes_mismatch() {
    let safe_head = crate::engine::test_utils::test_block_info(34);
    let unsafe_head = crate::engine::test_utils::test_block_info(76);
...
async fn consolidate_rejects_attribute_transaction_with_trailing_bytes() {
    let safe_head = crate::engine::test_utils::test_block_info(0);
    ...
    assert_eq!(
        AttributesMatch::check(
            &cfg,
            &attributes,
            &crate::engine::test_utils::MockEngineClient::native_block(
    ...
    let mut state = TestEngineStateBuilder::new()
        .with_safe_head(safe_head)
        .with_unsafe_head(crate::engine::test_utils::test_block_info(block_number))
        .build();
