...
use base_consensus_batch::{BlockInfo, FromBlockError, L2BlockInfo};
...
/// An unsafe, safe, and finalized [`L2BlockInfo`] returned by the [`crate::engine::find_starting_forkchoice`]
/// function.
...
impl L2ForkchoiceState {
    ...
    >(
        ...
        let finalized = match engine_client.get_l2_block(BlockNumberOrTag::Finalized.into()).await {
            Ok(Some(rpc_block)) => {
        ...
        let safe = match engine_client.get_l2_block(BlockNumberOrTag::Safe.into()).await {
            Ok(Some(block)) => {
                match block_info_from_reth_or_checkpoint(
                    cfg,
...
>(
    ...
    match base_consensus_batch::L2BlockInfoDecoder::from_block_and_genesis(&block, &cfg.genesis) {
...
async fn find_earliest_unpruned_block<EngineClient_: EngineClient>(
    ...
    let mut last_known_unpruned =
        match base_consensus_batch::L2BlockInfoDecoder::from_block_and_genesis(
            &latest_consensus,
            &cfg.genesis,
        ) {
            Ok(info) => info,
