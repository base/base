...
use crate::engine::{EngineClient, Metrics};
...
>(
    ...
    let unsafe_walk_result = async {
        loop {
            ...
            current_fc.un_safe = base_consensus_batch::L2BlockInfoDecoder::from_block_and_genesis(
    ...
    let safe_walk_result = async {
        loop {
            ...
            safe_cursor = base_consensus_batch::L2BlockInfoDecoder::from_block_and_genesis(
                &block.into_block(),
                &cfg.genesis,
            )?;
            safe_walked_blocks = safe_walked_blocks.saturating_add(1);
...
mod tests {
    ...
    use crate::engine::{EngineClient, test_utils::test_engine_client_builder};
    ...
    /// the sake of `base_consensus_batch::L2BlockInfoDecoder::from_block_and_genesis`.
    #[tokio::test]
