...
use crate::{
    CheckpointWriter, ConsolidateTask, Engine, EngineActorRequest, EngineClient, EngineClientError,
    EngineDerivationClient, EngineError, EngineSyncStateUpdate, EngineTask, EngineTaskError,
    EngineTaskErrorSeverity, EngineTaskErrors, FinalizeTask, ForkchoiceCheckpointLabel,
    ForkchoiceCheckpointReader, InsertTask, InsertTaskResult, NoopCheckpointWriter,
    NoopForkchoiceCheckpointReader, actors::sequencer::CanonicalReconciliationInputs,
};
...
mod tests {
    ...
    use base_consensus_batch::{BaseTimeUpdateTx, BlockInfo, L1BlockInfoBedrock, L2BlockInfo};
    ...
    /// [`base_consensus_batch::L2BlockInfoDecoder::from_block_and_genesis`] accepts the block via the genesis path.
    ...
    fn external_payload_logs_denim_activation_once() {
        ...
        let (traces, _guard) = base_consensus_batch::capture_traces!();
    ...
    async fn bootstrap_at_genesis_validator_seeds_without_probing_el_sync() {
        ...
        // base_consensus_batch::L2BlockInfoDecoder::from_block_and_genesis accepts the block via the genesis fast path.
        let cfg = Arc::new(RollupConfig {
            genesis: ChainGenesis {
                l2: BlockNumHash { number: 0, hash: genesis_hash },
                l1: BlockNumHash { number: 0, hash: B256::ZERO },
