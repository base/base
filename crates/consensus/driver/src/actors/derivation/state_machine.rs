...
pub enum DerivationState {
    ...
    AwaitingL1Data,
    /// [`base_consensus_batch::AttributesWithParent`] were sent to the [`crate::EngineActor`], and the
...
pub enum DerivationStateUpdate {
    ...
    MoreDataNeeded,
    /// Derivation has produced new [`base_consensus_batch::AttributesWithParent`].
    NewAttributesDerived(Box<AttributesWithParent>),
    /// The EL has confirmed the derived [`base_consensus_batch::AttributesWithParent`] as the new safe
    /// head.
...
/// When new [`base_consensus_batch::AttributesWithParent`] are derived, they must be sent to the EL,
/// and the EL must confirm them by creating a new L2 safe head from them prior to further
/// derivation. There will be at most one [`base_consensus_batch::AttributesWithParent`] awaiting
...
mod tests {
    ...
    use base_consensus_batch::{AttributesWithParent, BlockInfo};
    use rstest::rstest;
