...
impl SafeHeadListener for SafeDB {
    fn safe_head_updated(
        ...
        let txn = self.db.begin_write().map_err(|e| SafeDBError::Database(e.to_string()))?;
        {
            let mut table =
                txn.open_table(SAFE_HEADS).map_err(|e| SafeDBError::Database(e.to_string()))?;
            table
                .insert(l1_block.number, &value)
                .map_err(|e| SafeDBError::Database(e.to_string()))?;
        }
        txn.commit().map_err(|e| SafeDBError::Database(e.to_string()))?;

        tracing::debug!(
            l1_number = l1_block.number,
            l1_hash = %l1_block.hash,
            l2_number = safe_head.block_info.number,
            l2_hash = %safe_head.block_info.hash,
            "recorded safe head",
        );
    ...
    fn safe_head_reset(&self, reset_safe_head: L2BlockInfo) -> Result<(), SafeDBError> {
        ...
        let (entries_deleted, was_noop) = {
            ...
            // Collect all keys at or after the reset L1 origin.
                ...
                .map_err(|e| SafeDBError::Database(e.to_string()))?
            {
                ...
                entries.push((entry.0.value(), *entry.1.value()));
            }

            // Find the first entry whose L2 number is >= the reset target.
            let mut found_key = None;
            for &(key, ref val) in &entries {
                let (_, _, l2_number) = Self::decode_value(val);
            ...
            match found_key {
                Some(first_key) => {
                    // Delete all entries from first_key onward.
                    let keys_to_delete: Vec<u64> =
                        entries.iter().filter(|(k, _)| *k >= first_key).map(|(k, _)| *k).collect();
                    let deleted = keys_to_delete.len();
                    for key in keys_to_delete {
                        table.remove(key).map_err(|e| SafeDBError::Database(e.to_string()))?;
                    }

                    // Re-anchor at the reset's stated L1 origin (not first_key, which may be
...
impl SafeDBReader for SafeDB {
    fn safe_head_at_l1(&self, l1_block_num: u64) -> Result<SafeHeadResponse, SafeDBError> {
        let txn = self.db.begin_read().map_err(|e| SafeDBError::Database(e.to_string()))?;
        let table = txn.open_table(SAFE_HEADS).map_err(|e| SafeDBError::Database(e.to_string()))?;
...
mod tests {
    ...
    fn test_write_and_read_back() {
        ...
        db.safe_head_updated(l2, l1).unwrap();

        let resp = db.safe_head_at_l1(100).unwrap();
    ...
    #[test]
    fn test_safe_head_at_l1_returns_highest_at_or_before() {
        let dir = tempfile::tempdir().unwrap();
        let db = SafeDB::open(dir.path().join("test.redb")).unwrap();

        // Insert entries at L1 blocks 100, 200, 300.
        db.safe_head_updated(l2_block_info(0xAA, 1000, 0x01, 100), block_info(0x01, 100)).unwrap();
        db.safe_head_updated(l2_block_info(0xBB, 2000, 0x02, 200), block_info(0x02, 200)).unwrap();
        db.safe_head_updated(l2_block_info(0xCC, 3000, 0x03, 300), block_info(0x03, 300)).unwrap();

        // Query at 150 — should return the entry at 100.
        let resp = db.safe_head_at_l1(150).unwrap();
        assert_eq!(resp.l1_block.number, 100);
        assert_eq!(resp.safe_head.number, 1000);

        // Query at 200 — should return the entry at 200 exactly.
        let resp = db.safe_head_at_l1(200).unwrap();
        assert_eq!(resp.l1_block.number, 200);
        assert_eq!(resp.safe_head.number, 2000);

        // Query at 999 — should return the entry at 300.
        let resp = db.safe_head_at_l1(999).unwrap();
        assert_eq!(resp.l1_block.number, 300);
        assert_eq!(resp.safe_head.number, 3000);
    }

    #[test]
    fn test_safe_head_at_l1_not_found() {
        let dir = tempfile::tempdir().unwrap();
        let db = SafeDB::open(dir.path().join("test.redb")).unwrap();

        // Empty DB should return NotFound.
        let err = db.safe_head_at_l1(100).unwrap_err();
        assert!(matches!(err, SafeDBError::NotFound));

        // Insert at L1=200, query at L1=100 should still be NotFound.
        db.safe_head_updated(l2_block_info(0xAA, 1000, 0x01, 200), block_info(0x01, 200)).unwrap();
        let err = db.safe_head_at_l1(100).unwrap_err();
        assert!(matches!(err, SafeDBError::NotFound));
    }

    #[test]
    fn test_truncate_on_reset() {
        let dir = tempfile::tempdir().unwrap();
        let db = SafeDB::open(dir.path().join("test.redb")).unwrap();
        ...
        db.safe_head_reset(reset).unwrap();
        ...
        let resp = db.safe_head_at_l1(300).unwrap();
        assert_eq!(resp.l1_block.number, 200);
        assert_eq!(resp.safe_head.number, 1500);
    }

    #[test]
    fn test_truncate_on_reset_before_first_entry() {
        let dir = tempfile::tempdir().unwrap();
        let db = SafeDB::open(dir.path().join("test.redb")).unwrap();
        ...
        let resp = db.safe_head_at_l1(50).unwrap();
    ...
    #[test]
    fn test_truncate_on_reset_after_last_entry() {
        let dir = tempfile::tempdir().unwrap();
        let db = SafeDB::open(dir.path().join("test.redb")).unwrap();
    ...
    #[test]
    fn test_reset_creates_synthetic_anchor_in_gap() {
        let dir = tempfile::tempdir().unwrap();
        let db = SafeDB::open(dir.path().join("test.redb")).unwrap();
    ...
    #[test]
    fn test_disabled_db_is_noop() {
        let disabled = DisabledSafeDB;

        let l1 = block_info(0x11, 100);
        let l2 = l2_block_info(0x22, 200, 0x11, 100);

        disabled.safe_head_updated(l2, l1).unwrap();
        disabled.safe_head_reset(l2).unwrap();

        let err = disabled.safe_head_at_l1(100).unwrap_err();
