...
impl<EngineClient_: EngineClient> BuildTask<EngineClient_> {
    ...
    pub(super) async fn start_build(
        ...
        let upstream_attrs = crate::compat::to_upstream_attrs(attributes_envelope.attributes);
        let update = match forkchoice_version {
            EngineForkchoiceVersion::V3 => {
                engine_client.fork_choice_updated_v3(new_forkchoice, Some(upstream_attrs)).await
            }
            EngineForkchoiceVersion::V2 => {
                engine_client.fork_choice_updated_v2(new_forkchoice, Some(upstream_attrs)).await
            }
        }
        .map_err(|e| {
