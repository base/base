...
mod archive;
mod config_gen;
pub use config_gen::write_config;
mod extract;
mod fetch;
mod manifest;
pub use manifest::{
...
mod manifest_cmd;
pub use manifest_cmd::SnapshotManifestCommand;
mod planning;
mod progress;
mod session;
mod source;
...
use config_gen::config_for_selections;
...
impl DownloadCommand {
    ...
    fn finalize_modular_download(
        ...
        let start_command = startup_node_command(self.env.chain.as_ref());
...
fn startup_node_command(chain_spec: &BaseChainSpec) -> String {
    startup_node_command_for_binary(&current_binary_name(), chain_spec)
}

fn startup_node_command_for_binary(binary_name: &str, chain_spec: &BaseChainSpec) -> String {
    let mut command = format!("{binary_name} node");

    if let Some(chain_arg) = startup_chain_arg(chain_spec) {
...
fn startup_chain_arg(chain_spec: &BaseChainSpec) -> Option<String> {
    let current_chain = chain_spec.chain();
    let current_genesis_hash = chain_spec.genesis_hash();
    let default_chain = crate::BaseChainSpecParser::default_value()
        .and_then(|chain_name| crate::BaseChainSpecParser::parse(chain_name).ok());
    ...
    crate::BaseChainSpecParser::SUPPORTED_CHAINS
        .iter()
        .find_map(|chain_name| {
            let parsed_chain = crate::BaseChainSpecParser::parse(chain_name).ok()?;
...
impl DownloadCommand {
    ...
    pub const fn env(&self) -> &EnvironmentArgs {
        &self.env
    }
...
mod tests {
    ...
    fn test_download_resumable_defaults_to_true() {
        let args = CommandParser::<DownloadCommand>::parse_from(["reth"]).args;
    ...
    fn test_download_resumable_implicit_true() {
        let args = CommandParser::<DownloadCommand>::parse_from(["reth", "--resumable"]).args;
    ...
    fn test_download_resumable_explicit_false() {
        let args = CommandParser::<DownloadCommand>::parse_from(["reth", "--resumable=false"]).args;
    ...
    fn test_download_print_plan_json_parses() {
        let args = CommandParser::<DownloadCommand>::parse_from([
            "reth",
            "--manifest-path",
            "manifest.json",
            "--minimal",
    ...
    fn test_download_print_plan_json_rejects_single_archive() {
        let result = CommandParser::<DownloadCommand>::try_parse_from([
            "reth",
            "--url",
    ...
    fn resolve_manifest_source_requires_explicit_source_for_non_mainnet_defaults() {
        let args =
            CommandParser::<DownloadCommand>::parse_from(["reth", "--chain", "base-sepolia"]).args;
    ...
    fn startup_node_command_omits_default_chain_arg() {
        let command = startup_node_command_for_binary(
            "reth",
    ...
    fn startup_node_command_includes_non_default_chain_arg() {
        ...
        assert_eq!(command, "reth node --chain base_sepolia");
    }
    ...
    fn startup_node_command_uses_running_binary_name() {
        ...
        assert_eq!(command, "tempo node --chain base_sepolia");
    }
