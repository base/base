...
use base_execution_state_operations::init::init_from_state_dump;
...
mod without_evm;
pub use without_evm::setup_without_evm;

/// Initializes the database with the genesis block.
#[derive(Debug, Parser)]
pub struct InitStateCommand {
    #[command(flatten)]
    pub env: EnvironmentArgs,
...
mod tests {
    ...
    fn parse_init_state_command_with_without_evm() {
        let cmd: InitStateCommand = InitStateCommand::parse_from([
            "reth",
            "--chain",
            "base-sepolia",
            "--without-evm",
