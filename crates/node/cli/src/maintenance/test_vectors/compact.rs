...
macro_rules! compact_types {
    (regular: [$($regular_ty:ty),*], identifier: [$($id_ty:ty),*]) => {
        ...
        pub static IDENTIFIER_TYPE: std::sync::LazyLock<std::collections::HashSet<String>> = std::sync::LazyLock::new(|| {
            std::collections::HashSet::from([$(type_name::<$id_ty>()),*])
        });
    };
}
...
compact_types!(
    regular: [
        ...
        EthereumTxEnvelope::<EthereumTxEip4844>,
    ...
    identifier: [
        Signature,
        EthereumTypedTransaction::<EthereumTxEip4844>,
        TxType,
        TxKind
    ]
);
