...
use base_common_types_payload::{ConsensusEngineHandle, ExecutionData};
...
pub trait PayloadProvider: Send + Sync + 'static {
    ...
    fn get_payload(
        &self,
        block_number: u64,
    ) -> impl Future<Output = eyre::Result<ExecutionData>> + Send;
    ...
    P: PayloadProvider + Clone,
{
    ...
    pub async fn run(self) {
        ...
        let mut payload_stream = {
            let (tx, rx) = mpsc::channel::<ExecutionData>(64);
