...
pub struct EtherscanBlockProvider<RpcBlock> {
    http_client: Client,
    base_url: String,
    api_key: String,
    chain_id: u64,
...
impl<RpcBlock> EtherscanBlockProvider<RpcBlock>
where
...
impl<RpcBlock> PayloadProvider for EtherscanBlockProvider<RpcBlock>
where
    RpcBlock: Serialize + DeserializeOwned + 'static,
{
    async fn subscribe_payloads(&self, tx: mpsc::Sender<ExecutionData>) {
