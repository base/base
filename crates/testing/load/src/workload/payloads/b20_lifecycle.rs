...
use base_common_client_ethereum::{
    Ethereum, EthereumWallet, PendingTransactionBuilder, Provider, TransactionBuilder,
};
use base_common_types_rpc::TransactionRequest;
use base_execution_evm_runtime::{B20FactoryStorage, B20TokenRole, B20Variant, IB20, IB20Factory};
...
pub(super) async fn prepare(
    payload: &B20TransferPayload,
    ctx: &mut ChainPrepContext<'_>,
) -> Result<()> {
    ...
    let submit_futs = account_data.into_iter().map(|(sender, signer)| {
        ...
        async move {
            ...
            let grant_burn = IB20::grantRoleCall { role: B20TokenRole::Burn.id(), account: sender };
    ...
    while let Some((sender, result)) = receipt_stream.next().await {
        match result {
            Ok(receipt) if receipt.status() => {
                trace!(sender = %sender, tx_hash = %receipt.transaction_hash, "B-20 token created");
            }
            Ok(receipt) => {
...
async fn submit_b20_create(
    ...
    match provider.send_transaction(build(nonce, max_fee, max_priority_fee)).await {
        ...
        Err(e) => {
            let msg = e.to_string();
            if msg.contains("replacement transaction underpriced") || msg.contains("already known")
            {
                // A prior run left a pending tx at this nonce; replace it at a higher fee.
                provider
                    .send_transaction(build(nonce, replacement_max_fee, replacement_priority_fee))
                    .await
                    .map_err(|e| {
...
pub(super) async fn teardown(
    ...
    let signers: std::collections::HashMap<_, _> =
        ctx.accounts.accounts().iter().map(|a| (a.address, a.signer.clone())).collect();
    ...
    while let Some((sender, balance, result)) = receipt_stream.next().await {
        match result {
            Ok(receipt) if receipt.status() => {
                trace!(sender = %sender, amount = %balance, tx_hash = %receipt.transaction_hash, "B-20 burn confirmed");
                burn_count += 1;
            }
            Ok(receipt) => {
