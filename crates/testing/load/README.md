...
| Crate | Description |
|-------|-------------|
| `base-testing-load` | Core library with workload generation, transaction submission, and metrics collection |
...
```bash
# Build the crates
cargo build -p base-testing-load -p base-load-tester-bin

# Run tests
cargo test -p base-testing-load
...
FUNDER_KEY=0x... LOAD_TEST_OUTPUT=cobalt-results.json \
  ...
  crates/testing/load/examples/cobalt-devnet.yaml
```
...
Use `RUST_LOG=base_testing_load=debug` for pacing diagnostics. Normal per-transaction and per-account
