...
`base-testing-devnet` is the in-process integration-test harness for rollup
...
- The sequencer can optionally run the **production full-block builder**
  in-process (`BuilderBackedEngineClient`, launched via
  `ActionTestHarness::create_l2_sequencer_with_builder`). In this mode the
  production `SequencerActor` drives a real `BlockServiceBuilder` + real
  transaction pool through the native execution handle, so tests exercise real pool
  transaction selection and other block-construction
  policy that the default in-memory engine never reaches.
...
| Engine (default) | `BasePayloadBuilder`, Base EVM config, temporary Reth database, state-root comparison | `ActionEngineClient` implements the execution commands tests need | Simplified payload statuses, forkchoice handling, transaction pool, networking, persistence lifecycle, and execution error cases |
| Builder (opt-in) | Production `BlockServiceBuilder` + real transaction pool driving a real in-process Reth node through native execution services (`BuilderBackedEngineClient`); real pool selection, DA/gas limits, metering | `L2Sequencer<BuilderBackedEngineClient>` via `create_l2_sequencer_with_builder`; the production `SequencerActor` drives it, but transactions still arrive via harness injection rather than RPC ingress | Block scheduling is wall-clock based, so pool-dependent tests use the wall-clock timestamp mode (anchor L1+L2 genesis near `now`, within `max_sequencer_drift`) and must land within the inserted-block timeout |
...
- Unsafe gossip acceptance and rejection when in-memory signing validation is
  enabled.
- With the opt-in builder backend, production block-construction policy:
  transaction-pool selection, and (as coverage grows)
  DA/gas limits and metering.
