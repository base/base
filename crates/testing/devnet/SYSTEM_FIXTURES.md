...
cargo run -p base-bin-devnet --bin base-devnet -- snapshot \
  --chain sepolia \
...
cargo run --release -p base-bin-benchmark --bin base-bench -- snapshot \
  --chain mainnet \
  --builder-datadir "$BUILDER_DATADIR" \
  --client-datadir "$CLIENT_DATADIR" \
  --load-test-config \
    crates/testing/load/examples/account-create-mainnet-snapshot.yaml \
...
```bash
cargo run -p base-bin-devnet --bin base-devnet -- snapshot --help
cargo run -p base-bin-benchmark --bin base-bench -- snapshot --help
```
