...
use base_consensus_driver::{L1OriginSelectorError, L1OriginSelectorProvider, PreparedL1Origin};

use crate::action_fixtures::{L1Block, block_info_from};
...
impl ChainProvider for ActionL1ChainProvider {
    ...
    async fn block_info_and_transactions_by_hash(
        &mut self,
        hash: B256,
    ) -> Result<(BlockInfo, Vec<base_common_types_chain::TxEnvelope>), Self::Error> {
        self.chain.with(|blocks| {
            blocks
                .iter()
                .find(|b| b.hash() == hash)
