...
//!
//! [`L2Sequencer`](crate::action_fixtures::L2Sequencer) is generic over this trait so it can drive either the
//! in-memory [`ActionEngineClient`](crate::action_fixtures::ActionEngineClient) (the default, "Light" backend) or the
//! production-builder-backed [`BuilderBackedEngineClient`](crate::action_fixtures::BuilderBackedEngineClient) through
//! the same production `SequencerActor` seam.

use async_trait::async_trait;
use base_common_types_chain::BaseTxEnvelope;
use base_consensus_driver::EngineClientResult;

use crate::action_fixtures::SharedBlockHashRegistry;

/// A sequencer engine backend: a [`SequencerEngineClient`](base_consensus_driver::SequencerEngineClient)
...
#[async_trait]
pub trait SequencerEngineBackend: base_consensus_driver::SequencerEngineClient + 'static {
