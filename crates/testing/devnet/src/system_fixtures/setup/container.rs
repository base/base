...
use crate::system_fixtures::config::{BATCHER, BUILDER, CHALLENGER, DEPLOYER, PROPOSER, SEQUENCER};
...
impl SetupImage {
    ...
    /// process's current directory, so this still finds the Dockerfile when `base-testing-devnet`
    ...
    pub fn find_repo_root() -> Result<PathBuf> {
        let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .ancestors()
            .find(|path| path.join(SETUP_DOCKERFILE_PATH).is_file())
            .ok_or_else(|| eyre::eyre!("devnet setup Dockerfile not found above crate directory"))?
            .to_path_buf();
        ensure!(
