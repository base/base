...
pub async fn get_l2_output_data(l2_block_num: u64) -> Result<HostCli> {
    let l1_rpc = env::var("CLABBY_RPC_L1").unwrap_or_else(|_| "http://localhost:8545".to_string());
    let l1_beacon_rpc =
        env::var("ETH_BEACON_URL").unwrap_or_else(|_| "http://localhost:5052".to_string());
    let l2_rpc = env::var("CLABBY_RPC_L2").unwrap_or_else(|_| "http://localhost:9545".to_string());

    let l1_provider = Provider::<Http>::try_from(l1_rpc.as_str())?;
    let l2_provider = Provider::<Http>::try_from(l2_rpc.as_str())?;
    ...
    Ok(HostCli {
        ...
        l2_chain_id,
        l2_head: l2_head.0.into(),
        l2_node_address: Some(l2_rpc.into()),
        l1_node_address: Some(l1_rpc.into()),
        l1_beacon_address: Some(l1_beacon_rpc.into()),
