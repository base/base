...
async fn test_game_chain_validation_anchor_reset() -> Result<()> {
    ...
    let a0_root = fetcher.l2_provider.compute_output_root_at_block(U256::from(a0_block)).await?;
    let a0_extra = (U256::from(a0_block), u32::MAX).abi_encode_packed();

    let tx =
        factory.create(TEST_GAME_TYPE, a0_root, a0_extra.into()).value(init_bond).send().await?;
    let receipt = tx.with_required_confirmations(3).get_receipt().await?;
    ...
    let a1_root = fetcher.l2_provider.compute_output_root_at_block(U256::from(a1_block)).await?;
    let a1_extra = (U256::from(a1_block), a0_index.to::<u32>()).abi_encode_packed();

    let tx =
        factory.create(TEST_GAME_TYPE, a1_root, a1_extra.into()).value(init_bond).send().await?;
    let receipt = tx.with_required_confirmations(3).get_receipt().await?;
    ...
    let b0_root = fetcher.l2_provider.compute_output_root_at_block(U256::from(b0_block)).await?;
    let b0_extra = (U256::from(b0_block), u32::MAX).abi_encode_packed();

    let tx =
        factory.create(TEST_GAME_TYPE, b0_root, b0_extra.into()).value(init_bond).send().await?;
    let receipt = tx.with_required_confirmations(3).get_receipt().await?;
    ...
    let b1_root = fetcher.l2_provider.compute_output_root_at_block(U256::from(b1_block)).await?;
    let b1_extra = (U256::from(b1_block), b0_index.to::<u32>()).abi_encode_packed();

    let tx =
        factory.create(TEST_GAME_TYPE, b1_root, b1_extra.into()).value(init_bond).send().await?;
    let receipt = tx.with_required_confirmations(3).get_receipt().await?;
    ...
    warp_time(&env.anvil.provider, Duration::from_secs(DISPUTE_GAME_FINALITY_DELAY_SECONDS + 60))
        .await?;
