...
fn load_static_mempool_content<P: AsRef<Path>>(filepath: P) -> Result<TxpoolContent, Box<dyn std::error::Error>> {
...
async fn test_e2e_static_data() {
    ...
    let filtered_geth_mempool = rebroadcaster.filter_underpriced_txns(&geth_mempool, base_fee, gas_price);
    let filtered_reth_mempool = rebroadcaster.filter_underpriced_txns(&reth_mempool, base_fee, gas_price);
    ...
    let expected_missing_hashes = vec![
...
async fn test_e2e_filtering_logic() {
    ...
    let filtered_geth_mempool = rebroadcaster.filter_underpriced_txns(&geth_mempool, very_high_base_fee, very_high_gas_price);

    // Assert all transactions are filtered out due to high base fee
    let total_pending = filtered_geth_mempool.pending.values()
        .map(|nonce_txs| nonce_txs.len())
        .sum::<usize>();
    let total_queued = filtered_geth_mempool.queued.values()
        .map(|nonce_txs| nonce_txs.len())
        .sum::<usize>();
