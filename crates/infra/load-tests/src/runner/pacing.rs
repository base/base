...
impl InFlightTarget {
    fn new(
        ...
        let initial_target_in_flight = target_gps.map_or_else(
            ...
            |gps| {
                let lookahead = Duration::from_secs_f64(TARGET_LOOKAHEAD_SECONDS);
                let target_gas =
    ...
    fn clamp_target_in_flight(&self, target_in_flight: u64) -> u64 {
    ...
    fn begin_measurement(
        ...
        if let Some(target_gps) = self.target_gps {
            let lookahead_end = started_at
                .checked_add(Duration::from_secs_f64(TARGET_LOOKAHEAD_SECONDS))
...
impl LoadRunner {
    ...
    async fn sender_start_nonces(
        ...
        stream::iter(nonce_futures).buffered(super::funding::FUNDING_CONCURRENCY).try_collect().await
    }
...
mod tests {
    ...
    fn open_loop_headroom_target_without_cap_matches_previous_formula() {
        ...
        target.begin_measurement(sampled_at, None, 0);
    ...
    fn capped_headroom_tracks_cumulative_gas_deficit() {
        let now = Instant::now();
        let started_at = now - TARGET_UPDATE_INTERVAL;
        let mut target =
            InFlightTarget::new(10_000, Some(2_100_000), 21_000, 0, started_at);
        assert_eq!(
