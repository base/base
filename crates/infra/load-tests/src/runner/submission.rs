...
impl SubmissionPipeline {
    ...
    ///
    /// `base_fee` is the latest observed `baseFeePerGas`. The result keeps the
    /// invariant `max_fee >= base_fee + priority_fee` (subject to the
    /// `max_gas_price` cap) so a transaction can never be minted underwater.
