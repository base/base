...
pub struct BlockContribution {
    ...
    pub timestamp: u64,
    /// Total transaction count accumulated from blocks.
    pub tx_count: usize,
}
...
impl L1Block {
    ...
    pub const fn from_info(info: L1BlockInfo) -> Self {
        Self {
            ...
            base_batcher_encoding: info.base_batcher_encoding,
    ...
    pub fn compression_ratio(&self) -> Option<f64> {
        ...
        if self.base_batcher_encoding == 0 {
            return None;
        }
        let l1_bytes = self.base_batcher_encoding * BLOB_SIZE;
...
impl DaTracker {
    ...
    pub fn record_l1_block(&mut self, info: L1BlockInfo) {
        ...
        if l1_block.base_batcher_encoding > 0 {
    ...
    fn try_attribute_l2_to_l1(&mut self) {
        ...
        let mut unmatched: Vec<usize> = self
            ...
            .iter()
            .enumerate()
            .filter(|(_, b)| b.base_batcher_encoding > 0 && b.l2_blocks_submitted.is_none())
            .map(|(i, _)| i)
            .collect();
        ...
        let total_blobs: u64 =
            unmatched.iter().map(|&i| self.l1_blocks[i].base_batcher_encoding).sum();
        if total_blobs == 0 {
            return;
        }
        ...
        for (nth, &idx) in unmatched.iter().enumerate() {
            let blobs = self.l1_blocks[idx].base_batcher_encoding;
    ...
    pub fn base_blob_share(&self, n: usize) -> Option<f64> {
        ...
        let base: u64 = blocks.iter().map(|b| b.base_batcher_encoding).sum();
