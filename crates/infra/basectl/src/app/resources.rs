...
pub struct ConductorState {
    ...
    /// Original per-node connection settings.
...
pub struct DaState {
    ...
    buffered_blocks: Vec<BlockDaInfo>,
...
impl DaState {
    ...
    pub fn poll(&mut self) {
        ...
        for info in block_infos {
            if self.loaded {
                self.record_block(info);
            } else {
                self.buffered_blocks.push(info);
            }
        }
    ...
    /// Incorporates a canonical block without double-counting backlog entries.
    pub fn record_block(&mut self, info: BlockDaInfo) {
        let new_block = !self
            .tracker
            .block_contributions
            .iter()
            .any(|block| block.block_number == info.block_number);
        self.tracker.update_block_info(info.block_number, info.da_bytes, info.timestamp);
        if new_block && info.block_number > self.tracker.safe_l2_block {
            self.tracker.growth_tracker.add_sample(info.da_bytes);
        }
        if let Some(block) = self
            .tracker
            .block_contributions
            .iter_mut()
            .find(|block| block.block_number == info.block_number)
        {
            block.tx_count = info.tx_count;
        }
    }

    fn flush_buffers(&mut self) {
        for block in std::mem::take(&mut self.buffered_blocks) {
...
mod tests {
    ...
    #[test]
    fn canonical_updates_do_not_duplicate_backlog() {
        let mut state = DaState::new();
        state.tracker.add_backlog_block(10, 100, 20);
        state.tracker.set_initial_backlog(9, 100);
        state.record_block(crate::rpc::BlockDaInfo {
            block_number: 10,
            da_bytes: 100,
            timestamp: 20,
            tx_count: 2,
        ...
        assert_eq!(state.tracker.da_backlog_bytes, 100);
        assert_eq!(state.tracker.block_contributions.len(), 1);
        assert_eq!(state.tracker.block_contributions.front().unwrap().tx_count, 2);
    }
