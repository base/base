...
impl BlockCommand {
    /// Runs the block inspection and renders its output.
    pub async fn run(self, config: MonitoringConfig) -> Result<()> {
        let block_ref = Self::parse_block_ref(&self.reference)?;
        ...
        match (self.json, self.raw) {
            ...
            (false, _) => BlockSummaryJson::print_pretty(&config.name, block_ref, &block)?,
        }
        Ok(())
    }
    ...
    ///
    /// Accepts bare decimal numbers, explicit 64-hex-character block hashes,
    /// and standard block tags except `pending`.
    pub fn parse_block_ref(s: &str) -> Result<BlockId, BlockRefParseError> {
        let trimmed = s.trim();
        if trimmed.is_empty() {
        ...
        let tag = trimmed.parse::<BlockNumberOrTag>().map_err(|e| {
            BlockRefParseError::InvalidTag { raw: trimmed.to_string(), message: e.to_string() }
        })?;
...
pub struct BlockSummaryJson {
    /// Selected network name.
    pub network: String,
    /// Requested block reference.
    pub reference: String,
    /// Block number.
    pub number: u64,
    /// Block hash.
    ...
    /// Block gas limit.
    pub gas_limit: u64,
    /// Base fee per gas in wei, when present.
    pub base_fee_per_gas_wei: Option<u128>,
    /// Encoded block size in bytes, when present.
    pub size_bytes: Option<u64>,
    /// Blob gas used, when present.
    pub blob_gas_used: Option<u64>,
    /// Excess blob gas, when present.
    pub excess_blob_gas: Option<u64>,
    /// Number of withdrawals, when present.
    pub withdrawals: Option<usize>,
}

impl BlockSummaryJson {
    /// Builds a humanized summary from an RPC block response.
    pub fn from_block(
        network: &str,
        reference: BlockId,
    ...
    /// Renders a block response as the pretty key-value table.
    pub fn print_pretty(
        network: &str,
        reference: BlockId,
        block: &<Base as Network>::BlockResponse,
...
mod tests {
    ...
    fn parses_decimal() {
        assert_eq!(
            BlockCommand::parse_block_ref("123").unwrap(),
            BlockId::Number(BlockNumberOrTag::Number(123)),
        );
        assert_eq!(
            BlockCommand::parse_block_ref("  42  ").unwrap(),
    ...
    fn parses_hex() {
        assert_eq!(
            BlockCommand::parse_block_ref("0x1a").unwrap(),
            BlockId::Number(BlockNumberOrTag::Number(26)),
        );
        assert_eq!(
            BlockCommand::parse_block_ref("0X1A").unwrap(),
            BlockId::Number(BlockNumberOrTag::Number(26)),
    ...
    fn parses_tags() {
        assert_eq!(
            BlockCommand::parse_block_ref("latest").unwrap(),
            BlockId::Number(BlockNumberOrTag::Latest)
        );
        assert_eq!(
            BlockCommand::parse_block_ref("safe").unwrap(),
            BlockId::Number(BlockNumberOrTag::Safe)
        );
        assert_eq!(
            BlockCommand::parse_block_ref("finalized").unwrap(),
            BlockId::Number(BlockNumberOrTag::Finalized),
        );
        assert_eq!(
            BlockCommand::parse_block_ref("earliest").unwrap(),
    ...
    fn parses_block_hash() {
        ...
        for input in [canonical.clone(), canonical.replace("0x", "0X"), canonical.to_uppercase()] {
            let parsed = BlockCommand::parse_block_ref(&input).unwrap();
    ...
    fn rejects_pending() {
        for input in ["pending", "Pending", "PENDING"] {
            assert!(matches!(
                BlockCommand::parse_block_ref(input).unwrap_err(),
    ...
    fn rejects_invalid_input() {
        assert!(matches!(
            BlockCommand::parse_block_ref("notatag").unwrap_err(),
            BlockRefParseError::InvalidTag { .. }
        ));
        assert!(matches!(
            BlockCommand::parse_block_ref("").unwrap_err(),
            BlockRefParseError::Empty
        ));
        assert!(matches!(
            BlockCommand::parse_block_ref("   ").unwrap_err(),
            BlockRefParseError::Empty
    ...
    fn block_summary_json_serializes_with_camel_case_and_nested_timestamp() {
        let summary = BlockSummaryJson {
            network: "sepolia".to_string(),
            reference: "latest".to_string(),
            number: 42,
            hash: B256::repeat_byte(0x11),
            parent_hash: B256::repeat_byte(0x22),
            timestamp: TimestampJson::from_unix(1_780_614_804),
            transactions: 7,
            gas_used: 21_000,
