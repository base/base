...
use alloy_primitives::{
    Address, B256, Bytes, Log, Signature as PrimitiveSignature, TxKind, U256, keccak256,
};

use async_trait::async_trait;
use base_batcher_encoder::{
    BatchEncoder, BatchPipeline, DaType, EncoderCompressionAlgo, EncoderConfig, FrameEncoder,
    StepResult as EncoderStepResult,
};
...
use base_protocol::{
    AttributesWithParent, BatchType, BatchValidationProvider, BlockInfo, L1BlockInfoTx, L2BlockInfo,
};
use k256::ecdsa::{SigningKey, VerifyingKey, signature::hazmat::PrehashSigner as _};
...
fn l2_block_info_from(block: &BaseBlock, l1_origin: BlockNumHash, seq_num: u64) -> L2BlockInfo {
    L2BlockInfo {
        block_info: BlockInfo {
...
pub struct Devnet {
    ...
    /// Current L1 origin used by the sequencer for the next L2 block.
    l1_origin: BlockNumHash,
    ...
    system_config: SystemConfig,
    /// Batcher encoder that turns L2 blocks into derivation frames.
    ...
    /// Number of frames submitted in the most recent epoch submission.
    last_submitted_frame_count: u64,
...
impl Devnet {
    ...
    pub async fn new() -> Self {
        ...
        let rollup_config = Arc::new(RollupConfig {
            ...
            hardforks: HardForkConfig { fjord_time: Some(2), ..Default::default() },
        ...
        let encoder_config = EncoderConfig {
            ...
            target_num_frames: 1,
            compression_algo: EncoderCompressionAlgo::Brotli10,
            ..EncoderConfig::default()
        };
        encoder_config
            .validate_for_rollup_config(&rollup_config, rollup_config.block_time)
            .expect("invalid batch encoder config for rollup state");
        let mut batch_encoder = BatchEncoder::new(Arc::clone(&rollup_config), encoder_config);
        batch_encoder.advance_l1_head(0);
        let (dap, dap_queue) = InMemoryDap::new_with_queue();

        let l1_origin_info =
            BlockInfo { hash: genesis_l1_hash, number: 0, parent_hash: B256::ZERO, timestamp: 0 };
    ...
    fn l1_tip(&self) -> L1Block {
        self.l1_chain.with(|blocks| blocks.last().expect("L1 chain empty").clone())
    }

    fn l1_block_by_number(&self, number: u64) -> L1Block {
        self.l1_chain.with(|blocks| {
            blocks
                .get(number as usize)
                .unwrap_or_else(|| panic!("L1 block {number} not found"))
                .clone()
        })
    }

    fn mine_single_l1_block(&mut self) -> L1Block {
        let tip = self.l1_tip();
        let new_block = L1Block {
            header: Header {
                number: tip.number() + 1,
                timestamp: tip.timestamp() + self.l1_block_time,
                parent_hash: tip.hash(),
                ..Default::default()
            },
            receipts: vec![],
        };
        self.l1_chain.push(new_block.clone());
        self.batch_encoder.advance_l1_head(new_block.number());
        new_block
    }

    fn ensure_l1_head_after(&mut self, timestamp: u64) {
        while self.l1_tip().timestamp() <= timestamp {
            self.mine_single_l1_block();
        }
    }

    fn refresh_l1_origin_for_l2_timestamp(&mut self, next_l2_timestamp: u64) {
        loop {
            let next_origin = self.l1_origin.number + 1;
            let next_origin_block =
                self.l1_chain.with(|blocks| blocks.get(next_origin as usize).cloned());
            let Some(next_origin_block) = next_origin_block else {
                break;
            };
            if next_l2_timestamp < next_origin_block.timestamp() {
                break;
            }
            self.l1_origin = BlockNumHash { number: next_origin, hash: next_origin_block.hash() };
            self.seq_num = 0;
        }
    }

    fn queue_block_for_batcher(&mut self, block: BaseBlock) {
        self.batch_encoder.add_block(block).expect("batch encoder failed to add L2 block");
    }

    fn flush_batcher_to_dap(&mut self) -> u64 {
        let l1_tip = self.l1_tip();
        self.batch_encoder.advance_l1_head(l1_tip.number());

        loop {
            match self.batch_encoder.step().expect("batch encoder step failed") {
                EncoderStepResult::Idle => break,
                EncoderStepResult::BlockEncoded | EncoderStepResult::ChannelClosed => {}
            }
        }
        self.batch_encoder.force_close_channel();

        let mut submitted_frames = 0u64;
        while let Some(submission) = self.batch_encoder.next_submission() {
            let submission_id = submission.id;
            submitted_frames += submission.frames.len() as u64;
            for frame in submission.frames {
                self.dap_queue.push(l1_tip.hash(), FrameEncoder::to_calldata(frame.as_ref()));
            }
            self.batch_encoder.confirm(submission_id, l1_tip.number());
        }
        self.last_submitted_frame_count = submitted_frames;
        submitted_frames
    }

    /// Mine `n` L1 blocks and return the new L1 tip block number.
    ///
    /// L1 block time is fixed at 12 seconds (independent from 2-second L2 time).
    ...
    pub fn produce_l2_block(&mut self) -> BaseBlock {
        let parent_timestamp = self.l2_head.header.timestamp;
        let next_l2_timestamp = self.l2_head.header.timestamp + self.rollup_config.block_time;
        self.ensure_l1_head_after(next_l2_timestamp);
        self.refresh_l1_origin_for_l2_timestamp(next_l2_timestamp);
        let l1_origin_block = self.l1_block_by_number(self.l1_origin.number);

        let mut extra = self.upgrade_txs_for_next_block(parent_timestamp, next_l2_timestamp);
        extra.extend(std::mem::take(&mut self.pending_txs));
        let block = produce_l2_block(
            &self.rollup_config,
            &l1_origin_block.header,
            &self.l2_head,
            self.seq_num,
            &self.system_config,
            extra,
        );
        let block_info = l2_block_info_from(&block, self.l1_origin, self.seq_num);
    ...
    fn upgrade_txs_for_next_block(
        &self,
        parent_timestamp: u64,
        next_l2_timestamp: u64,
    ) -> Vec<BaseTxEnvelope> {
        let mut upgrade_txs = Vec::new();
        if self.rollup_config.is_ecotone_active(next_l2_timestamp)
            && !self.rollup_config.is_ecotone_active(parent_timestamp)
        {
            upgrade_txs.extend(Upgrades::ECOTONE.txs());
        }
        if self.rollup_config.is_fjord_active(next_l2_timestamp)
            && !self.rollup_config.is_fjord_active(parent_timestamp)
        {
            upgrade_txs.extend(Upgrades::FJORD.txs());
        }
        if self.rollup_config.is_isthmus_active(next_l2_timestamp)
            && !self.rollup_config.is_isthmus_active(parent_timestamp)
        {
            upgrade_txs.extend(Upgrades::ISTHMUS.txs());
        }
        if self.rollup_config.is_jovian_active(next_l2_timestamp)
            && !self.rollup_config.is_jovian_active(parent_timestamp)
        {
            upgrade_txs.extend(Upgrades::JOVIAN.txs());
        }

        upgrade_txs
            .into_iter()
            .map(|tx| {
                BaseTxEnvelope::decode_2718(&mut tx.as_ref())
                    .expect("upgrade transaction must decode as BaseTxEnvelope")
            })
            .collect()
    }
    ...
    pub fn submit_l2_blocks(&mut self, blocks: Vec<BaseBlock>) {
        for block in blocks {
            self.queue_block_for_batcher(block);
        }
        self.flush_batcher_to_dap();
    }
            ...
            match self.pipeline.step(self.safe_head).await {
                StepResult::PreparedAttributes => {
                    ...
                    if let Some(attrs) = self.pipeline.next() {
                        ...
                        let l1_origin = l1_origin_from_attrs(&attrs).unwrap_or(parent.l1_origin);
    ...
    /// Mine `l1_blocks`, produce `l2_blocks` on a 2s cadence with dynamic epoch origins,
    /// encode with the real batcher encoder, and derive until idle.
    ...
    /// Produce and encode `n` L2 blocks into the DAP without deriving.
    ///
    /// Returns the total number of encoded frames emitted by the batcher.
    ...
    /// Return the number of batch frames submitted in the most recent epoch submission.
    pub const fn last_submitted_frame_count(&self) -> u64 {
        self.last_submitted_frame_count
    }
    ...
    fn parse_call_data(
        call_obj: &serde_json::Map<String, serde_json::Value>,
    ...
    pub fn rpc_request(&mut self, request_json: &str) -> String {
        ...
        let result: Result<serde_json::Value, (i64, String)> = match method {
            "eth_chainId" => {
                Ok(serde_json::json!(format!("0x{:x}", self.rollup_config.l2_chain_id.id())))
            }
            ...
            "eth_call" => (|| {
                ...
                let value =
                    call_obj.get("value").and_then(Self::parse_quantity_u256).unwrap_or(U256::ZERO);
            ...
            "eth_getLogs" => {
                ...
                let topics_filter =
                    filter.and_then(|f| f.get("topics")).and_then(serde_json::Value::as_array);
            ...
            "eth_sendRawTransaction" => {
                ...
                match raw
                    .and_then(|s| alloy_primitives::hex::decode(s.trim_start_matches("0x")).ok())
                {
