...
impl ExecutionGenesis {
    ...
    pub fn upgrades(config: &GenesisConfig) -> Result<UpgradeConfig> {
        ...
        for (upgrade, timestamp) in config
            .upgrades
            .iter()
            .filter_map(|(upgrade, timestamp)| timestamp.map(|timestamp| (upgrade, timestamp)))
        {
            upgrades.set_activation_timestamp(upgrade, timestamp);
        }
        // Jovian must not activate before a delayed Isthmus.
        if config.upgrades.isthmus_time.is_some_and(|timestamp| timestamp > 0) {
    ...
    pub fn deploy_config(config: &GenesisConfig) -> Result<Value> {
        ...
        input["fork"] =
            json!(if config.upgrades.isthmus_time.is_some_and(|timestamp| timestamp > 0) {
                3
            } else {
                5
            });
