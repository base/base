        ...
        loop {
            tokio::select! {
                ...
                result = listener.accept() => {
                    ...
                    // Spawn the handshake into its own task so the accept loop
                    // can immediately handle the next connection. A slow or
                    ...
                    tokio::spawn(async move {
                        ...
                        // is the sole owner, making the subsequent
                        // `lock().unwrap()` infallible.
                        ...
                        match accept_result {
                            Ok(stream) => {
                                ...
                                tokio::spawn(async move {
                                    ...
                                    metrics_clone.on_connection_closed(connected_at.elapsed());
...
mod tests {
    ...
    fn parse_resume_valid() {
        assert_eq!(parse_resume(Some("block_number=10&flashblock_index=3")), Some((10, 3)));
        assert_eq!(parse_resume(Some("flashblock_index=5&block_number=20")), Some((20, 5)));
    }
    ...
    async fn listener_with_resume_replays_buffered() {
        ...
        // Pre-populate ring buffer with two entries
        {
            ...
            buf.push(Some((1, 0)), Utf8Bytes::from("entry-1-0"));
            buf.push(Some((1, 1)), Utf8Bytes::from("entry-1-1"));
        }
        ...
        let (mut client, _) =
            connect_async(format!("ws://{addr}/?block_number=1&flashblock_index=0")).await.unwrap();
