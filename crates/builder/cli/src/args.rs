...
pub struct Args {
    ...
    /// Whether to run the stateless EIP-8130 manifest pre-check that drops
    /// transactions invalidated earlier in the block before executing them.
    /// Conservative (never admits invalid txs); disable only to forgo the
    /// fast-drop optimization.
    #[arg(long = "builder.eip8130-manifest-precheck", default_value_t = true)]
...
mod tests {
    ...
    fn default_args_produce_valid_config() {
        ...
        // The EIP-8130 manifest pre-check defaults on.
