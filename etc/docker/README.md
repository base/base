...
`Dockerfile.devnet` builds `base` with the optional `genesis` feature and a prepared Forge project from pinned `base/contracts`. The `base-genesis` shell workflow generates L1/L2 execution genesis, rollup configuration, beacon state, validator keys, and development network settings offline. Lighthouse libraries replace the Go beacon/key tools.
...
The setup image uses the interface-versioned tag `devnet-setup:local-v4`. System
...
Compose and system tests invoke the `base-genesis` shell workflow (`just genesis`
locally). It executes `base/contracts` scripts in Forge, calls Rust to compute the
final L2 output root, and replays L1
deployment with that anchor. It verifies that the deployed addresses are unchanged,
then assembles the final L1 header and Lighthouse beacon state. No L1 RPC or
transaction broadcasting is required during setup.

The normal devnet uses the real `ProtocolVersions` proxy deployed with the system.
Its schedule follows `BaseUpgrade::CONTRACT_VARIANTS`; genesis-only Zenith remains
outside that contract. `upgrade-signal.env` contains the generated proxy address.
`UPGRADE_SIGNAL_PREINSTALL=false` disables signal wiring for the Nitro variant and
system tests that deploy their own mock. The minimum version defaults to `4294967296`
...
[base-genesis](../../crates/infra/genesis/README.md).
...
then reuses the pinned contracts prepared by `just build genesis-contracts` and deploys the development
...
`just devnet up` initializes the L1 `ProtocolVersions` proxy in genesis, writes
`.devnet/genesis/l2/upgrade-signal.env`, and starts the normal L2 nodes in
`runtime-admin` upgrade-signal mode. Inspect the deployed contract with:

```bash
just devnet upgrade-signal status
```

The default upgrades activate shortly after genesis and are frozen by the real
contract, so they cannot be rescheduled. To exercise dynamic scheduling, start a
fresh devnet with Denim unset, then schedule it through the existing proxy:

```bash
L2_BASE_DENIM_BLOCK= just devnet up
just devnet upgrade-signal-future denim 120
```

These commands reuse the address in `upgrade-signal.env`. `upgrade-signal-future`
adds the offset after the later of the latest L2 timestamp and L1's earliest
allowed activation (currently one hour of notice). Explicit timestamps can be
set with `just devnet upgrade-signal set denim <unix-timestamp>` and must satisfy
the same notice, freeze, and upgrade-order constraints.
