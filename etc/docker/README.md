...
`Dockerfile.nitro-enclave` remains separate because it has different toolchain and runtime requirements.
...
To exercise Zenith validity transactions on the native payload builder, the
deployment must schedule Zenith and configure both sides of the forwarding path:
