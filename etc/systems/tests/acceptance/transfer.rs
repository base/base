...
impl Transfer {
    ...
    pub async fn send(
        ...
        provider.wait_for_receipt(transaction_hash, Duration::from_secs(60)).await.wrap_err_with(
            || format!("transfer {transaction_hash} to {} for {} wei", transfer.recipient, transfer.value),
        )?;
        let receipt =
            rpc.call(sequencer_url, "eth_getTransactionReceipt", json!([transaction_hash])).await?;
        ensure!(
            ...
            receipt["from"] == json!(signer.address()) && receipt["to"] == json!(transfer.recipient),
        ...
        let full = provider
            ...
            calculate_transaction_root(&full.body.transactions) == block.header.transactions_root,
            "receipt block's transaction body does not match its authenticated root"
        );
        ensure!(
            full.body.transactions.iter().any(
                |transaction| transaction.encoded_2718().as_slice() == raw_transaction.as_ref()
            ),
        ...
        let l1_origin = rpc.header(l1_url, json!(format!("{:#x}", info.l1_origin.number))).await?;
        ensure!(info.l1_origin.hash == l1_origin.hash, "L2 origin differs from canonical L1");
        let balance_before = rpc.balance(sequencer_url, transfer.recipient, parent.hash).await?;
        ...
        }).await.wrap_err_with(|| format!("transfer submission and receipt-block observations timed out for {} to {} after {within:?}", transfer.value, transfer.recipient))?
    }
