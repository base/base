...
use super::{
    ...
    InProcessFollowConsensus, InProcessFollowConsensusConfig, InProcessStandaloneConsensus,
    InProcessStandaloneConsensusConfig, L2ContainerConfig, SnapshotBoundary,
};
...
pub struct SnapshotL2Stack {
    ...
    standalone_consensus: Option<InProcessStandaloneConsensus>,
...
impl SnapshotL2Stack {
    ...
    pub async fn start_sequencer(config: SnapshotL2StackConfig) -> Result<Self> {
        ...
        let builder = InProcessBuilder::start(InProcessBuilderConfig {
            genesis_json: Vec::new(),
            chain_spec: Some(Arc::clone(&chain_spec)),
            ...
            flashblocks_enabled: block_interval == DevnetBlockInterval::TwoSeconds,
        ...
        let mut standalone_consensus =
            InProcessStandaloneConsensus::start(InProcessStandaloneConsensusConfig {
                rollup_config: rollup_config.as_ref().clone(),
                jwt_secret,
    ...
    pub fn anchored_rollup_config(
        ...
        if block_interval == DevnetBlockInterval::TwoHundredMilliseconds {
            config.set_upgrade_activation_timestamp(BaseUpgrade::Zenith, first_block_timestamp);
        }
    ...
    pub fn chain_spec(
        ...
        if block_interval == DevnetBlockInterval::TwoHundredMilliseconds {
            chain_spec
                .set_fork(BaseUpgrade::Zenith, ForkCondition::Timestamp(first_block_timestamp));
        }
        chain_spec
    }
...
mod tests {
    ...
    #[test]
    fn subsecond_cl_and_el_activate_at_same_timestamp() {
        ...
        assert!(!rollup.is_zenith_active(activation - 1));
        assert!(rollup.is_zenith_active(activation));
        assert!(!chain_spec.is_zenith_active_at_timestamp(activation - 1));
        assert!(chain_spec.is_zenith_active_at_timestamp(activation));
        assert_eq!(chain_spec.fork(BaseUpgrade::Zenith), ForkCondition::Timestamp(activation));
    }
}
