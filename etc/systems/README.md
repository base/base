...
`base-bench` generates an ephemeral funder key for each run. If you explicitly override it with
`--funder-key` or `FUNDER_KEY`, do not use the standard Anvil development key. Its address has
delegated-account state at the snapshot tested during development, which can trigger Reth's
delegated-account in-flight limit while funding senders.
...
the stack down. It does not own the snapshot clones. Pass `--funder-key` or set `FUNDER_KEY` only
when a stable key is useful for debugging.
...
cargo run --release -p base-system-tests --bin base-bench -- snapshot \
  ...
  --output results/account-create-2s.json \
  --benchmark-run snapshot-throughput \
  --visualizer-output-dir results/account-create-2s
```
...
`--visualizer-output-dir` additionally writes the `base/benchmark` report contract directly:
`metadata.json`, `metrics-sequencer.json`, `metrics-validator.json`, and
`load-test-result.json`. Set `BASE_BENCH_CLIENT_VERSION` to a stable build label when the report
...
visualizer output directory is one self-contained report run. Give comparable invocations the same
`--benchmark-run` cohort and a unique
`--visualizer-output-dir`; the directory name becomes the run ID unless `--run-id` is set. Upload
...
- **Address delegation or funding errors with an override:** omit `FUNDER_KEY` to use an ephemeral
  signer, or supply a newly generated throwaway key instead of the standard Anvil key.
