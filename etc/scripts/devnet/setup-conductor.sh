...
wait_for_rpc() {
  local url="$1"
  local name="$2"
  local method="${3:-conductor_leader}"
  local ready="${4:-.result != null}"
  local max_retries=120
  local count=0
  echo "Waiting for $name at $url..."
  until curl -fsS --max-time 2 -X POST "$url" \
    -H 'Content-Type: application/json' \
    -d "{\"jsonrpc\":\"2.0\",\"method\":\"$method\",\"params\":[],\"id\":1}" \
    | jq -e "$ready" >/dev/null 2>&1; do
    count=$((count + 1))
...
wait_for_rpc "$CONDUCTOR0_URL" "initial Raft leader" conductor_leader '.result == true'
...
curl -s -X POST "$CONDUCTOR0_URL" \
  -H 'Content-Type: application/json' \
  -d '{"jsonrpc":"2.0","method":"conductor_clusterMembership","params":[],"id":1}' \
  | jq -e 'if (.result.servers | length) == 3 then . else error("expected three Raft voters") end'

# The batchers immediately fetch rollup configuration through this leader-only proxy.
wait_for_rpc "$CONDUCTOR0_URL" "leader rollup proxy" optimism_rollupConfig '.result.l2_chain_id != null'
