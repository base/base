...
ACTIVATION_OFFSET="${UPGRADE_SIGNAL_ACTIVATION_OFFSET:-30}"
...
ACTIVATION_TIMESTAMP="${UPGRADE_SIGNAL_ACTIVATION_TIMESTAMP:-$((L2_TIMESTAMP + ACTIVATION_OFFSET))}"
...
latest L2 timestamp:   $L2_TIMESTAMP
...
Restart the observer services with:
  docker compose --env-file etc/docker/devnet-env --env-file .devnet/upgrade-signal.env -f etc/docker/docker-compose.yml -f etc/docker/docker-compose.upgrade-signal.yml up -d --no-build --no-deps --force-recreate base-builder-cl base-client base-client-cl

Watch logs with:
  docker compose --env-file etc/docker/devnet-env --env-file .devnet/upgrade-signal.env -f etc/docker/docker-compose.yml -f etc/docker/docker-compose.upgrade-signal.yml logs -f base-builder-cl base-client base-client-cl
