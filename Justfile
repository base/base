...
build-builder:
    cargo build -p op-rbuilder -p p2p
...
test-builder:
    ...
    cargo test -p op-rbuilder -p p2p --verbose

# Runs clippy on builder crates (using nightly, matching op-rbuilder's original)
check-clippy-builder:
    cargo +nightly clippy -p op-rbuilder -p p2p --all-features -- -D warnings

# Fixes formatting for builder crates
format-builder:
    cargo +nightly fmt -p op-rbuilder -p p2p
