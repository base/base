...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("cargo:rustc-env=BASE_UNIFIED_VERSION_SUFFIX={version_suffix}");
    ...
    println!("cargo:rustc-env=BASE_UNIFIED_BUILD_PROFILE={profile}");
    ...
    println!(
        "cargo:rustc-env=BASE_UNIFIED_SHORT_VERSION={pkg_version}{version_suffix} ({sha_short})"
    );
    ...
    // ```
    println!("cargo:rustc-env=BASE_UNIFIED_LONG_VERSION_0=Version: {pkg_version}{version_suffix}");
    println!("cargo:rustc-env=BASE_UNIFIED_LONG_VERSION_1=Commit SHA: {sha}");
    println!(
        "cargo:rustc-env=BASE_UNIFIED_LONG_VERSION_2=Build Timestamp: {}",
        env::var("VERGEN_BUILD_TIMESTAMP")?
    );
    println!(
        "cargo:rustc-env=BASE_UNIFIED_LONG_VERSION_3=Build Features: {}",
        if features.is_empty() { "no features enabled".to_string() } else { features }
    );
    println!("cargo:rustc-env=BASE_UNIFIED_LONG_VERSION_4=Build Profile: {profile}");
