...
balance slots plus the corresponding storage trie nodes.
...
The tool writes directly to the flat state tables and then rebuilds the affected storage
tries, so the resulting datadir is a valid target for `reth`/`base-reth` with a correct
state root.
...
| Command | Description |
|---------|-------------|
| `populate` | Write balance slots, hashed storage, and storage trie nodes for a contract |
...
Pre-seed a load test's sender addresses with balances:
...
- Writes go to `PlainStorageState` and `HashedStorages`.
...
- Storage trie nodes are computed via `StorageRoot::from_tx_hashed` and written to `StoragesTrie`,
  yielding a correct storage root.
