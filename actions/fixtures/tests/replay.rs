...
use base_action_fixtures::{ActionFixtureCatalog, DerivationFixtureReplayer};
...
async fn captured_l1_data_derives_expected_l2_payloads() {
    ...
    for (payload, fixture_block) in payloads.iter().zip(&fixture.l2_blocks) {
        ...
        assert_eq!(
            ...
            (derivation.safe_head.l1_origin.number, derivation.safe_head.l1_origin.hash),
        );
        assert!((l1_origin.number..=l1_end.number).contains(&derived_from.number));
        assert_eq!(payload.attributes.transactions.as_ref(), Some(&fixture_block.transactions));
    }
}
