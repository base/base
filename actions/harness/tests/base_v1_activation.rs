...
///
/// Identical to `MODEXP_INIT_CODE` except the STATICCALL target is `PUSH2 0x0100`
...
async fn base_v1_p256_verify_gas_cost_increase() {
    ...
    let gas_delta_pre;
    {
        ...
        let sentinel =
            acct.storage.get(&P256_SENTINEL_SLOT).copied().unwrap_or(U256::ZERO);
        let success =
            acct.storage.get(&P256_SUCCESS_SLOT).copied().unwrap_or(U256::ZERO);
        gas_delta_pre =
            acct.storage.get(&P256_GAS_DELTA_SLOT).copied().unwrap_or(U256::ZERO);
