...
use base_action_harness::{
    ActionTestHarness, BatcherConfig, BuilderBackedEngineClient, L1MinerConfig, L2Sequencer,
...
-> eyre::Result<(ActionTestHarness, L2Sequencer<BuilderBackedEngineClient>)> {
...
async fn builder_backed_sequencer_includes_in_window_bundle() -> eyre::Result<()> {
    let (h, mut sequencer) = wall_clock_builder_sequencer().await?;
