//! Derivation-layer rejection of pre-Cobalt EIP-8130 transactions.
...
///
/// Setup (`block_time` = 2, Cobalt at ts = 8 → L2 block 4):
/// ```text
...
///   L1 block 3: batch for L2 block 3 (EIP-8130, ts = 6) → DROPPED (pre-Cobalt)
///   L1 block 4: empty (closes epoch-0 window)           → block 3 force-included
///                                                          as deposit-only
/// ```
#[tokio::test]
async fn eip8130_batch_is_dropped_before_cobalt() {
    ...
    // Build three pre-Cobalt L2 blocks. Blocks 1-2 carry a normal user tx; block
    ...
    assert_eq!(
        node.l2_safe_number(),
        2,
        "blocks 1-2 derive; block 3's EIP-8130 batch is dropped pre-Cobalt"
    );
    ...
    assert_eq!(
        user_txs(3),
        Some(0),
        "block 3: EIP-8130 batch dropped pre-Cobalt -> deposit-only, 0 user txs"
    );
}
