...
impl ArenaSparseSubtrie {
    ...
    fn update_cached_rlp(&mut self, new_epoch: TrieNodeEpoch) {
    ...
    fn maybe_collapse_or_remove_branch(&mut self, cursor: &mut ArenaCursor) {
        loop {
            ...
            let Some(child_idx) = remaining_child_idx else {
                debug_assert!(
                    false,
                    "single remaining child is blinded — should have been caught by check_subtrie_collapse_needs_proof"
                );
                return;
            };
    ...
    fn set_root(
        ...
        match root {
            ...
            TrieNodeV2::Extension(_) => {
                panic!(
                    "set_root does not support Extension nodes; extensions are represented as branches with a short_key"
                )
            }
        }
