...
pub(super) fn test_set_root_with_branch_node(new_trie: fn() -> ArenaParallelSparseTrie) {
...
pub(super) fn test_set_root_with_leaf_node(new_trie: fn() -> ArenaParallelSparseTrie) {
...
/// After `set_root` + `reveal_nodes`, `root()` must match the reference hash.
pub(super) fn test_set_root_with_extension_node(new_trie: fn() -> ArenaParallelSparseTrie) {
...
pub(super) fn test_set_root_does_not_retain_updates_when_not_requested(
    new_trie: fn() -> ArenaParallelSparseTrie,
) {
    let mut key_a = B256::ZERO;
    key_a.0[0] = 0x10;
...
pub(super) fn test_set_root_with_empty_root(new_trie: fn() -> ArenaParallelSparseTrie) {
    let mut trie = (new_trie)();
    trie.set_root(TrieNodeV2::EmptyRoot, None, true).expect("set_root should succeed");
    assert_eq!(trie.root(epoch(0)), EMPTY_ROOT_HASH);
}
