...
mod rpc_compat {
    ...
    impl RecoveredBlock {
        ...
        /// # Examples
        /// ```ignore
        /// let rpc_block: base_common_types_rpc::Block = get_rpc_block();
        /// let recovered = RecoveredBlock::from_rpc_block(rpc_block)?;
        /// ```
        pub fn from_rpc_block<U>(
            block: base_common_types_rpc::Block<U>,
    ...
    impl<U> TryFrom<base_common_types_rpc::Block<U>> for RecoveredBlock
    where
        BaseTxEnvelope: From<U>,
    {
        type Error = BlockRecoveryError<BaseBlock>;

        fn try_from(block: base_common_types_rpc::Block<U>) -> Result<Self, Self::Error> {
