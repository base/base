...
impl From<const_hex::FromHexError> for AddressError {
    #[inline]
...
impl Address {
    ...
    pub fn parse_checksummed<S: AsRef<str>>(
        ...
        fn parse_checksummed(s: &str, chain_id: Option<u64>) -> Result<Address, AddressError> {
            ...
            if !s.starts_with("0x") {
                return Err(AddressError::Hex(const_hex::FromHexError::InvalidStringLength));
            }
    ...
    fn to_checksum_inner(&self, buf: &mut [u8; 42], chain_id: Option<u64>) {
        buf[0] = b'0';
        buf[1] = b'x';
        const_hex::encode_to_slice(self, &mut buf[2..]).unwrap();
    ...
    /// Converts a [`secp256k1_0_31_1::PublicKey`] to its corresponding Ethereum address.
    #[inline]
    #[cfg(feature = "secp256k1")]
    pub fn from_secp256k1_public_key(pubkey: &secp256k1_0_31_1::PublicKey) -> Self {
        Self::from_raw_public_key(&pubkey.serialize_uncompressed()[1..])
    }

    /// Converts a [`secp256k1_0_31_1::SecretKey`] to its corresponding Ethereum address.
    #[inline]
    #[cfg(feature = "secp256k1")]
    pub fn from_secp256k1_secret_key(secret_key: &secp256k1_0_31_1::SecretKey) -> Self {
        let secp = secp256k1_0_31_1::Secp256k1::signing_only();
        let pubkey = secp256k1_0_31_1::PublicKey::from_secret_key(&secp, secret_key);
...
mod tests {
    ...
    fn create2() {
        ...
        for (from, salt, init_code, expected) in tests {
            ...
            let salt = const_hex::decode(salt).unwrap();
            let salt: [u8; 32] = salt.try_into().unwrap();

            let init_code = const_hex::decode(init_code).unwrap();
    ...
    fn test_raw_public_key_to_address() {
        ...
        let pubkey_bytes = const_hex::decode("76698beebe8ee5c74d8cc50ab84ac301ee8f10af6f28d0ffd6adf4d6d3b9b762d46ca56d3dad2ce13213a6f42278dabbb53259f2d92681ea6a0b98197a719be3").unwrap();
