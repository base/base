...
impl Display for Signature {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "0x{}", const_hex::encode(self.as_bytes()))
    }
}
...
impl FromStr for Signature {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Self::from_raw_array(&const_hex::decode_to_array(s)?)
    }
}
...
impl From<secp256k1_0_31_1::ecdsa::RecoverableSignature> for Signature {
    fn from(value: secp256k1_0_31_1::ecdsa::RecoverableSignature) -> Self {
        let (recid, bytes) = value.serialize_compact();
        let r = U256::from_be_slice(&bytes[..32]);
        let s = U256::from_be_slice(&bytes[32..]);
        let v = matches!(
            recid,
            secp256k1_0_31_1::ecdsa::RecoveryId::One | secp256k1_0_31_1::ecdsa::RecoveryId::Three
        );
        Self { y_parity: v, r, s }
    }
}
...
impl Signature {
    ...
    pub fn to_secp256k1(
        &self,
    ) -> Result<secp256k1_0_31_1::ecdsa::RecoverableSignature, secp256k1_0_31_1::Error> {
        let mut bytes = [0u8; 64];
        bytes[..32].copy_from_slice(&self.r.to_be_bytes::<32>());
        bytes[32..].copy_from_slice(&self.s.to_be_bytes::<32>());
        secp256k1_0_31_1::ecdsa::RecoverableSignature::from_compact(&bytes, self.secp256k1_recid())
    }
    ...
    /// Returns the recovery ID as a [`secp256k1_0_31_1::ecdsa::RecoveryId`].
    #[cfg(feature = "secp256k1")]
    #[inline]
    pub fn secp256k1_recid(&self) -> secp256k1_0_31_1::ecdsa::RecoveryId {
        if self.y_parity {
            secp256k1_0_31_1::ecdsa::RecoveryId::One
        } else {
            secp256k1_0_31_1::ecdsa::RecoveryId::Zero
        }
    }
    ...
    /// Recovers a [`secp256k1_0_31_1::PublicKey`] from this signature and the given message by first
    ...
    pub fn recover_from_msg_secp256k1<T: AsRef<[u8]>>(
        &self,
        msg: T,
    ) -> Result<secp256k1_0_31_1::PublicKey, SignatureError> {
        self.recover_from_prehash_secp256k1(&crate::eip191_hash_message(msg))
    }

    /// Recovers a [`secp256k1_0_31_1::PublicKey`] from this signature and the given prehashed message.
    #[cfg(feature = "secp256k1")]
    #[inline]
    pub fn recover_from_prehash_secp256k1(
        ...
        let msg = secp256k1_0_31_1::Message::from_digest(prehash.0);
        #[cfg(feature = "std")]
        {
            secp256k1_0_31_1::SECP256K1.recover_ecdsa(msg, &sig).map_err(Into::into)
        }
        #[cfg(not(feature = "std"))]
        {
            let secp = secp256k1_0_31_1::Secp256k1::verification_only();
...
mod tests {
    ...
    fn bincode_roundtrip() {
        ...
        let bin = bincode_1_3_3::serialize(&signature).unwrap();
        assert_eq!(bincode_1_3_3::deserialize::<Signature>(&bin).unwrap(), signature);
    }
    ...
    fn signature_rlp_encode() {
        ...
        assert_eq!(const_hex::encode(&buf), expected);
    }
