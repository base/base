...
use crate::{BlsPublicKey, BlsSignature, header::BeaconBlockHeader, requests::ExecutionRequestsV4};
...
mod ssz_impls {
    ...
    fn fixed_part_len<T: ssz::Encode>() -> usize {
        if T::is_ssz_fixed_len() { T::ssz_fixed_len() } else { ssz::BYTES_PER_LENGTH_OFFSET }
    }
