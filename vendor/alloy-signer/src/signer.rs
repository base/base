...
use alloy_primitives::{Address, B256, ChainId, Signature, eip191_hash_message};
#[cfg(feature = "eip712")]
...
mod tests {
    ...
    async fn unimplemented() {
        ...
        async fn test_unimplemented_signer<S: Signer + SignerSync + Send + Sync>(s: &S) {
            ...
            #[cfg(feature = "eip712")]
            assert!(
                s.sign_typed_data_sync(&Eip712Data::default(), &Eip712Domain::default()).is_err()
            );
            #[cfg(feature = "eip712")]
            assert!(
                s.sign_typed_data(&Eip712Data::default(), &Eip712Domain::default()).await.is_err()
            );
        }
