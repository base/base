    ...
    ($target:ty $(where [$($generics:tt)*])?) => {
        $crate::macros::delegate_impls_to_as_ref!(
            ...
            HashedPostStateProvider $(where [$($generics)*])? {
                fn hashed_post_state(&self, bundle_state: &base_execution_evm_runtime::database::BundleState) -> reth_storage_api::errors::provider::ProviderResult<reth_trie::HashedPostState>;
            }
        );
