    ...
    ($int: ty) => {
            ...
            T: From<$int> + Into<$int> + Copy + TryFrom<u64>,
        {
            ...
            where
                E: serde::de::Error,
            {
                s.parse::<$int>().map(T::from).map_err(serde::de::Error::custom)
            }
        ...
        {
            deserializer
                .deserialize_any(QuotedIntVisitor { require_quotes: false, _phantom: PhantomData })
        }
...
mod test {
    ...
    #[test]
    fn u256_with_quotes() {
        assert_eq!(&serde_json::to_string(&WrappedU256(U256::from(1))).unwrap(), "\"1\"");
        assert_eq!(
    ...
    fn negative_i64_with_quotes() {
        assert_eq!(serde_json::from_str::<WrappedI64>("\"-200\"").unwrap().0, -200);
        assert_eq!(serde_json::to_string(&WrappedI64(-12_500)).unwrap(), "\"-12500\"");
    }
