...
impl<T> FrameStack<T> {
    ...
    pub fn new() -> Self {
        ...
        Self { stack: Vec::with_capacity(8), index: None }
    }
    ...
    pub unsafe fn push(&mut self, token: FrameToken) {
        ...
        *index += 1;
        ...
        debug_assert!(*index < self.stack.capacity(), "Stack capacity is not enough for index");
...
impl<'a, T> OutFrame<'a, T> {
    ...
    pub unsafe fn new_maybe_uninit(ptr: *mut T, init: bool) -> Self {
        Self { ptr, init, lt: Default::default() }
    }
