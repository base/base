...
pub fn pop<H: ?Sized>(context: Ictx<'_, H>) -> Result {
...
pub fn push0<H: ?Sized>(context: Ictx<'_, H>) -> Result {
...
pub fn push<const N: usize, H: ?Sized>(context: Ictx<'_, H>) -> Result {
    let slice = context.interpreter.bytecode.read_slice(N);
    if context.interpreter.stack.push_slice(slice).is_err() {
...
pub fn dup<const N: usize, H: ?Sized>(context: Ictx<'_, H>) -> Result {
...
pub fn swap<const N: usize, H: ?Sized>(context: Ictx<'_, H>) -> Result {
    assert!(N != 0);
...
pub fn dupn<H: ?Sized>(context: Ictx<'_, H>) -> Result {
...
pub fn swapn<H: ?Sized>(context: Ictx<'_, H>) -> Result {
    check!(context.interpreter, AMSTERDAM);
    let x: usize = context.interpreter.bytecode.read_u8().into();
    if let Some(n) = decode_single(x) {
        if !context.interpreter.stack.exchange(0, n) {
...
pub fn exchange<H: ?Sized>(context: Ictx<'_, H>) -> Result {
    check!(context.interpreter, AMSTERDAM);
    let x: usize = context.interpreter.bytecode.read_u8().into();
