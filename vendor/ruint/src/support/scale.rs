...
impl<const BITS: usize, const LIMBS: usize> Decode for CompactUint<BITS, LIMBS> {
    fn decode<I: Input>(input: &mut I) -> Result<Self, Error> {
        ...
        Ok(Self(match prefix % 4 {
            ...
            1 => {
                let x = u16::decode(&mut PrefixInput { prefix: Some(prefix), input })? >> 2; // right shift to remove mode bits
            ...
            2 => {
                let x = u32::decode(&mut PrefixInput { prefix: Some(prefix), input })? >> 2; // right shift to remove mode bits
            ...
            _ => match (prefix >> 2) + 4 {
                ...
                bytes => {
                    let le_byte_slice =
                        (0..bytes).map(|_| input.read_byte()).collect::<Result<Vec<_>, _>>()?;
                    ...
                    if bits > 0 {
                        new_limbs[limbs - 1] &=
                            if bits.is_multiple_of(64) { u64::MAX } else { (1 << (bits % 64)) - 1 }
                    }
