...
impl<const BITS: usize, const LIMBS: usize> Visitor<'_> for ByteVisitor<BITS, LIMBS> {
    ...
    fn visit_bytes<E: Error>(self, value: &[u8]) -> Result<Self::Value, E> {
        ...
        Uint::try_from_be_slice(value).ok_or_else(|| {
            Error::invalid_value(Unexpected::Other(&format!("too large for Uint<{BITS}>")), &self)
        })
    }
}
...
mod tests {
    ...
    fn test_serde_machine_readable() {
        const_for!(BITS in SIZES {
            const LIMBS: usize = nlimbs(BITS);
            proptest!(|(value: Uint<BITS, LIMBS>)| {
                let serialized = bincode_1_3_3::serialize(&value).unwrap();
                let deserialized = bincode_1_3_3::deserialize::<Uint<BITS, LIMBS>>(&serialized[..]).unwrap();
                assert_eq!(value, deserialized);
            });
            proptest!(|(value: Bits<BITS, LIMBS>)| {
                let serialized = bincode_1_3_3::serialize(&value).unwrap();
                let deserialized = bincode_1_3_3::deserialize::<Bits<BITS, LIMBS>>(&serialized[..]).unwrap();
                assert_eq!(value, deserialized);
            });
        });
    }
