...
impl<const BITS: usize, const LIMBS: usize> Decodable for Uint<BITS, LIMBS> {
    fn decode(s: &Rlp) -> Result<Self, DecoderError> {
        Self::try_from_be_slice(s.data()?)
            .ok_or(DecoderError::Custom("RLP integer value too large for Uint."))
    }
}
...
impl<const BITS: usize, const LIMBS: usize> Decodable for Bits<BITS, LIMBS> {
    fn decode(s: &Rlp) -> Result<Self, DecoderError> {
        s.decoder().decode_value(|bytes| match bytes.len().cmp(&Self::BYTES) {
...
mod test {
    ...
    fn test_bits_rlp() {
        ...
        assert_eq!(
            "0xef2d6d194084c2de36e0dabfce45d046b37d1106".parse::<B160>().unwrap().rlp_bytes()[..],
