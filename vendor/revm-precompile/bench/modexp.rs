...
enum ModexpCase {
    Encoded { name: &'static str, base: Vec<u8>, exponent: Vec<u8>, modulus: Vec<u8> },
    Raw { name: &'static str, input: &'static str },
}
...
const fn encoded(
    ...
    ModexpCase::Encoded { name, base, exponent, modulus }
}
...
fn modexp_cases() -> Vec<ModexpCase> {
    vec![
        ...
        encoded(
            ...
            repeated(0xff, 1024),
            vec![0x03],
            (0..32).flat_map(|_| [vec![0x00], repeated(0xff, 31)].concat()).collect(),
        ),
    ]
}
