...
use crate::{
    history_shards::{
        ShardedHistoryTable, prepare_history_shard_writes_parallel,
        prepare_history_shard_writes_serial,
    },
    providers::{HistoryInfo, compute_history_rank, needs_prev_shard_check},
};
...
mod tests {
    ...
    fn test_with_default_tables_registers_required_column_families() {
        ...
        assert!(
            !column_families.iter().any(|name| name == tables::BlockAccessListBlockNumbers::NAME)
        );
    }
