...
use reth_era_downloader::{EraClient, EraMeta, EraStream, EraStreamConfig, read_dir, read_era_dir};
...
/// The ERA history stage.
///
/// Imports block headers and bodies from
/// [ERA1](https://github.com/eth-clients/e2store-format-specs/blob/main/formats/era1.md) files,
/// which hold pre-merge execution blocks, and
/// [ERE](https://github.com/eth-clients/e2store-format-specs/blob/main/formats/ere.md) files,
/// which hold both pre-merge and post-merge execution blocks, and consensus-layer
/// [ERA](https://github.com/eth-clients/e2store-format-specs/blob/main/formats/era.md) files, whose
/// post-merge beacon blocks embed an execution payload. The reader is selected per file from its
/// extension (`.era` vs `.era1` vs `.ere`/`.erae`).
///
/// Imports block headers and bodies up to the configured target block. Receipts are generated by
/// execution, which is not done in this stage.
    ...
    fn create(self, input: ExecInput) -> Result<ThreadSafeEraStream<Header, Body>, StageError> {
        match self {
            ...
            Self::Url(url, folder) => {
                ...
                // `start_from` maps a block number to a file index, valid only for block-indexed
                // `.era1`/`.ere`. Consensus `.era` files are slot-indexed, so stream from 0 and let
                // the pipeline skip already-imported blocks.
                let is_era = EraFileType::from_url(url.as_str()) == EraFileType::Era;
                let client = EraClient::new(Client::new(), url, folder);
                let mut config = EraStreamConfig::default();
                if !is_era {
                    config = config.start_from(input.next_block());
                }

                Self::convert(EraStream::new(client, config))
            }
        }
    }
}
...
mod tests {
    ...
    use reth_era::{
        common::file_ops::{EraFileFormat, StreamWriter},
        ere::{
            file::{EreFile, EreWriter},
            types::{
                execution::{BlockTuple, CompressedBody, CompressedHeader},
    ...
    use reth_testing_utils::generators::{
        self, BlockRangeParams, random_block_range, random_header,
    };
    ...
    /// Writes `blocks` to a header+body-only ERE file at `path`.
    fn write_ere_file(path: &Path, blocks: &[SealedBlock<Block>]) {
        let tuples = blocks
            .iter()
            .map(|block| {
                BlockTuple::new(
                    CompressedHeader::from_header(block.header()).unwrap(),
                    CompressedBody::from_body(block.body()).unwrap(),
                )
            })
            .collect::<Vec<_>>();

        let start = blocks[0].number;
        // Header + body only is component-count 2. The index offsets are stored verbatim and not
        // interpreted when assembling blocks, so placeholder values are fine.
        let component_count = 2;
        let offsets = vec![0; blocks.len() * component_count as usize];
        let index = DynamicBlockIndex::new(start, component_count, offsets);
        let group = EreGroup::new(tuples, None, index);
        let file = EreFile::new(group, EreId::new("mainnet", start, blocks.len() as u32));

        EreWriter::new(File::create(path).unwrap()).write_file(&file).unwrap();
    }
    ...
    #[tokio::test]
    async fn convert_decodes_ere_files() {
        ...
        // ERE files are published with either the `.ere` or `.erae` extension; both must be
        // recognized and routed to the ERE reader rather than decoded as era1.
        for ext in ["ere", "erae"] {
            let path = dir.path().join(format!("mainnet-00000-abcd1234.{ext}"));
            write_ere_file(&path, &blocks);

            let stream =
                stream::iter(vec![Ok::<_, eyre::Error>(EreTestMeta { path: path.clone() })]);
            let mut stream =
                EraImportSource::convert::<Header, BlockBody<TransactionSigned>>(stream).unwrap();

            let item = stream.next().await.expect("a file to decode").expect("decoding to succeed");
            let decoded = item.collect::<eyre::Result<Vec<_>>>().unwrap();

            assert_eq!(
                decoded.len(),
                blocks.len(),
                "ERE file with `.{ext}` extension should decode every block"
            );
            for ((header, body), block) in decoded.iter().zip(&blocks) {
