...
impl MerkleHasher {
    ...
    fn process_left_node(&mut self, id: usize, preimage: Preimage<'_>) {
        self.half_nodes.push(HalfNode::new(get_parent(id), preimage))
    }
...
mod test {
    ...
    #[test]
    fn context_size() {
        assert_eq!(mem::size_of::<HalfNode>(), 232, "Halfnode size should be as expected");
    }

    fn compare_with_reference(leaves: &[Hash256], depth: usize) {
        let reference_bytes =
            leaves.iter().flat_map(|hash| hash.as_slice()).copied().collect::<Vec<_>>();
        ...
        assert_eq!(reference_root, merklizer_root_32_bytes, "32 bytes should match reference root");
    ...
    fn compare_reference_with_len(leaves: u64, depth: usize) {
        let leaves = (0..leaves).map(|leaf| Hash256::from(U256::from(leaf))).collect::<Vec<_>>();
    ...
    fn compare_new_with_leaf_count(num_leaves: u64, depth: usize) {
        let leaves =
            (0..num_leaves).map(|leaf| Hash256::from(U256::from(leaf))).collect::<Vec<_>>();
        ...
        let from_num_leaves = {
            ...
            for leaf in leaves.iter() {
                m.process_leaf(leaf.as_slice()).expect("should process leaf");
            }
            m.finish()
        };
