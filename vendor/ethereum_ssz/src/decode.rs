...
impl<'a> SszDecoderBuilder<'a> {
    ...
    pub fn new(bytes: &'a [u8]) -> Self {
        Self { bytes, items: smallvec![], offsets: smallvec![], items_index: 0 }
    }
    ...
    pub fn register_type_parameterized(
        ...
        if is_ssz_fixed_len {
            ...
            let slice =
                self.bytes.get(start..self.items_index).ok_or(DecodeError::InvalidByteLength {
                    len: self.bytes.len(),
...
pub fn split_union_bytes(bytes: &[u8]) -> Result<(UnionSelector, &[u8]), DecodeError> {
    ...
    let body = bytes.get(1..).ok_or(DecodeError::OutOfBoundsByte { i: 1 })?;
    Ok((selector, body))
}
...
pub fn read_offset(bytes: &[u8]) -> Result<usize, DecodeError> {
    decode_offset(bytes.get(0..BYTES_PER_LENGTH_OFFSET).ok_or(
        DecodeError::InvalidLengthPrefix { len: bytes.len(), expected: BYTES_PER_LENGTH_OFFSET },
    )?)
}
