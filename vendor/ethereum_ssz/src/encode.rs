...
impl<'a> SszEncoder<'a> {
    ...
    pub fn container(buf: &'a mut Vec<u8>, num_fixed_bytes: usize) -> Self {
        ...
        Self { offset: num_fixed_bytes, buf, variable_bytes: vec![] }
    }
        ...
        F: Fn(&mut Vec<u8>),
    {
        if is_ssz_fixed_len {
            ...
            self.buf.extend_from_slice(&encode_length(self.offset + self.variable_bytes.len()));
...
mod tests {
    ...
    fn test_encode_length() {
        ...
        assert_eq!(encode_length(MAX_LENGTH_VALUE), [255; BYTES_PER_LENGTH_OFFSET]);
    }
