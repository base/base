...
use revm_context::{block::BlockEnv, cfg::CfgEnv};
use revm_database::CacheState;
use revm_primitives::{AddressMap, B256, Bytes, hardfork::SpecId, keccak256};
...
impl TestUnit {
    ...
    pub fn state(&self) -> CacheState {
        ...
        for (address, info) in &self.pre {
            ...
            let acc_info = revm_state::AccountInfo {
    ...
    pub fn block_env(&self, cfg: &mut CfgEnv) -> BlockEnv {
        let mut block = BlockEnv {
            ...
            slot_num: self.env.slot_number.unwrap_or_default().try_into().unwrap_or(u64::MAX),
            ..BlockEnv::default()
        };
...
#[cfg(test)]
mod tests {
    use revm_context_interface::block::calc_blob_gasprice;
    use revm_primitives::{
        Address, U256,
    ...
    fn test_block_env_blob_fee_fraction_cancun() {
        ...
        let blob_info = block.blob_excess_gas_and_price.expect("blob info should be set");
    ...
    fn test_blob_fee_difference_affects_tx_validity() {
        ...
        assert!(max_fee_per_blob_gas < cancun_price, "Tx should fail with Cancun fraction");
        assert!(max_fee_per_blob_gas >= prague_price, "Tx should succeed with Prague fraction");
    }
}
