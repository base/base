...
use revm_primitives::{AddressMap, B256, Bytes, TxKind};
...
impl Test {
    ...
    pub fn tx_env(&self, unit: &TestUnit) -> Result<TxEnv, TestError> {
        ...
        let tx = TxEnv {
            ...
            chain_id: Some(
                unit.transaction.chain_id.map(|id| id.try_into().unwrap_or(u64::MAX)).unwrap_or(1),
            ),
            ...
            authorization_list: unit
                ...
                .map(|auth_list| {
                    auth_list
                        .into_iter()
                        .map(|i| revm_context::either::Either::Left(i.into()))
                        .collect::<Vec<_>>()
                })
                .unwrap_or_default(),
