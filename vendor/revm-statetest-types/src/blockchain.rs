...
use revm_context::{BlockEnv, TxEnv, transaction::AccessList};
use revm_context_interface::block::BlobExcessGasAndPrice;
use revm_primitives::{Address, B256, Bytes, FixedBytes, TxKind, U256};
...
impl Transaction {
    ...
    pub fn to_tx_env(&self) -> Result<TxEnv, String> {
        ...
        let kind = if let Some(to_addr) = self.to { TxKind::Call(to_addr) } else { TxKind::Create };
        ...
        builder.build().map_err(|e| format!("Failed to build TxEnv: {e:?}"))
    }
}
