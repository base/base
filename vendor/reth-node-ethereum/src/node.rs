...
///
/// Returns an interpreter-backed config, or an error if JIT was requested.
...
pub fn build_evm_config<C: EthereumHardforks>(
    ...
    if jit.enabled {
        eyre::bail!("JIT compilation is not supported by this vendored EVM");
    }
    ...
    Node: FullNodeTypes<Types = Types>,
{
    ...
    async fn build_evm(self, ctx: &BuilderContext<Node>) -> eyre::Result<Self::EVM> {
        ...
        let (mut evm_config, _) = build_evm_config(ctx.chain_spec(), jit, dump_dir)?;
