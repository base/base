...
impl<'a> B20PrecompileClient<'a> {
    /// Address of the signer used by this client.
    ...
    pub async fn pause(&self, token: Address, vectors: U256) -> Result<()> {
        let features = Self::pausable_features_from_mask(vectors);
    ...
    pub async fn unpause(&self, token: Address) -> Result<()> {
        let features = Self::pausable_features_from_mask(U256::from(0x0f));
    ...
    pub async fn send_and_wait(
        ...
        ensure!(receipt.transaction_hash() == expected_tx_hash, "{label} receipt hash mismatch");
        ensure!(receipt.inner.from == self.signer.address(), "{label} receipt sender mismatch");
        ensure!(receipt.inner.to == Some(to), "{label} receipt target mismatch");
        Ok(receipt)
    }
    ...
    /// Converts pause-vector mask bits into the ABI feature list.
    ///
    /// This spike intentionally duplicates the helper from `etc/systems/src/b20.rs` rather than
    /// coupling the acceptance crate to `base-system-tests`.
    pub fn pausable_features_from_mask(mask: U256) -> Vec<IB20::PausableFeature> {
