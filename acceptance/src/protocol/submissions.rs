...
impl Submissions {
    ...
    pub async fn wait_for_transfer(
        ...
        let batcher = rollup
            ...
            .system_config
            .as_ref()
            .ok_or_else(|| eyre::eyre!("generated rollup has no batcher identity"))?
            .batcher_address;
        let result = timeout(Duration::from_secs(120), async {
            loop {
                ...
                while self.next_block <= head.header.number {
                    ...
                    {
                        if transaction["from"] != json!(batcher)
        ...
        result.wrap_err_with(|| {
            format!(
                "no complete L1 batch channel for transfer {} at L2 timestamp {}; scanned through L1 block {}, {} incomplete channels",
                transfer.transaction_hash,
                transfer.block.header.timestamp,
                self.next_block.saturating_sub(1),
                self.observer.channels.len()
            )
        })?
    }
}
