...
impl BatchObserver {
    /// Consumes a decoded blob payload from a successful inbox submission.
    pub fn ingest(
        &mut self,
        payload: Bytes,
...
mod tests {
    ...
    use super::{super::BlobEvidence, BatchObserver, Submission, TransferTarget};
    ...
    fn attributes_blob_frames_to_the_exact_transfer_not_just_the_inbox_sender() {
        ...
        for (number, payload) in payloads(&target).into_iter().enumerate() {
            ...
            let submission = submission(11 + number as u64);
            attributed = observer
